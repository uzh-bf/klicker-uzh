...
case "${KLICKER_LOCAL_KB_RUNTIME_ONLY:-0}" in
  0|1) ;;
  *) echo '[post-start] ERROR: invalid isolated runtime mode.' >&2; exit 1 ;;
esac
...
# Real local retrieval is opt-in; unrelated checkouts must not inherit a signer.
if [ "${KLICKER_LOCAL_KB_RUNTIME_ONLY:-0}" != 1 ] && [ -n "${LOCAL_KB_SIGNER_ENV_FILE:-}" ]; then
  if [[ "$LOCAL_KB_SIGNER_ENV_FILE" != /* ]] ||
    [ ! -f "$LOCAL_KB_SIGNER_ENV_FILE" ] ||
    [ -L "$LOCAL_KB_SIGNER_ENV_FILE" ] ||
    [ ! -O "$LOCAL_KB_SIGNER_ENV_FILE" ] ||
    [ "$(stat -c '%a' "$LOCAL_KB_SIGNER_ENV_FILE" 2>/dev/null || stat -f '%Lp' "$LOCAL_KB_SIGNER_ENV_FILE")" != 600 ]; then
    echo '[post-start] ERROR: LOCAL_KB_SIGNER_ENV_FILE must be an absolute, owned, regular mode-600 file.' >&2
    exit 1
  fi
  # shellcheck source=/dev/null
...
if [ "${KLICKER_LOCAL_KB_RUNTIME_ONLY:-0}" = 1 ] && [ "$PROFILE_WANTS_MCP" = yes ]; then
  echo '[post-start] ERROR: isolated retrieval requires an explicit profile without the deterministic MCP fixture.' >&2
  exit 1
fi
...
if [ "$PROFILE_WANTS_DEV" = yes ] && [ -z "${HATCHET_CLIENT_TOKEN:-}" ]; then
  if [ "${KLICKER_LOCAL_KB_RUNTIME_ONLY:-0}" = 1 ]; then
    echo '[post-start] ERROR: isolated setup must supply the Hatchet token.' >&2
    exit 1
  fi

...
[[ "$(shasum -a 256 "$root/util/dev-runtime.sh" | cut -d " " -f 1)" = "68e3b9af6daaa18b844418c58a2f6debb94ebf6e89eb3af7c8ded03089c4af96" ]] || { echo "Recovery source changed: util/dev-runtime.sh" >&2; exit 1; }
...
[[ "$(docker exec "$app_id" sha256sum /workspaces/klicker-uzh/util/dev-runtime.sh | cut -d " " -f 1)" = "68e3b9af6daaa18b844418c58a2f6debb94ebf6e89eb3af7c8ded03089c4af96" ]] || { echo "Mounted runtime helper differs from reviewed source." >&2; exit 1; }

...
dependencies, clears only owned `.next/dev` output on an exact dependency
change, and checks semantic readiness for each Next.js app. If a route returns
the known stale `404` HTML response, it requests one bounded cache repair and
rechecks the apps; unexpected responses fail closed without deleting caches.

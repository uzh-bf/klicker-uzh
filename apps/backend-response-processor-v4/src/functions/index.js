...
import { computeAwardedPoints, computeAwardedXp, gradeQuestionFreeText, gradeQuestionKPRIM, gradeQuestionMC, gradeQuestionNumerical, gradeQuestionSC, } from '@klicker-uzh/grading';
...
const serviceBusTrigger = async function (queueItem, context) {
    ...
    try {
        ...
        if (queueItem.cookie) {
            try {
                ...
                participantData = JWT.verify(parsedCookie.participant_token, process.env.APP_SECRET);
            }
            catch (e) {
            ...
            if (participantData &&
                ...
                context.log('Participant has already responded to this question instance');
                return { status: 200 };
            }
        }
        ...
        const { type, solutions, startedAt, firstResponseReceivedAt, sessionBlockId, choiceCount, pointsMultiplier, } = instanceInfo;
        let parsedSolutions;
        try {
            if (solutions) {
        ...
        switch (type) {
            ...
            case 'KPRIM': {
                ...
                if (participantData) {
                    ...
                    if (pointsPercentage !== null &&
                        ...
                        redisExec.hset(`${instanceKey}:info`, 'firstResponseReceivedAt', responseTimestamp);
                    }
                    redisMulti.hset(`${instanceKey}:responses`, participantData.sub, response.choices);
                    redisMulti.hincrby(`${sessionKey}:b:${sessionBlockId}:lb`, participantData.sub, pointsAwarded);
                    redisMulti.hincrby(`${sessionKey}:lb`, participantData.sub, pointsAwarded);
            ...
            case 'NUMERICAL': {
                ...
                redisMulti.hset(`${instanceKey}:responseHashes`, responseHash, response.value);
                redisMulti.hincrby(`${instanceKey}:results`, 'participants', 1);
                ...
                if (participantData) {
                    ...
                    redisMulti.hset(`${instanceKey}:responses`, participantData.sub, response.value);
                    redisMulti.hincrby(`${sessionKey}:b:${sessionBlockId}:lb`, participantData.sub, pointsAwarded);
                    redisMulti.hincrby(`${sessionKey}:lb`, participantData.sub, pointsAwarded);
                    redisMulti.hincrby(`${sessionKey}:xp`, participantData.sub, xpAwarded);
                }
            ...
            case 'FREE_TEXT': {
                ...
                redisMulti.hincrby(`${instanceKey}:results`, responseHash, 1);
                redisMulti.hset(`${instanceKey}:responseHashes`, responseHash, cleanResponseValue);
                redisMulti.hincrby(`${instanceKey}:results`, 'participants', 1);
                if (participantData) {
                    ...
                    redisMulti.hset(`${instanceKey}:responses`, participantData.sub, cleanResponseValue);
                    redisMulti.hincrby(`${sessionKey}:b:${sessionBlockId}:lb`, participantData.sub, pointsAwarded);
                    redisMulti.hincrby(`${sessionKey}:lb`, participantData.sub, pointsAwarded);
                    redisMulti.hincrby(`${sessionKey}:xp`, participantData.sub, xpAwarded);
                }

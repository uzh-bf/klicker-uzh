...
// TODO: readd question attachments to evaluation view
...
function EvaluationLayout({
  ...
  const onToggleSolution = () => {
    ...
    sessionStorage?.setItem(
      ...
      JSON.stringify(newShowSolution)
    )
  }
  ...
  return (
    <Layout
      ...
      className="flex flex-col min-h-screen print:h-auto print:max-h-auto md:h-screen md:max-h-screen md:max-w-full evaluationLayout"
    >
      {((): React.ReactElement => {
        if (
          ...
          <div className="instanceChooser md:border-0 md:border-b md:border-primary md:border-solid md:pb-0 md:p-1.5 hidden md:flex flex-[0_0_auto] print:!hidden">
            ...
            >
              ...
              <div
                className={twMerge(
                  'instanceDropdown',
                  instanceSummary.length >= 7 ? 'block' : 'hidden'
                )}
              >
                <Menu.Item
                  fitted
              ...
              {instanceSummary.length < 7 &&
                instanceSummary.map(
                  (
                    { id, blockStatus, title, totalResponses: count },
                    index
                  ): React.ReactElement => (
                    <Menu.Item
                      fitted
              ...
              {sessionStatus === SESSION_STATUS.COMPLETED && existsFeedback && (
                <Menu.Item
                  ...
                  onClick={(): void => {
                    ...
                    activateInstance(
                      existsConfusion ? numberOfTabs - 2 : numberOfTabs - 1
                    ) // if there is a confusion-tab, the tab is the second to last, otherwise the last
                  }}
                >
                  Feedback-Channel
                </Menu.Item>
      ...
      <div className="bg-primary-bg">
        ...
        >
          <p>
            {(showQuestionLayout && (
              <Description formatted content={description} />
            )) ||
        ...
        !showConfusionTS ? (
          <div
            className={twMerge(
              ...
              showQuestionLayout
                ? 'md:border md:border-solid md:border-gray-300'
                : '0'
            )}
          >
            {children}
                ...
                {QUESTION_GROUPS.WITH_STATISTICS.includes(type) &&
                  ...
                  !showFeedback && (
                    <div className="mt-6">
                      <Statistics
                        {...statistics}
                        withBins={activeVisualization === CHART_TYPES.HISTOGRAM}
                      />
                    </div>
                  )}
              </>
        ...
        {`
          ...
          .evaluationLayout {
            .questionDetails {
              ...
              :global(.description ul:not(:last-child), .description
                  ol:not(:last-child)) {
                margin-bottom: 0.7rem;
              }

              :global(.description ul:not(:first-child), .description
                  ol:not(:first-child)) {
                margin-top: 0.7rem;
              }
            }

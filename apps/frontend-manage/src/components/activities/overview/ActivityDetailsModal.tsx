...
function ActivityDetailsModal({
  ...
  const basePointsActivity = metadata?.totalBasePoints ?? 0
  const correctnessPointsActivity = metadata?.totalCorrectnessPoints ?? 0
  const bonusPointsActivity = metadata?.totalBonusPoints ?? 0
  const totalPointsActivity = metadata?.totalPoints ?? 0
      ...
      <UserNotification type="warning" className={{ root: 'mb-4' }}>
        {t.rich(
            ...
            : 'manage.activities.instanceUpdateDraftScheduled',
          {
            ...
            li: (content) => <li className="mt-0.5 last:hidden">{content}</li>,
          }
        )}
      </UserNotification>
    )
      ...
      <div className="flex h-auto min-h-0 flex-col gap-4 xl:h-full xl:max-h-full xl:flex-row">
        <div className="flex min-h-0 w-full flex-col gap-2 xl:max-h-[calc(100vh-13rem)] xl:w-2/5 xl:overflow-y-auto xl:pr-10">
          ...
          <ActivityPointsTable
            ...
            basePoints={basePointsActivity}
            correctnessPoints={correctnessPointsActivity}
            bonusPoints={bonusPointsActivity}

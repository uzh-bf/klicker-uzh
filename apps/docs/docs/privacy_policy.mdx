...
In this statement, we explain what user data is collected, processed and shared, for what purpose, and what measures are taken to ensure the security of this data. All measures are in accordance with the applicable data protection regulations. This statement refers to details that are specific to "KLICKERUZH". The [UZH data protection requirements](https://www.uzh.ch/en/privacy.html) apply in addition.
...
For normal course functions, such as running and evaluating activities, ADMINISTRATORS are given access to specific data: PARTICIPANT profile information (email and pseudonym, if provided), their responses and feedback in the applicable evaluation view, and optionally collected points. This normal course use is separate from the optional learning analytics described below.
...
### Optional learning analytics

Learning analytics can help ADMINISTRATORS identify patterns in course activity and improve course support, activities, and feedback for students. Learning analytics is disabled by default. If ADMINISTRATORS enable learning analytics for a course, PARTICIPANTS make a neutral, voluntary choice about whether their data may be used for it. They may change this choice at any time. Choosing not to participate does not affect course access, normal activities, assessment, feedback, points, or achievements. The learning-analytics choice is separate from any research participation and does not constitute consent to research.

Dedicated learning-analytics results may be computed from participating persons' normal course use. The data categories used are interaction activity, learning progress, response performance, scores, and feedback ratings. Free-text responses and free-text feedback are not used for learning analytics or combined with learning-analytics results.

ADMINISTRATORS do not see names, email addresses, participant identifiers, or participation choices in learning analytics. Participant-level tables use fresh report-specific labels such as "Student 1" and show only coarse performance and coverage information. Results are shown only when at least five participating persons contribute after the selected filters. These labels and results are de-identified but must not be understood as guaranteed anonymous. ADMINISTRATORS must not attempt to identify persons or combine learning analytics with other information for that purpose. Counts of participants who participate, do not participate, or have not decided are not shown.

First or renewed participation applies from the time of the choice. Periods during which a person personally chose not to participate are not included later. When a person chooses not to participate, participant-level learning-analytics results are hidden and deleted immediately, and that person's activity is excluded from future learning-analytics calculations. Existing aggregate results are updated on the normal calculation schedule and are not necessarily recomputed immediately.

If ADMINISTRATORS disable learning analytics for the whole course, the learning-analytics views are hidden immediately and all dedicated learning-analytics results are deleted. Normal course data and PARTICIPANTS' saved choices remain. If course learning analytics is later re-enabled, permitted learning analytics may be recomputed from normal course data; a course-level disabled period is not categorically blocked. A period of personal non-participation, however, is not backfilled after a later renewed participation.
...
| Interaction activity, learning progress, response performance, scores, and feedback ratings                                         | PARTICIPANTS who choose learning analytics    | Improvement of course support and teaching   | Optional learning analytics only; de-identified, coarse results from at least five persons |

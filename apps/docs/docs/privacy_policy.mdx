...
If the learning-analytics choice is **Yes**, eligible individual analytics may be produced from the current inclusion boundary. If it is **No**, individual analytics are filtered immediately; individual derived analytics are physically removed in the next successful overnight cleanup, while aggregate analytics remain until ordinary recomputation. Re-enabling the choice starts inclusion only with later activity and does not backfill earlier activity.

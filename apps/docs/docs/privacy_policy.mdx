...
If the learning-analytics choice is **Yes**, eligible individual analytics may use all eligible stored activity data for the account after a successful course recomputation whose marker is strictly newer than the current choice. If it is **No**, no individual learning-analytics data is used; existing individual learning-analytics data is physically removed in the next successful overnight cleanup, while aggregate analytics remain until ordinary recomputation. Re-enabling the choice allows all eligible stored activity data to be included again.

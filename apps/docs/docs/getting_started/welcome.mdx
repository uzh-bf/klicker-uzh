...
## Official KlickerUZH Instance

The public instance of KlickerUZH (www.klicker.uzh.ch) is provided by the Department of Finance of the University of Zurich. The application is hosted on Microsoft Azure and within Europe. Data processing is outlined further in our [Privacy Policy](/privacy_policy).

The core functionalities (live quiz, courses, gamification) are freely accessible for anyone logging in to our public instance. For more advanced current and future features, especially features that generate a lot of load on our resources and therefore cannot be made available for free outside of UZH, a participation in our Catalyst program is required (see [Catalyst](/catalyst) for more information). Users at the University of Zurich and affiliated institutions are automatically part of the program, if their Edu-ID is linked to their UZH account.
...
In the spirit of open source, we welcome any good-spirited contributions to the KlickerUZH (be it from institutions or individuals). It is generally a good idea to first get an orientation about the general direction of the project from the Roadmap. Please feel free to create an issue or discussion on the [klicker-uzh](https://github.com/uzh-bf/klicker-uzh) project if you are unsure about your idea or the next steps to its implementation.

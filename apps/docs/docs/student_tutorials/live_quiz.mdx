...
The quiz screen states how responses are handled. In the default mode, KlickerUZH keeps only anonymous, aggregated results. Some lecturers may enable correlated response storage: your answers are then linked across that quiz under a pseudonymous identifier, including when you participate without logging in. Lecturer downloads do not contain your name, account identifier, or response timestamps. Free-text answers are exported as entered, so avoid including personal information unless the question explicitly requires it.

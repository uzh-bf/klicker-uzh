...
If no KlickerUZH account exists for a student yet, they will be greeted by a welcome page, which will allows them to create a new account. On this page they set an (anonymous) username and password, as well as their personal avatar.
...
In general, it is also possible to participate anonymously in all elements of KlickerUZH. For live quizzes, students can find an overview of all running instances of a course through the lecturers shortname: `https://pwa.klicker.uzh.ch/join/{shortname}`

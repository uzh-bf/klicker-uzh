...
:::info
Selection questions require students to choose a specific number of items from a predefined list, known as an **Answer Collection**. You present a question or scenario, define how many answers the student must select, specify which items from the linked Answer Collection are the correct solutions, and optionally provide a general explanation. This type is useful for scenarios where students need to identify multiple correct elements from a larger set (e.g., identifying indicated medications, relevant symptoms, or applicable concepts). All text fields support editor formatting, Markdown, and LaTeX syntax, and can include images. The preview shows the student's perspective.
:::
...
Students are presented with a case and must provide a value for each criterion for every listed item, usually via sliders or input fields constrained by the defined ranges. This type is ideal for simulating complex decision-making processes where multiple factors must be weighed across different scenarios. All text fields support editor formatting, Markdown, and LaTeX syntax, and can include images. The preview shows the student's perspective.
:::

...
Different access levels allow you to restrict the actions that are possible when using a delegated login. Currently, the only available access setting is _full access_, which allows users to perform all actions that are possible with your owner account, except for the creation of new logins. In the future, more fine-grained access levels will be available.

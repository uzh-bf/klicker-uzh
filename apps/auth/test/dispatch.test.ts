...
describe('auth dispatch', () => {
  test('parseAuthAction extracts action and provider from path segments', () => {
    expect(parseAuthAction(['signin', 'eduid'])).toEqual({
      action: 'signin',
      providerId: 'eduid',
    })
    expect(parseAuthAction(['callback', 'eduid'])).toEqual({
      action: 'callback',
      providerId: 'eduid',
    })
    expect(parseAuthAction(['session'])).toEqual({
      action: 'session',
      providerId: undefined,
    })
    expect(parseAuthAction(undefined)).toEqual({
      action: 'unknown',
      providerId: undefined,
    })
    expect(parseAuthAction(['bogus'])).toEqual({
      action: 'unknown',
      providerId: undefined,
    })
  })
  ...
  test('callback with a matching participant state cookie resolves participant', async () => {
    ...
    expect(result).toEqual({ audience: 'participant' })
  })
  ...
  test('callback with a matching lecturer state cookie resolves lecturer', async () => {
    ...
    expect(result).toEqual({ audience: 'lecturer' })
  })
  ...
  test('an attempt issued for one audience cannot be resolved as the other', async () => {
    ...
    // participant cookie is missing entirely (expired/cleared): no candidate may
    // match and the result must be a safe failure, not a lecturer default.
    ...
    expect(result).toEqual({ audience: null, reason: 'no_state_match' })
  })
  ...
  test('overlapping attempts keep their own state and destination', async () => {
    // Both audiences started concurrently: each callback resolves only its own
    // attempt even though both cookies coexist in the browser.
    ...
    expect(participantResult).toEqual({ audience: 'participant' })
    ...
    expect(lecturerResult).toEqual({ audience: 'lecturer' })
  })
  ...
  test('missing state and duplicate state parameters fail safely', async () => {
    ...
    ).toEqual({ audience: null, reason: 'invalid_state_param' })
    ...
    ).toEqual({ audience: null, reason: 'duplicate_state' })
  ...
  test('a state value matching both audience cookies is ambiguous and rejected', async () => {
    // A duplicated state value across both cookie namespaces must never resolve
    // to a default audience.
    ...
    expect(result).toEqual({ audience: null, reason: 'ambiguous_state_match' })
  })
  ...
  test('provider error callbacks are detected for controlled failure', () => {
    ...
    expect(isProviderErrorCallback({ state: 'x' })).toBe(false)
    expect(isProviderErrorCallback({ error: '' })).toBe(true)
    expect(isProviderErrorCallback({ error: ['a', 'b'] })).toBe(true)
  })
})

...
export default async function dataUseAssets(
  req: NextApiRequest,
  res: NextApiResponse
) {
  ...
  try {
    ...
    if (asset === 'guide') {
      ...
      const html = content
        .toString('utf8')
        .replace(
          '<a class="back" href="prototype.html">Zurück zum Prototyp</a>',
          '<a class="back" href="/de/account/data-use">Zurück zu deinen Einstellungen</a>'
        )
        .replaceAll(
          'href="outputs/consent-la-download-examples/',
          'href="/api/data-use-assets/'
        )

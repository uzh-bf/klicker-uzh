...
def aggregate_analytics(df_details, include_first_last=False):
    ...
    df_analytics_corr_temp = (
        df_details.groupby(["participantId", "elementInstanceId", "courseId", "correctness"])
        .size()
    ...
    correctCount = df_analytics_corr_temp["CORRECT"] if "CORRECT" in df_analytics_corr_temp.columns else 0
    partialCount = df_analytics_corr_temp["PARTIAL"] if "PARTIAL" in df_analytics_corr_temp.columns else 0
    incorrectCount = df_analytics_corr_temp["INCORRECT"] if "INCORRECT" in df_analytics_corr_temp.columns else 0
    ...
    if include_first_last:
        ...
        df_course_analytics.columns = df_course_analytics.columns.map("_".join).str.strip("_")

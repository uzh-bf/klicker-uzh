-- AggregatedChatbotAnalytics rollup for a single (analytics_type, timestamp, [win_start, win_end)) window.
-- Parameters:
...
-- For WEEKLY the newParticipants column is the count of participants whose first-ever
-- user-role message to the chatbot fell inside [win_start, win_end). For other types it
-- is set equal to activeParticipants and returningParticipants is set to 0 (the plan
-- notes new/returning is meaningful only for WEEKLY).
...
first_seen AS (
  -- first-ever user message per (participant, chatbot), over ALL history
...
disclaimer_counts AS (
  -- snapshot counts (not window-scoped; disclaimer acceptance is a one-time consent event)
  SELECT
    "chatbotId",

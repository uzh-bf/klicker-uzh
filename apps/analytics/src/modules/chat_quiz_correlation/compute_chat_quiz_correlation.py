...
def assert_preconditions(session: Session, verbose: bool = False) -> None:
    ...
    perf_rows = session.execute(
        text('SELECT COUNT(*) AS n FROM "ParticipantPerformance"')
    ).scalar_one()
    if verbose:
        print(
...
def compute_participant_chat_outcomes(session: Session, verbose: bool = False):
...
def update_has_chat_activity(session: Session, verbose: bool = False):

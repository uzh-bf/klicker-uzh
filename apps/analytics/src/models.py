"""SQLAlchemy declarative models mirroring the Prisma schema.

Intended to be regenerated by ``sqlacodegen`` against the live dev DB:

    pnpm run prisma:setup
    pnpm --filter @klicker-uzh/analytics run generate

The current hand-authored version covers every model the analytics pipeline
reads or writes; tables not touched here still exist in Postgres (managed by
the TypeScript side's Prisma migrations) and can be introspected via
``sqlacodegen`` if they become relevant.

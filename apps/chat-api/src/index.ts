...
app.post('/api/chatbots/:chatbotId/chat', async (c) => {
          ...
          messages: [
            {
              ...
              content: [
                ...
                { type: 'text', text: descriptionPrompt(lastMessage?.content) },
              ],
            },
          ],
    ...
    onAbort: async () => {
      ...
      if (
        ...
        try {
          ...
          await persistAssistant(partial, abortedReasoningContent, creditsUsed)
        } catch (error) {
          console.error('Failed to save partial message:', { requestId, error })
        }
      }
    },

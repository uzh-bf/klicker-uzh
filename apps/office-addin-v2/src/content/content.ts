...
const contentIframe = document.getElementById('content-iframe') as HTMLIFrameElement | null;
const messageBox = document.getElementById('message-box') as HTMLDivElement | null;
const messageText = document.getElementById('message-text') as HTMLSpanElement | null;
const embedButton = document.getElementById('embed-button') as HTMLButtonElement | null;
const changeEmbeddedUrlButton = document.getElementById('change-embedded-url-button') as HTMLButtonElement | null;
...
async function loadInitialState(): Promise<void> {
  ...
  const savedUrl = Office.context.document.settings.get(SETTINGS_KEY) as string | undefined;
  ...
  if (savedUrl && isValidUrl(savedUrl)) {
    ...
    try {
      ...
      const legacyUrl = Office.context.document.settings.get(legacyKey) as string | undefined;
...
async function getSlideID(maxRetries = 3): Promise<number> {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      retryCount++;
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.log(`Attempt ${retryCount} of ${maxRetries} failed:`, errorMessage);
 ...
 */
function saveUrlToSettings(url: string, callback?: (success: boolean) => void): void {
 ...
 */
function showMessage(message: string, type: MessageType, duration = 3000): void {
  console.log(

...
export class AuditTableClient {
  ...
  async ensureTable(): Promise<void> {
    try {
      ...
      logger.info({ tableName: this.client.tableName }, 'Azure Table created or already exists')
    } catch (error) {
      // Ignore "table already exists" errors
      if (error instanceof Error && error.message.includes('TableAlreadyExists')) {
        logger.debug({ tableName: this.client.tableName }, 'Table already exists')
        return
      }
  ...
  async upsertEntity(entity: AuditTableEntity): Promise<void> {
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('RequestBodyTooLarge') || error.message.includes('EntityTooLarge')) {
          throw new Error('Entity exceeds Azure Table Storage size limits')
        }
        
        // Throttling (429)
        if (error.message.includes('ServerBusy') || error.message.includes('TooManyRequests')) {
          logger.warn({
            partitionKey: entity.partitionKey,
            error: error.message,

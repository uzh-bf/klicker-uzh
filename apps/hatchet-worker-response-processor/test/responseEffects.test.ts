...
describe('live quiz response effects', () => {
  it('validates assessment responses with cached instance metadata', () => {
    assert.equal(
    ...
    assert.equal(
      validateStudentResponse({
        type: 'CASE_STUDY',
        response: {
          assessment: { caseA: { '1': { criterion: 2 } } },
        },
        instanceInfo: {
          caseStudyResponseShape: JSON.stringify({
            cases: ['caseA'],
            items: [1],
            criteria: [{ id: 'criterion', min: 0, max: 5 }],
          }),
        },
      }).valid,
      true
  ...
  it('keeps legacy aggregate and assessment cache shapes valid', () => {
    assert.equal(
      validateStudentResponse({
        type: 'SELECTION',
        response: { selection: [11, -1] },

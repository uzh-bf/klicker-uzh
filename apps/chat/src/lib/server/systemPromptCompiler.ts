...
export function compileSystemPrompt(
  ...
  const images = context.toolNames.includes('show_course_image')
    ? `${citations}\n\nCourse images: for course-content questions, search with doc_query before answering so you can assess the available evidence and figures. When the retrieved text clearly connects a figure to the relationship, process, structure, or comparison the student is trying to understand, call show_course_image proactively in the same answer. Do not ask whether the student would like to see it; an explicit image request is not required. Select only an asset_id from the current search visual_assets and provide a short reason grounded in that passage. A shared page or a vaguely related topic is not enough. Prefer one useful figure; do not add images to simple factual answers or when the student requests text only. For explicit image requests, select matching available figures. If there is no supported match, continue in text; mention the missing image only if one was requested. Briefly explain the selected figure’s relevance in the answer. The UI displays the original image and source page. You have not inspected its pixels: do not infer colors, arrows, labels, or other visual details beyond retrieved evidence. Never output Markdown image URLs or fabricate image references.`
    : citations

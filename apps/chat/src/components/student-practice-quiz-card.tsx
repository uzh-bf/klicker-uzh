...
} from '@klicker-uzh/shared-components/src/elementTypes'
...
function getEvaluations(submission: unknown): StudentInstanceEvaluation[] {
  const grading = getGradingPayload(submission)
  if (!Array.isArray(grading?.evaluations)) return []
  return grading.evaluations.filter(Boolean) as StudentInstanceEvaluation[]
}

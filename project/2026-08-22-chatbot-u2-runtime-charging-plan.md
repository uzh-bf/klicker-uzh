...
- 2026-08-26: exact-stack final review found that the terminal-only message
  ...
  production build succeeds. The correction simplifier found no material
  simplification. The lifecycle-integrity review then found two blockers: a
  first-turn retry without a thread ID could create a second thread instead of
  reclaiming the failed attempt, and in-progress collisions exposed a distinct
  response from completed or foreign-scope conflicts.
- 2026-08-26: the lifecycle review corrections reuse the participant-owned
  failed-attempt thread when a retry omits its thread ID, discard a newly
  created transient thread when pre-provider claim or required-MCP preflight
  fails, and return the same generic conflict for in-progress, completed, and
  foreign-scope collisions. The focused route portfolio passes 21 tests and

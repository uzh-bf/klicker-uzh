...
Execution active after restored Git writes. Source branch `rs/required-ci-rulesets` starts at stable `9d258244535f855b96d9548262f57ac639f83124`; controller commits add mandatory exact candidate CI, eight Playwright shard jobs, bounded selection artifacts and manual controller/release preconditions. Focused controller tests: 26 passing. Suite and image reporters are under implementation and review.

Live integration CI ruleset 23042613 now matches `refs/heads/v3-*` and allows administrator PR-only bypass. Stable and structural rulesets have no bypass. Full required contexts await reporter rollout. Current release remains `9cb4042334751fd80fde5b95319a6c41cdd4cdaa`; selector remains `v3-ai`, promotion enabled. No source merge or staging write has occurred.

Audit image repair is [draft PR #5917](https://github.com/uzh-bf/klicker-uzh/pull/5917), branch `rs/audit-worker-image-pins`, head `3b5c64cb5b499c2e70a7ead9e3a3b662fe19c0b8`. Actual Helm cases and four image tests pass. Slice review found no source correctness issue; stale docs corrected. Existing automated render coverage omits enabled audit; actual focused renders satisfy the plan's verification boundary. CI and final review pending.

Synchronization preview: stable-to-AI has one conflict in `apps/chat/src/lib/sources/normalizeSources.ts`; AI-to-audit merges cleanly at current selected refs. Earlier PRs include broader production changes, so prepare narrow successor synchronization PRs with predecessor links after controller delivery.

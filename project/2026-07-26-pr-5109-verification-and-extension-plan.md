...
| X3 | Converge lecturer READ authZ — route through the persisted-GraphQL resolver path (carries `withPermission`) or a shared `checkAccess`-equivalent; never a bare service-function call | §1 + §5.1a — **resolver-wrapped service functions have no internal permission filter; a direct call skips authZ entirely** | Correctness gate, not cleanup: naively wrapping a resolver-wrapped read as an MCP tool is an authZ bypass, and every new read multiplies the drift surface | M |

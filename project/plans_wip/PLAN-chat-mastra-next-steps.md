...
**Single most important prerequisite.** The prototype deliberately uses `provider.chat()` (Chat Completions) in `engine/agent.ts` to dodge the Responses-API tool-call breakage. Whether the configured `OPENAI_BASE_URL` (OpenRouter / Azure) surfaces reasoning tokens for a reasoning-capable model in Chat Completions streaming is the open question — it has historically varied. Verify this first; the answer determines whether the rest of A2 is a half-day or a multi-day provider-branching exercise.

**Concrete steps.**

| Step | Description | File(s) |
|------|-------------|---------|
| 1 | **Transport check (one-time):** raw Chat Completions streaming request to `OPENAI_BASE_URL` with a reasoning model; check for a `reasoning_content` field on any chunk. If absent, evaluate a direct OpenAI key or Azure Responses API — which expands scope (per-capability model instantiation in `agent.ts`). Document the result. | — |
| 2 | Create `src/check-reasoning.ts`: instantiate an agent with the reasoning model, stream a "think step by step" prompt with `providerOptions: { openai: { reasoningEffort: 'low' } }`, assert ≥1 `reasoning-start` and ≥1 non-empty `reasoning-delta` in the raw Mastra stream, then assert the same after `toAISdkStream({ sendReasoning: true })`. Respect Mastra's single-consumer stream semantics (collect via callback, do not double-read). | New file |
...
| 4 | Accumulate reasoning text via `onStepFinish` (`AgentStreamOptions`, types.d.ts ~line 681–759) rather than re-reading the stream; confirm step results carry reasoning text. | `prototype/mastra-chat/src/server.ts` |
...
| Risk | Mitigation |
|------|-----------|
| Provider does not surface reasoning tokens via Chat Completions streaming. | Switch to a direct OpenAI key or Azure Responses API; branch model instantiation per capability (Chat Completions for tool models, Responses for reasoning). |
...
| Single-consumer `MastraModelOutput` blocks accumulate-and-pipe. | Use `onStepFinish` as a side-effect collector (step 4). |

**Dependencies.** A reasoning-capable model on the configured provider. No frontend changes.
...
| 18 | Replace `streamText` in `apps/chat-api/src/routes/chat.ts` with a per-request Mastra `Agent` via `buildAgent` from `packages/chat-engine`; replace `getAggregatedMCPTools` with `buildMcpToolset`; convert via `toAISdkStream({ from:'agent', version:'v6', sendReasoning:true, messageMetadata })`. Pin `provider.chat()` (Chat Completions — non-negotiable, per the prototype). Remove the `responsesApiFetch` workaround. Wire the resolved model id from Mastra step metadata into the finish shim (fixes the S0 fallback-attribution gap). Keep all persistence/credit callbacks. | `apps/chat-api/src/routes/chat.ts` |

...
- Post-merge CI correction 2026-09-14: the first CI run on merge head `2f4316c574` passed `pnpm install --frozen-lockfile`, the pull-request commit-identity guard and the hosted-cache and CI-queue policies, then failed in `check` at "Check Playwright host launcher". Root cause was ours, not upstream's: this branch had removed the external `klicker-uzh-pnpm-store-v1` declaration from `.devcontainer/docker-compose.yml` so the worktree used project-scoped pnpm storage, while upstream's generated dependency mounts and `util/run-playwright-host.test.mjs` now require that store to stay external and shared; the current `v3` check run passes, confirming the deviation. Restored upstream's declaration, which retires the pnpm-store isolation workaround in favour of the supported shared store. `format:check` also failed on the katex syncpack label line, which exceeded the formatter's width under the merged Biome configuration; re-wrapped by running the repository formatter. Verification on the corrected tree: `run-playwright-host` 39/39, `generate-dependency-mounts` 7/7, `next-pages-development-config` 1/1, `format:check` clean across 1939 files, `syncpack lint` valid, prisma-sync, AGENTS.md and removed-wiki-path checks pass, Markdown typecheck plus 53 tests pass, Chat typecheck plus 656 tests pass (60 files, 21 skipped), and the CI-filtered Turbo build and typecheck both pass at 15/15 tasks. Two local-environment notes: stale September 10 `.next` directories in pwa, manage, control and chat produced duplicate `PagesPageConfig` type errors that the runtime's cache repair addresses, so they were moved aside rather than deleted, and the Prisma client needed local regeneration for the new upstream `aiFeaturesEnabled` field.
- Gate and evidence correction 2026-09-14: the account quota reset recorded earlier as 2026-09-15 is actually 2026-09-20 08:36 UTC (primary weekly window at 100% used, zero reset credits), so the layer-2 planner gate stays blocked for longer than the previous entry stated and no planner recheck was attempted. GitGuardian reports two findings on the merge head; the local gitleaks scan is clean over both the eight-commit range and the merge commit's full 5.93 MB diff, and the only secret-shaped match in that diff is a JWT-like substring inside a `sha512` integrity digest in upstream's `pnpm-lock.yaml`, so these read as false positives that need dashboard dismissal by someone with GitGuardian access. PR #5870 is now `MERGEABLE` (was `DIRTY`) and still draft; #5676 remains open and unmerged, so Stack B stays blocked.
- Merge-integration checkpoint 2026-09-14: integrated `origin/v3` into `rs/scientific-visuals-isolated` and committed merge `6d9f1eae0d` (merge base `0c2a7a6a33`), clearing the PR #5870 `DIRTY` merge state; the branch is now 0 behind and 7 ahead of its target. Five files conflicted and were resolved deliberately. `.devcontainer/post-start.sh` keeps upstream as its base with both review fixes re-applied: the `DEV_BUILD_FILTERS` computation and the `--prepare-command` capability probe stay gated on the dev profile, `start_managed_runtime` builds `prepare_args`, and the confirmed stale-cache repair restart passes `no-prepare`; `bash -n` passes. `util/dev-runtime.sh`, `util/dev-runtime-readiness.test.mjs` and the root `package.json` took upstream, because upstream is a strict superset of those earlier review fixes: same bounded probe timeouts and curl-status messages, the hanging-response test carried over, and a `test:dev-runtime` that already chains the readiness test with `util/test-recover-bootstrap.sh`. `pnpm-lock.yaml` took upstream and was then regenerated with the repository-pinned Node 24.16.0 and pnpm 11.5.0, so its entire delta against upstream is the 15 added lines that register katex for the five exact-pin importers plus the docs tilde range, introducing no dependency drift. Local pre-checks pass: the commit-identity range guard accepts `merge-base..HEAD`, and `syncpack lint` reports the katex alignment group valid across all five production surfaces. The readiness suite cannot run on this host because another process already holds 127.0.0.1:3002, which matches the container-only test rule; `.github/workflows/check.yml` runs that suite together with `pnpm install --frozen-lockfile` and the full check set, so exact-head CI on the merged head is the authoritative verification for this resolution. Chemistry and biology source is untouched by the merge, so the recorded chemistry render, docs-build and browser evidence still applies. The draft gate and the pending `final-ai-review` status are unchanged, and Stack B still waits on the unmerged #5676.
- Head CI and quota checkpoint 2026-09-10: exact-head CI on c476f42613 is fully green (initialize, resolve_lock, trusted_policy, Analyze x3, GitGuardian, ocr-review pass at 17m30s); final-ai-review stays pending on the draft gate (manual z-ai/glm-5.3-flash final review required for this head) and needs the explicit ready-mark approval plus the standing-authorized /final-review repost, both still awaiting the user decision from the preceding checkpoint. Account usage evidence confirms quota exhaustion as the planner-gate cause: the primary codex limit is 100% used on the weekly window resetting 2026-09-15, so no planner recheck was attempted per the routing recheck rule, and reset controls remain manual-only with the user. This is the second consecutive goal turn with the same blocker pair (ready-mark authority + planner quota); a third unchanged continuation triggers the blocked audit.
- Layer 2 gate checkpoint 2026-09-10: PR #5870 remains draft - the explicit ready-marking approval question returned no answer, so no ready-mark, /final-review repost, or final-ai-review run occurred (the earlier standing-authorized comment remains consumed by the draft-state rejection). Planner gate for feat/scientific-visual-display-contract: the native planner dispatch (Galileo, gpt-6-astra) and the single permitted continuity child (Pauli, 01a088b8-a3e7-7720-9c14-1e31e7dfc27d, gpt-5.6-sol xhigh, clean context, complete dispatch manifest) both failed terminally before work with HTTP 429 Too Many Requests; per the routing continuity ladder no third child was launched, so the required planner pass is BLOCKED until quota or runtime evidence changes. Non-gate work continued in the main session: a layer-2 concrete package draft was prepared, grounded in the prototype results (KET authority, FASTA header normalization, unsuitable biology PNG depictions, unresolved preview-trust decision) and the plan layer-2 test portfolio rows; it is held uncommitted in the gitignored path project/_local/plans/2026-09-10-visual-display-contract-package-draft.md pending the planner challenge, and is neither presented for agreement nor committed. Stack B prerequisite PR #5676 re-verified OPEN (not merged); no Stack B work is unblocked.

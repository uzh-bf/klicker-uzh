...
| Skills | Mastra | Can DB-backed, versioned tutor guidance packages replace the flat per-mode prompt, with progressive disclosure and no required lecturer authoring? | Agent loads a skill on demand and applies it | **adopt-with-changes** (S2; thin tools until `WorkspaceSkillsImpl` exported) |

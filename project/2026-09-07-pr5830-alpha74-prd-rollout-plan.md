...
Status: draft [PR #5830 for the alpha.74 production rollout](https://github.com/uzh-bf/klicker-uzh/pull/5830) is published. Fifteen tag changes are committed and verified. Helm lint passes. Baseline and candidate renders differ only in sixteen image references; all other bytes match. All thirteen referenced repositories resolve to alpha.74 ARM manifests with recorded OCI index digests. Slice and integrated final reviews pass with no findings against draft preparation. The merged CI fix at `7592bc8e2b246c1a6f5e6883d24968ad022cb3f1` is integrated without changing the reviewed deployment diff. Current-head platform checks remain pending. Preparation is complete; no production action is authorized.

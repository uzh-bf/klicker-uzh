...
User approval received on 2026-09-08. Source inspection is complete; implementation is starting. Native planner review: round 1 REVISE; all four findings accepted; round 2 APPROVED. The final wording clarifies that alias stripping preserves required canonical destination variables. Optional external review excluded because the draft is unpublished; no private configuration or credentials are eligible for that route. Full agent-consumed wording follows writing-for-agents; secret-operation procedure follows rs-infisical-operator.

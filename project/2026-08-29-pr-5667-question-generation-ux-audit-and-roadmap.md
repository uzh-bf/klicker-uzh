...
- **Status — 2026-08-30:** published head is `befe4b97a34deefa3384aed832d72d7e5b66fae0`; draft PR [#5667](https://github.com/uzh-bf/klicker-uzh/pull/5667) is open, mergeable, and exactly based on dependency head `8971f63b9e5df9fba096a9b6bac34fa881feb3cd`. All exact-head GitHub Actions branch checks are terminal pass or intentionally skipped, including repository, GraphQL, OpenCodeReview, gitleaks, GitGuardian, trusted-policy, and MCP checks. The external `final-ai-review` status remains pending only because the repository asks for a manual z-ai review; its trusted-policy and initialization jobs passed. There are zero unresolved non-outdated review threads. The task worktree remains dirty only for unrelated user-owned `.devcontainer/devcontainer.json` and `.devrouter.yml` changes, which remain excluded.
...
- **Stack evidence:** #5635 remains the immutable dependency at `8971f63b9e5d`; #5667 reports the same exact base and remains draft. The published implementation branch is 20 commits ahead and zero behind that dependency. No merge, rebase, force-push, ready transition, or dependency edit is authorized.
...
- **Remaining:** the English/German desktop browser matrix at `823 × 863` and `1440 × 900` is the only missing local behavior proof. It is `delivery_pending` on a healthy task route publication path; do not repeat the same restart loop or modify the user-owned runtime files without a separate runtime fix decision. The manual z-ai status also remains a separate review-infrastructure disposition before any ready transition. The PR stays draft.

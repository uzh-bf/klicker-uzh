...
- **Participant email is currently a global auth/linking key**: Non-assessment account creation, magic-link login, LTI account linking, Edu-ID assessment linking, invitations, leaderboards, and exports still read/write `Participant.email` and/or `ParticipantAccount.ssoEmail`; privacy work must stop new writes before dropping fields. (`packages/graphql/src/services/accounts.ts`, `apps/auth/src/lib/helpers.ts`, `packages/prisma/src/prisma/schema/participant.prisma`)
- **Participant merges must be course-aware**: `Participation`, `QuestionResponse`, `LiveQuizResponse`, `LeaderboardEntry`, analytics/performance rows, achievements, push subscriptions, and chat usage credits have participant/course or participant/instance uniqueness constraints. Duplicate manual/LTI accounts can only be merged safely through a dry-run planner plus per-course selected-side decisions; derived analytics/leaderboards should be recomputed, not blindly summed. (`packages/prisma/src/prisma/schema/participant.prisma`, `packages/prisma/src/prisma/schema/response.prisma`, `packages/prisma/src/prisma/schema/analytics.prisma`, `packages/prisma/src/prisma/schema/chat.prisma`)
- **Current `ParticipantAccount` shape blocks multiple same-type LTI links**: `ParticipantAccount` uses `@@unique([participantId, ssoType])`, so a future "one participant, many LMS identities" design needs a `ParticipantExternalIdentity` table or relaxed uniqueness keyed by external subject hash instead. (`packages/prisma/src/prisma/schema/participant.prisma`)
- **LTI 1.1 participant flow is not independently verified in PWA**: `apps/frontend-pwa/src/lib/getParticipantToken.ts` signs POSTed `lis_person_sourcedid` / email into internal JWT after parsing the request body and carries a TODO to verify the LTI body. Any trust-model change that removes email fallback needs either verified LTI 1.1 handling or retirement of that path.
- **No general participant inbox exists**: Current participant-facing messaging is group-scoped (`GroupMessage`) and push subscriptions exist, but actual web-push sending is commented/stubbed in `handleSendPushNotifications`; migration notices need email-before-purge plus a new in-app checklist/notice surface if they must reliably reach existing students. (`packages/graphql/src/services/notifications.ts`, `packages/prisma/src/prisma/schema/participant.prisma`)

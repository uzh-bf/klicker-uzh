...
- **Rollup + plain TypeScript packages**: Packages that only need unbundled `dist` output should build with `tsc -p tsconfig.json` and `include: ["src/**/*"]`, not Rollup. Keep Rollup only where bundling/copy behavior is still explicitly needed. (`packages/*/package.json`, `packages/*/tsconfig.json`)

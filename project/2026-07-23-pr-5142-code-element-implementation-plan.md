...
- The deployed source accepts only `librechat_jwt` and `openid_reuse` principal sources. A Klicker-specific source therefore still needs either the preferred allow-list patch or the semantically weaker `openid_reuse` claim.
...
- Active: recover routed browser verification for Slice 2b; choose the codeapi principal-source contract and pilot course/package set before starting Slice 3.

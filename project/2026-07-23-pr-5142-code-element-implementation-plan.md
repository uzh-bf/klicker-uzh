...
- Active: seal the clean-target security receipt, push the reviewed branch, and refresh draft PR 5142 from whole-branch evidence. The final Standards/Spec and maintainability reviews found and closed six implementation defects: persistent instances could be moved across activity boundaries, rejected group-activity edits could delete clues before that boundary was validated, submission preflight exposed an instance/type/course oracle, authoring accepted test identifiers and aggregate weights that runtime rejected, provider rate limits consumed the ordinary three-attempt grading budget, and Analytics lacked a CODE correctness branch. ADRs 0001/0002 also lacked required wiki metadata and creation-log entries. The exact-commit autosave closure is complete through `9d8508515`; the exhaustive pre-fix security pass identified and closed three low-impact boundaries: lecturer-bound authoring autosaves, permission-scoped template type preflights, and pre-expansion JSON breadth limits. Keep live staging and load proof gated on the separate CodeAPI allow-list deployment and user-provided connectivity.
...
- Next: seal the clean-target security receipt, then push and update draft PR 5142 with the complete verification record. Live staging/load proof and the pilot package audit remain explicit external gates rather than local implementation work.

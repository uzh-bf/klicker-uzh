...
- Active: Slice 5 participant flow. Render participant-safe CODE questions in the PWA, persist the code and active receipt for reload recovery, subscribe with polling fallback until a terminal receipt arrives, expose only public feedback, allow retry after failure, and advance only after completion. Verify the practice-quiz journey in the routed PWA with `agent-browser` and focused Playwright coverage.
...
- Next: implement and verify the Slice 5 participant flow with routed PWA and `agent-browser` proof.

...
- CodeAPI source check: local checkout `/Volumes/HOME/Git/df/code-interpreter` is clean at `5e459dd4f2d8bea6ae7a3004f15051dff26abae0`. That checkout still hardcodes `librechat_jwt | openid_reuse`, so it is not the deployed staging verifier. The staging `codeapi-api-klicker-test` workload now exposes an environment-configurable principal-source allow-list containing `klicker_jwt`; `POST /v1/exec` remains the verified flat `ExecuteResult` contract, which the shared client treats as hostile input until parsed.
...
- Next: the repository now wires the staging CodeAPI Service and its non-secret JWT client settings through both Helm chart generations, with the internal HTTP exception guarded by `.svc.cluster.local` validation. The PR remains draft until the externally managed signing keys are provisioned to the general grading-worker Secret, the authenticated staging smoke and at-least-50 concurrent load proof pass, and the pilot course/package audit closes.
- 2026-08-31 staging read-only verification: `aks-stg-apps` exposes ready `codeapi-api-klicker-test` endpoints on port `3112`, and its `CODEAPI_JWT_ALLOWED_PRINCIPAL_SOURCES` includes `klicker_jwt`. A temporary port-forward returned `401` for unauthenticated `POST /v1/exec`, proving reachability only; the authenticated smoke could not run because the local Infisical CLI has no valid session and the deployed Klicker Secrets currently contain no `CODEAPI_*` keys. No cluster resource was modified.
- Blockers: the externally managed Klicker signing keys and Infisical access required for the authenticated smoke remain open, as does the pilot course/package audit. Context7 remains unavailable. The Auth Google Fonts fetch prevents a complete offline full-workspace build. The current scoped Manage build additionally stops on the unrelated media-library/404 prerender router error after successful TypeScript and webpack compilation; routed `/` proof passed before the current local devnet-alias loss.

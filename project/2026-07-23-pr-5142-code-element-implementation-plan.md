...
- Active: Slice 6 microlearning and instructor evaluation. Enable CODE submission for identified microlearning participants, restore the finalized database evaluation into participant-scoped completion storage, and render a simple per-test aggregate in Manage. Keep generic charts and templates unsupported. Verify a real microlearning participant journey and instructor evaluation readback with focused Playwright plus routed browser proof.
...
- Next: implement and verify Slice 6 microlearning submission, participant evaluation readback, and the Manage per-test aggregate.

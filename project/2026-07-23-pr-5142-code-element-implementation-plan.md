...
- Active: Slice 7 hardening and finish. Honor CodeAPI rate-limit delays without retry storms, prove the hostile-output caps and authorized instructor snapshot boundary, audit English/German parity, and add a bounded local concurrency tracer. Keep the live staging smoke gated on the separate CodeAPI allow-list deployment and user-provided connectivity. Then run the final security, branch, and maintainability gates and refresh the draft PR from whole-branch evidence.
...
- Next: implement and verify Slice 7 rate-limit deferral, boundary/snapshot coverage, i18n parity, and bounded local load tracing; then run the final review gates and update the draft PR.

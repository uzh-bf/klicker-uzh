...
- Next: the repository now wires the staging CodeAPI Service and its JWT settings through both Helm chart generations, with the internal HTTP exception guarded by `.svc.cluster.local` validation. The PR remains draft until the externally managed Klicker signing keys are provisioned to the backend/worker Secrets, the authenticated staging smoke and at-least-50 concurrent load proof pass, and the pilot course/package audit closes.

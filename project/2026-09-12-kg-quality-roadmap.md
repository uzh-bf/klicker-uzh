...
**W12a — Move feature admission to GrowthBook.** Owner: Klicker backend/UI maintainer; deployment owner verifies worker compatibility. Status: implemented in source and merged to `v3-ai` via [PR #6178](https://github.com/uzh-bf/klicker-uzh/pull/6178); the GrowthBook definitions exist for staging; the `v3-ai` integration reached `v3-audit` through [PR #6199](https://github.com/uzh-bf/klicker-uzh/pull/6199), so staging promotion carries the flags. Live activation still awaits the staging catalog-revision value ([PR #6192](https://github.com/uzh-bf/klicker-uzh/pull/6192)) and the acceptance matrix below. This bounded source package can precede W8/W9 quality comparisons; widening still requires their evidence. Continue from the [generation simplification plan](2026-09-20-kg-generation-simplification-plan.md).

The four admission flags described below are now defined in the self-hosted GrowthBook instance (project `KlickerUZH`). Each is a boolean with `defaultValue: false` in every environment. `dev` is disabled, `production` is disabled with no rules, and `staging` is enabled with one force-true rule conditioned on `actorType: user`, so staging lecturers are admitted while participants and anonymous callers are not. The production SDK payload does not contain any of the four keys, so production still fails closed. Per-cohort narrowing (role, Catalyst, or saved groups) is a later staging decision, not part of this definition pass.

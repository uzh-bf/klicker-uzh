...
#### Stage-1 conflict finding: preserve the v3-audit SSR logging

A second integration vehicle already exists: PR #6199 (`rs/v3-audit-sync-20260920e` → `v3-audit`, by the repository owner, not the auto-sync controller). It also merges `v3-ai` into `v3-audit`, but its resolution of the same two pages takes the `v3-ai` side and silently drops code that exists only on the `v3-audit`/staging line: the SSR request logging (`createSsrRequestLogging`, `logFailure`, `initializeApollo(undefined, ctx, requestContext)`) and the corresponding `requestContext` propagation in `apps/frontend-pwa/src/lib/apollo.ts`. That logging comes from `1b368fb255` (`enhance(logging): complete server-side app adoption`), which is an ancestor of `v3-audit` and `stg-release` but of neither `v3` nor `v3-ai`, so a naive `v3-ai`-side resolution reverts it.

Correct resolution keeps both sides: the DPO notice/choice behavior and imports from `v3-ai`, plus the `v3-audit` SSR logging and Apollo request-context propagation. A resolution was prepared locally on a task branch; it is not pushed or merged. Before landing, confirm whether #6199 or #6203 is the intended single vehicle so staging is not advanced twice, and confirm the prepared resolution compiles and typechecks in the container. Merging remains a named-authority action.

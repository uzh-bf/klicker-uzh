...
##### Current STG head: `5d63a34234`, 21 September (after #6217)

`7cc37fce83` is superseded as the STG candidate. The later `v3-ai` → `v3-audit` integration merges #6218 and #6219 landed as `5d63a34234`, and promotion run [`35583296016`](https://github.com/uzh-bf/klicker-uzh/actions/runs/35583296016) applied it on top of release SHA `fae58d7656` in one attempt with no failures and a verified read-back. Receipt artifact `10631616053` records green exact-head CI (check, gitleaks, GraphQL, eight Playwright shards, unit, OLAT API, intl production smoke, staging image builds and SonarCloud) plus image-scan receipts for the backend and migrator images. `origin/v3-audit` and `origin/stg-release` both name `5d63a34234`.

The notice source is unchanged across the move: the `7cc37fce83` → `5d63a34234` diff touches only `.agents` documentation, `.devcontainer`, `.devrouter.yml`, the devrouter installer, `AGENTS.md`, the staging values file and the `docs/` pages. No notice component, policy copy, Prisma schema or migration changed, so the notice-surface acceptance recorded for the previous head still describes the source under test. Live re-check at the new head confirms the surfaces serve `200` and render the notice content: the PWA `/de/createAccount` page returns the German research, Learning Analytics and privacy wording, the assessment `/de/account/data-use` page returns the assessment access page with the assessment disclosure strings in its payload, and `auth` `/student` returns `200`.

Two Stage-1 acceptance items remain open and are unchanged by the head move. First, the expanded assessment wording still needs an authenticated assessment session; without an authorized Edu-ID test account only the unauthenticated redirect and the shipped bundle can be inspected. Second, the public policy and student Learning Analytics pages remain unpublished for verification: `www.klicker.uzh.ch` resolves through CNAME `idwww.uzh.ch` (130.60.184.132) and TCP connections to port 443 time out from this workspace, and no docs service is exposed on the application hosts. The disclaimer links point at that host, so a PRD release would hand students a link whose updated policy text cannot yet be confirmed live.

Note that `v3` has advanced beyond the STG candidate: `origin/v3` is `542dafe88b` and additionally contains the leaderboard redesign #5792, #6207, #6224 and #6217. PRD promotion must name the exact `v3-audit` head rather than `v3`, and any further `v3-audit` advance repeats STG verification for that head.
...
#### PRD preparation state, 21 September

Preparation-only review performed after #6217; nothing here commits, tags, changes a production value or deploys.

**Migration and rollback (source review).** The notice layer ships four additive migrations. `20260912141124_participant_policy_onboarding` adds nullable or defaulted columns to `Participant` (`dataUseAcknowledgedAt`, `dataUseAcknowledgedVersion`, `dataUseRevision` default `0`, `learningAnalyticsConsent` default `false`, `learningAnalyticsDisclosureVersion`, `learningAnalyticsChoiceAt`, `researchConsent` default `false`, `researchConsentDisclosureVersion`, `researchConsentChoiceAt`), creates `ParticipantDataUseEvent` with a unique `(participantId, revision)` index and a cascading participant foreign key, and adds `protect_participant_data_use_event` triggers that raise on `UPDATE`, on `DELETE` without a surviving participant row, and on `TRUNCATE`. `20260913150006_participant_research_consent_index` and `20260913150007_participant_learning_analytics_consent_index` each add one `CREATE INDEX CONCURRENTLY` and are deliberately single-statement files so the concurrent build runs outside a transaction. `20260914090000_participant_analytics_withdrawal` creates `ParticipantAnalyticsWithdrawal` with a composite foreign key onto `ParticipantDataUseEvent`.

A rollback to the previous application tag keeps every saved choice, acknowledgement timestamp and audit row: no migration drops or rewrites existing data, the added columns have database defaults so older inserts still succeed, and the previous release never reads the new tables. Restoring the pre-migration schema is not part of the rollback path. The audit-table triggers intentionally reject row updates and deletes, so any future cleanup of that history must disable the trigger explicitly rather than "fixing" it during a rollback. STG already applied these migrations from the `cd308e7bae` migrator image during the earlier `PreSync` hook, so the ordered application path is exercised, but production still relies on the `PreSync` hook running with `migrator.enabled: true`.

**Open assessment-database check.** The chart deploys the assessment backend with a separate secret, `<release>-secret-backend-assessment`, while the migration job and the main backend read `<release>-secret-backend-graphql`. The chart itself does not pin a database, so this plan cannot yet state that the assessment backend targets the migrated database. Confirming it needs a values-free look at the provisioned secret keys (host and database name only) or an operator statement. Until then, the assessment surfaces on PRD carry an unverified data prerequisite.

**PRD values.** `deploy/env-uzh-prd/values.yaml` pins `v3.4.0-alpha.80` in 17 image tags, with `migrator.enabled: true` and the migrator tag auto-tracking `backendGraphql.image.tag`. The release change is a single image-reference roll once the release version exists. The deploy-parity gate (`.github/scripts/deploy-parity.cjs`, run from `check.yml` on `v3` pushes and `v3`-based pull requests against `v3-ai`) must stay green with the image reference as the only intended exclusion.

**Still missing before the release commit.** The release version and `CHANGELOG.md` entry are not generated yet (`pnpm run release:alpha --skip.tag` still to run under the release authority), live optional-processing containment evidence is still outstanding, and the two Stage-1 acceptance items above remain open. PRD preparation cannot substitute for the user's separate decision to execute the release.
...
| Core notices/persistence     | Merged into `v3`, `v3-ai` and `v3-audit`; promoted to STG at `5d63a34234` and live-tested                           | Close the two open Stage-1 acceptance items, then prepare the PRD release.                   |
| Optional processing          | Python/GraphQL containment merged; live state unproven                                                            | Prove live containment and any retained-LA reconciliation before PRD exposure (release gate). |
| Chat                         | Not implemented; deferred to Stage 3                                                                              | Implement after the PRD release, against current `v3-ai` contracts.                         |
| Documentation                | Bilingual student pages, policy and group-only examples merged with the layer                                     | Publish the policy and LA pages at the linked public host; hold the guest claim until Stage 3. |
| Release                      | STG serves the verified `5d63a34234` receipt; PRD still pinned to `v3.4.0-alpha.80`                                | Finish PRD preparation, then release to PRD under named authority.                           |

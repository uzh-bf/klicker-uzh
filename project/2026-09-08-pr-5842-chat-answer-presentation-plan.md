...
The executor owns template extraction, the small server renderer, affected contract tests, dependency declaration, lockfile, and Next asset inclusion. Main owns baseline equality, integrated checks, isolated standalone packaging proof, reviews, and publication to the existing PR. This changes no product primitive, evaluation contract, provider, or data flow. No ADR is needed for this reversible extraction. Update the Chat platform guide to explain authoring and packaging if the renderer creates a non-obvious maintenance obligation.

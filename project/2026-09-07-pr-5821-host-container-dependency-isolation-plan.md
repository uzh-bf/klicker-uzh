...
On 2026-09-08 the user approved a fresh isolated startup check, followed by exact stop with data retained, final review and draft delivery. The main session created `trees/rs/dependency-mount-startup-proof` on `rs/dependency-mount-startup-proof` at `b2cc69cd5b274b00d6f139d9b1da5fbfd3d3c54d`. The retained proof workspace and running validation workspace remain excluded. Installed Devrouter 0.0.59 uses Devsy on this host. The Infisical-backed `manage,chat,ai,mcp` ensure generated the previously absent overlay, created an app container with all 31 project-scoped dependency volumes and the unchanged shared store, and completed post-create.

Cold and unchanged-inventory warm ensures both returned `ready` with no drift. Auth, Chat, PWA and Manage semantic readiness passed; Linux Next module resolution passed for Auth, Manage and Chat. The warm run reused container `eed972874f2c4c2d7a4e6b50581a2ecf090c02ca145da660f75811f126f21638`, with unchanged start time and zero restarts, and reported matching dependency volumes without installation. Existing MCP bootstrap intentionally rotates its credential generation and restarts its managed processes on ensure; no unchanged-process claim is made for this profile. The warm course-route prefetch warned, while required readiness passed. Application builds emitted baseline TypeScript warnings; this is not a full typecheck claim.

Exact stop completed for source path `trees/rs/dependency-mount-startup-proof`, provider ID `rs-dependency-mount-startup-proo`, Compose project `default-rs-f2d86`. Devsy reported `Stopped`, all seven project containers were exited, and no exact workspace routes remained. Volumes were retained. A temporary invalid workspace manifest then made the installed managed hook return `Managed host preparation failed` before startup. The fixture was restored. Fresh combined Node 24.17.0 host-tooling tests passed all 30 cases. Historical authoring results below remain the product-suite evidence; no fresh browser-suite pass is claimed. New-container mount application is proven; changed retained mount inventories remain explicitly unsupported and outside this delivery.
...
Slice simplifier and seam review are complete, with accepted findings corrected; reports are under `project/_local/reviews/2026-09-08-generated-mount-*`. Installed consumer startup and exact shutdown are verified above. Integrated-final review covered `6af9532fe9b1ffd5a92e0466ada93121a90ba8ce..29d64021ff2ba9f2acb45392ed5f14d73ee99e53` and found only the README prerequisite still naming Devrouter 0.0.55. It is corrected to 0.0.59; the documentation-only correction needs no repeated review. Ordinary push and draft-description update follow. CI and merge remain separate gates. Do not recreate the retained proof workspace or mutate the running validation workspace.
...
Historical source-only checkpoint, superseded by the runtime proof above: executor `mount_generator` owned the generator and its test; main owned the wiring and effective-model coverage. Installed and configured Devrouter 0.0.59 preserves the 0.0.58 host-hook contract. Its upgrade prompt leaves changed retained mounts unsupported. Combined profile resolution, native shell syntax and focused configuration formatting passed before runtime verification.

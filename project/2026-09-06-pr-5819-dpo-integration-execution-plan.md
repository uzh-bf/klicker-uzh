...
The user authorized local implementation, synthetic checks, configured reviews, local commits, task-branch pushes, and draft PR delivery. Core delivery is [PR #5819 — retained points and leaderboard publication](https://github.com/uzh-bf/klicker-uzh/pull/5819), branch `rs/dpo-core-integration`, targeting `v3`. AI changes retain their separate `rs/dpo-ai-integration` branch targeting `v3-ai`. The user explicitly approved incorporating the canonical consent prerequisites into the existing task branches, preserving provenance and migrations; the research delivery contract remains undecided. Shared-branch integration, marking ready, merging, deployment, real-data processing, and unrelated changes remain outside authority.
...
[PR #5819 — retained points and leaderboard publication](https://github.com/uzh-bf/klicker-uzh/pull/5819) is open and draft at `602285f6b63242d84ea92009c251570c235ec38a`, matching the core task branch and remote. Before this documentation reconciliation, the branch is clean, four commits ahead and six behind `origin/v3`; no target integration occurred. The three implementation commits follow the approved plan commit. The AI task has no functional implementation or draft PR.

| Workflow | Implemented and verified | Remaining boundary |
| --- | --- | --- |
| W1 — account/profile integration | Canonical consent schema/API/settings dependencies identified; no duplicate consent model introduced | Incorporation of the open prerequisites requires explicit authorization; account usability and withdrawal enforcement remain unimplemented |
...
| W3 — authenticated course exports | Source mapping and acceptance contract recorded | Consent/audit contract and research delivery decision; no authenticated export implementation |
...
The canonical [consent schema prerequisite](https://github.com/uzh-bf/klicker-uzh/pull/5569), [consent API prerequisite](https://github.com/uzh-bf/klicker-uzh/pull/5590), and [consent settings prerequisite](https://github.com/uzh-bf/klicker-uzh/pull/5595) remain open at `93344b11c0c03aa28e26611cc84ca2f752aabb0e`, `1e0153b91aca10a8220b777e86316bf4de5445c2`, and `a830650e44af3a2e6a680ad70891eed12fb54b69`. Their cumulative change spans 55 files, including migrations. Resolve explicit permission for incorporating these prerequisites into the task branch, preserving provenance and separate core/AI targets, before dependent account implementation. Do not interpret workflow acceptance as permission to merge their branches into shared targets.

The separate research decision remains: single-course ADMIN JSON with a proposed 50,000-record/25-MiB cap, and direct attested versus separately reviewed release of free text/transcripts. A generic go-ahead does not select between these outcomes.

The configured architecture-advisor model is listed, but its startup attempts unrelated MCP OAuth setup. No consultation succeeded. Preserve the knowledge-base contract boundary without implicitly changing integrations or substituting providers.

...
**Variable state (2026-08-23, re-checked):** the workflows reference exactly four repository variables — `NEXT_PUBLIC_GROWTHBOOK_API_HOST_STG`, `NEXT_PUBLIC_GROWTHBOOK_API_HOST_PRD`, `NEXT_PUBLIC_GROWTHBOOK_CLIENT_KEY_STG`, `NEXT_PUBLIC_GROWTHBOOK_CLIENT_KEY_PRD` — used as build args in all five Next.js Dockerfiles (auth, chat, frontend-control, frontend-manage, frontend-pwa). `gh variable list` at repo level still returns none. Org-level variables could not be inspected with the current token (403, needs `admin:org`); whoever provisioned them should confirm the four names exist at repo or org level before release images build.

### Findings from the takeover session (2026-08-23, later) — verified against the files

1. **GrowthBook cannot revoke anything on a running pod, as wired here.** `NodeFeatureFlagClient.initialize()` memoizes one init promise (`packages/feature-flags/src/node.ts:60-88`); `refresh()` has no caller outside `packages/feature-flags/test/node.test.ts`; there is no polling timer; `init()` is called without streaming; and the SDK's background sync is SSE-only while `globalThis.EventSource` is `undefined` on Node 24.16.0 (verified by running it). A pod therefore freezes its flag payload for its process lifetime. This is why the billing entitlement belongs in Postgres and not in a flag — it is the argument that survives every counter-argument.

2. **Probable shipping bug in the Helm chart.** `GROWTHBOOK_API_HOST` and `GROWTHBOOK_CLIENT_KEY` appear nowhere in `deploy/charts/klicker-uzh-v3/`, and `templates/deployment-chat.yaml` mounts no `secret-growthbook` — four other deployments do (`deployment-app.yaml:379`, `-assessment`, `-hatchet-workers`, `-response-api`). Chat receives only `GROWTHBOOK_ENV` via `cm-global.yaml`. Unless the externally-provisioned `secret-chat` carries those two keys, `configured` is false in stg/prd, every flag evaluates false, and the lecturer assistant is dark *and un-openable* without a chart change. It fails closed, so nothing is unsafe — but A6 would look like a successful dark deploy and A7 would then fail with no obvious cause. **Verify `secret-chat`'s keys on the cluster before A6.**

3. **A7's exit criterion is currently unsatisfiable.** The beta opt-in is self-service, not administrative: `setBetaFeatures` writes `ctx.user.sub` into the saved group for any Catalyst account (`packages/graphql/src/services/betaFeatures.ts:104-128`). Since the A5 amendment, opting in grants nothing until an administrator also sets `aiFeaturesEnabled`, and neither UI signals the missing half. A7 still reads "tell VK2 participants how to opt in" with exit criterion "opting in works".

   **Ruling (2026-08-23):** this is by design, not a bug. The beta flag is a general gate holding for many features, not just AI; only the AI features additionally require the `aiFeaturesEnabled` approval because they carry billing. The gap to close is therefore the UI not signaling that an opt-in grants the general beta but not AI access until an administrator approves it — not a change to the opt-in's semantics. A7's enablement story should reflect the two halves.

4. **ADR debt.** ADR 0007 needs an amending record for the reversed timing (already known). ADR 0008 argues the opposite direction of travel — Postgres booleans out, GrowthBook in — and specifically criticizes the profile-query coupling that `aiFeaturesEnabled` reintroduced in `QUserProfile`. The reversal currently lives only in a plan file. It is defensible (ADR 0008's own Decision section prohibits authorization decisions in flags, and billing consent is one), but the record needs a superseding or amending note.

### Base divergence (2026-08-23, later)

`origin/v3` advanced by 7 commits since the last fetch (07:50): three deploy promotions, `chore(load-test): add safe chatbot k6 suite (#5478)`, `feat(manage): add assessment participant invitations (#5394)`, `enhance(assessment): store edu-ID participant identity (#5452)`, and the promoted `8503c14248`. 91 files changed; 26 of them overlap files this branch also touches — most notably `packages/graphql/src/schema/mutation.ts`, `packages/graphql/src/schema/query.ts`, `packages/graphql/src/services/assessmentReports.ts`, `packages/i18n/messages/{en,de}.ts`, `deploy/env-uzh-stg/values.yaml`, `turbo.json`, and several assessment-PWA files. The merge base needs refreshing before A6; nothing has been merged or rebased yet.

**Resolved (2026-08-23):** `origin/v3` merged into `feat/v3-ai-reintegration` as `16f14e9254`. Conflicts were confined to the three generated GraphQL artifacts (`ops.ts`, `client.json`, `server.json`); resolved by taking our side and regenerating via `graphql-codegen`, which re-added the merged assessment-invitation ops. The Prisma client was regenerated and the `@klicker-uzh/prisma`, `@klicker-uzh/types`, `@klicker-uzh/util`, and `@klicker-uzh/graphql` packages rebuilt (the merge's new schema fields and exports required it — the stale `dist` builds were the only failure source). Branch is 0 behind / 30 ahead of `origin/v3`.

### Re-baseline #2 and A0 resolution (2026-08-24)

`origin/v3` advanced again (20 new commits past `16f14e9254`, through `d8e964c284`), so the branch was re-baselined a second time before final review. The merge `6cf80bfda5` hand-merged two real conflicts: `apps/chat/src/proxy.ts` (v3 renamed `middleware.ts` → `proxy.ts` per the Next 16 convention; resolution keeps v3's filename with our hardened logic — guest/PWA-embed token verification, manage-chat matcher exclusion, fail-closed `APP_SECRET`) and `apps/chat/src/services/mcpClients.ts` (unified third parameter `string | MCPRequestOptions` so our per-participant JWT minting and v3's request-timeout/`redirect: 'error'` additions both survive). Two semantic conflicts surfaced by the gate run were fixed on top:

- `6eb459b3c6` — v3's ADR 0037 lifecycle-standardization commit removed the `asUserWithCatalyst` definition from `mutation.ts` while our `setBetaFeatures` still referenced it. The definition was restored locally (one line); the gate expression stays `{ ...asUserWithCatalyst, ...asUserFullAccess }`. Renaming the gate to `asUserFullAccessForStandardActivities` was considered and rejected: it would silently drop the Catalyst requirement, which is a settled contract of this plan (a non-Catalyst opt-in must grant nothing), and ADR 0037 itself retains the `catalyst` scope for Catalyst-gated surfaces like this one.
- `81348c10a8` — v3's new `mcp-compat-canary` test pinned a 1.17.5-era handshake; under our deliberate SDK 1.30.0 pin the stateless server transport cannot be reused across requests. The test's server now uses a stateful `sessionIdGenerator: () => randomUUID()`, which is the transport shape 1.30.0 requires; the production client path was already 1.30-compatible.

Gates at HEAD `92ed99beca`: typecheck 27/27, tracked formatting clean, lint 6/7 (sole failure the known pre-existing analytics pandas/meson cold-container issue), syncpack valid, chat vitest **553/553** (the v3 merge grew the suite from 534). The devcontainer workspace had to be recreated from scratch (`devrouter ensure .`); its fresh volumes exposed a cold-start ordering failure in post-create that the old warm dists had masked, resolved by rebuilding `prisma` → `types` → `util` → `graphql` in dependency order before regenerating. The dev database was reset and reseeded.

**A0 is resolved.** The four `NEXT_PUBLIC_GROWTHBOOK_*` GitHub repository variables exist (created 2026-08-24 ~10:04 UTC — the previous session's negative probes simply predated them), and the four identically-named dead secrets are gone. Cluster verification (read-only, stg and prd): `secret-chat` carries only `APP_SECRET`, `AZURE_API_KEY`, `DATABASE_URL`, `MCP_KEY`, `OPENAI_API_KEY` — no GrowthBook keys — confirming finding 2 above as a real shipping bug. Fix `92ed99beca` mounts the optional `secret-growthbook` on the chat deployment (mirroring the four deployments that already had it) and sets `backendGraphql.betaSavedGroupId: grp_2CeKcuxYw3c567fSV7CPPQ` in both env values files. Helm renders confirm the chat Deployment gains the envFrom source and the backend ConfigMap carries `GROWTHBOOK_BETA_SAVED_GROUP_ID` under env values while omitting it under chart defaults. Both environments point at the single production GrowthBook instance (`https://growthbook-features.prd.df-app.ch`), where the `lecturers-beta` saved group (type `list`, attribute `id`) was verified live.

**Final review:** dispatched natively (GPT-5.6 Sol, read-only) over `35142c81ac..92ed99beca` — 57 commits, 513 changed paths — after the earlier Claude-routed attempts failed (background child never returned; CLI OAuth expired). Manifest at `_local/reviews/2026-08-24-v3-ai-reintegration-final.md`. Verdict pending at this writing; nothing pushed, no PR, per standing user authority.

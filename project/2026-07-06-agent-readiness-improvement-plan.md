...
| 0.6 | Publish GraphQL schema in docs | high | Add a generated page in `apps/docs` that includes `packages/graphql/src/public/schema.graphql` (Docusaurus can import raw text; simplest: a docs page linking the schema plus a build step copying the file into `apps/docs/static/`). Add one line to AGENTS.md "GraphQL Workflow": schema file path + regen command. | Schema browsable on the docs site; AGENTS.md names the path. |

...
| Final `ContentInput.tsx` compatibility hunks: legacy `'<br>'` handling, dynamic placeholder, selection state, and contenteditable-clearing support | Compatibility | Selectively apply the source fixes in their final form. This layer may not invent a new editor architecture. Core already owns safe external synchronization, Formik-safe updates, and disabled-state no-op behavior. |

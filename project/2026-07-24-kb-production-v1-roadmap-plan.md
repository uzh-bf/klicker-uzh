...
- Objective: execute W4 chatbot binding and the doc-query retrieval seam on `kb-poc` after the verified W2/W3 foundations.
- Terminal condition: lecturers can attach, replace, and detach exactly one enabled KB per owned chatbot; the invariant is database-safe; authenticated chat requests derive scope from the enabled binding and mint a dedicated short-lived ES256 token for the `KB_doc_query` tool; no binding means no retrieval tool and a clear lecturer warning; EN/DE UI, schema/API/runtime behavior, and citation rendering are verified locally and in fresh PR CI.
- Boundaries: preserve W2/W3 ingestion behavior and existing MCP authentication. Do not reuse `APP_SECRET`, place participant identifiers in the retrieval token, trust client-supplied scope, add `KBCourse` or multi-KB behavior, alter legacy static course chatbots, port #5078 wholesale, create deployment keys, mutate external infrastructure, expose staging, or merge PR #5174.
- Branch: implement and commit W4 directly on `kb-poc`; PR #5174 remains the draft integration PR into `v3-ai`.
- Verification: migration and partial-index inspection; focused real-PostgreSQL GraphQL and chat/MCP tests; generated artifacts; per-slice review and simplification; root `check:all`; production build; delegated-login browser verification with screenshots for EN/DE desktop/mobile and attach/replace/detach/warning states; final maintainability/security/branch crosscheck; fresh PR CI. The live external retrieval canary remains gated on platform R4.3.
- Pause conditions: stop only for a material data-model/contract/security decision, unavailable required contract detail, blocked migration proof, or credentials/external infrastructure that prevent a required non-deferred verification gate. GitGuardian incident `1509424` remains visible but does not block W4 implementation.

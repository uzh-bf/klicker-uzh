...
- [ ] 2026-07-27: W3 active. Read-only comparison with [PR #5078](https://github.com/uzh-bf/klicker-uzh/pull/5078) retains only its attempt-history concept: current W2 correlation remains authoritative, each `ingestionAttemptId` becomes the durable run id, history is strictly resource-scoped, and no old UI, webhook parser, generated artifact, or standalone app is copied. Slice one adds the append-only attempt ledger plus `activeResourceVersion`, `activeContentSha256`, and `errorCode`; creates the run atomically with the resource claim; updates run, operation, and serving state atomically across dispatch, polling, and webhooks; backfills current attempts; exposes bounded owner-only recent history; and extends GraphQL/Hatchet/webhook transition tests. Slice two adds contextual Ingest/Retry/Re-ingest controls, separate operation and serving status, recent attempts, EN/DE copy, stable selectors, and browser evidence across desktop/mobile and replacement success/failure states. Browser target: `https://manage.klicker.kb-poc.localhost`.
...
- Objective: execute W3 two-axis status and ingestion history on `kb-poc` after the verified W2 contract alignment.
- Terminal condition: every ingestion attempt has durable resource-scoped history; current operation state and active serving version/digest are distinct and transition atomically; failed replacements retain the last serving version; lecturers can Ingest, Retry, or Re-ingest and inspect recent attempts in EN/DE; and schema/API/UI behavior is verified locally and in fresh PR CI.
- Boundaries: preserve W2's canonical transport, correlation, replay protection, URL safety, and fail-closed digest gates. Do not expose staging, change W4-W9 scope, add fake progress, copy PR #5078 wholesale, or merge either PR.
- Branch: implement and commit W3 directly on `kb-poc`; PR #5174 remains the draft integration PR into `v3-ai`.
- Verification: migration/backfill inspection; focused Prisma, GraphQL, webhook, and Hatchet tests; generated artifacts; per-slice review and simplification; root `check:all`; production build; delegated-login browser verification with screenshots for EN/DE desktop/mobile and replacement success/failure; final maintainability/security/branch crosscheck; fresh PR CI.
- Pause conditions: stop only for a material data-model/contract/security decision, unavailable required external contract, blocked migration proof, or credentials/external infrastructure that prevent required live verification. GitGuardian incident `1509424` remains visible but does not block W3 implementation.

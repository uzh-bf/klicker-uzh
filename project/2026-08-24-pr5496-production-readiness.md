...
| --- | --- | --- | --- | --- | --- |
| blocker | Data safety | The original batch path could hard-delete a scheduled activity after it became published during confirmation or execution. The publication-status race is remediated in `6ad0c3bcc`; shared guard plumbing is in `b1979fd466`, and the group hard-delete path's concurrent-instance predicate is hardened in `57a342d794`. | Current guarded mutations pass `onlyIfUnpublished: true`; practice-quiz, microlearning, and group-activity guarded paths use status-predicated deletes, and ordinary group hard delete requires `activityInstances: { none: {} }`. Focused regression coverage includes the guarded batch services and the group concurrent-instance predicate. | No further code action for this finding; retain the focused regression and recheck current-head CI. | **remediated** in `57a342d794`; the remaining readiness gaps below still block release. |

...
| In progress | S2 finish gate | Complete the container-dependent migration/client/codegen/test/build portfolio if the C1 runtime becomes available. Until then, keep `delivery_pending`, do not dispatch the integrated final reviewer, and report a local boundary candidate only with the runtime gap and all external delivery withheld. |

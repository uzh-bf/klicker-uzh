...
| In progress | S2 and finish gate | Wire effective mode resolution and layered compilation, reconcile ADR/wiki docs, run the named checks and review passes, then report a local `BOUNDARY_CANDIDATE` to `rs-roadmap-orchestrator`; keep external delivery withheld. |

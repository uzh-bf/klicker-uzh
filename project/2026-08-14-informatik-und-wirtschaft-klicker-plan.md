...
- PRD source-pair readback: deployment `origin/main@32d51401` contains the exact I&W and RadioSurfVet pairs in the 17-pair Klicker inventory. Draft deployment MR `!609` pins the STG-proven v0.8.1 image and passed pipeline `645943`, but remains blocked before publication by the separate Aibuddy `doc-query-eduai` Secret apply failure on pre-existing Azure BlobServiceProperties drift. This does not prove PRD collection readiness or runtime retrieval.
- Upstream ingestion readback: the Video-AI A8 package records 82 complete videos and 872 publishable units, but its A9 EXPERT target/read-write acceptance remains open. Treat the local/STG corpus as preparation evidence only; do not call it PRD collection readiness.

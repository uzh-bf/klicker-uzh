...
- No deployment-route, LiteLLM, pipeline, corpus, secret, merge, or cleanup work in this package. Production database preparation was authorized separately for the follow-up recorded below; that authorization ended after readback and did not include cluster deployment, route changes, MCP activation, release promotion, or merge.
...
- These stack branches may edit and commit local repository code and project artifacts. External Git publication was separately authorized. The production database transaction recorded below was a separate, completed authorization and is not a standing permission for any deployment or activation.
...
- Deliver this as a two-layer GitHub stack: generic runtime first, optional course provisioner second. The generic runtime is merged; the optional course-specific provisioner is kept as a closed review artifact and is not merged. Do not merge or close the source PR during conversion.
...
- Generic-script disposition: do not merge #5406 or extract its course-specific script into a generic seed. Its only plausible reusable core is a small schema-driven bundle primitive with strict input validation, owner/course checks, dry-run and values-free receipts, serializable create-or-verify, drift/partial-state refusal, and activation kept separate. Do not implement that primitive yet: there is no second concrete consumer or agreed generic API. Keep the course-specific prompts, disclaimer, model defaults, route/tool names, and target discovery out of any future generic layer; the one-off production transaction is sufficient for this chatbot.
...
- Sol hardening delivery: the credit-cap rejection, the required explicit template marker, and the target-bound runbook prerequisites are committed at `5aecbcc66`. The former #5406 PR is closed by decision; the branch and code remain intact for reference and are not a merge candidate.
...
- Baseline E2E repair acceptance: the delegated branch contains three clean local commits (`ce560a2fc`, `25fc24f88`, `ff1b0fbf8`) changing the five recovery assertions to explicit `data-cy` locators. Prettier, the Playwright TypeScript check, and Chromium discovery passed (867 tests listed). The two focused Chromium tests reached the real child app but both failed after the expected HTTP 404 because Next development hydration left the browser DOM empty; the same response's server-rendered HTML contains `chat-not-found` markers, so this remains a pre-existing dev-runtime rendering failure rather than a selector-fix failure. No full-suite browser pass is claimed.
- Baseline E2E repair publication: the isolated selector fix was published as PR #5416 and merged as `34f301bb9e1f35813d53fec88dc538f71742a494` after all required CI completed green, including all eight Playwright shards. It remains separate from the chatbot work.
...
- PRD source-pair readback: deployment `origin/main@32d51401` contains the exact I&W and RadioSurfVet pairs in the 17-pair Klicker inventory. The bounded PRD preflight also confirmed the mounted I&W pair, the `klicker_ai_informatik_und_wirtschaft` collection, `872/872` active rows, the expected 1536-dimensional schema, and the exact course/source metadata. The live PRD doc-query pod is still on the older v0.7.2 image (`sha256:8687d829…`), not the v0.8.1 image pinned by draft MR `!609`.
- PRD retrieval/read-write status: an authenticated I&W retrieval reaches the PRD tenant tool but fails in the reranker with `403 key_model_access_denied` because the live key is allowed only for `aibuddy-fleet` while the requested model is `klickeruzh/cohere/rerank-v4.0-fast`. The collection's active v1 corpus is therefore read-ready but end-to-end PRD retrieval is not proven. No new write was attempted: the existing active corpus has no current A9 activation receipt, and re-writing or introducing a new resource version would exceed the bounded proof.
- Upstream ingestion readback: the Video-AI A8 package records 82 complete videos and 872 publishable units. The values-free PRD collection read closes the collection-existence/scope portion of P0, but the A9 write-acceptance receipt and a successful PRD retrieval remain open. Treat the local/STG corpus and the existing PRD rows separately; do not claim a fresh PRD ingestion write.
- Remaining gate: production activation still requires the multi-tenant PRD service gates (the PRD credential/model-access repair, v0.8.1 publication, W5a evidence through W5e direct-Chat proof), a production chat runtime containing #5405/#5411/#5414, a read-only preflight, and explicit MCP activation, each separately authorized. STG provisioning, the v0.8.1 route cutover, the #5411 runtime deployment, and the #5414 structured-content fix are complete. Bounded quality testing still needs an authorized non-participant identity for its distinct denial check.
- Active children: none.
- Delivery layer: generic runtime #5405, structured citation runtime #5411, and parser follow-up #5414 are merged and deployed in STG; optional provisioner #5406 is closed and intentionally unmerged, and the original source PR #5404 remains untouched. Achieved layer: local code, documentation, disposable-database proof, bounded browser execution, closed review-PR publication, staging provisioning/activation, unauthenticated link verification, production database preparation, and native STG runtime reconciliation with source-card/citation proof. Production deployment, runtime release, and MCP activation remain explicitly withheld.
- Next action: hand the values-free P0 evidence and the concrete PRD reranker/publication blocker to thread `019febd4-cc79-7ce2-915a-6511aa7bd1a5`, then rerun the same single positive and single negative tenant probes only after its service fix lands. Keep production writes and activation withheld until the resulting W5e direct-Chat proof is accepted. Do not merge #5406, run a broad video sweep, or touch STG reingestion or legacy consumers. No operational credential is retained locally.
...
- Treat #5405/#5411/#5414 as the merged runtime reference and leave closed #5406 unmerged; the baseline chat recovery E2E repair is the separately merged PR #5416 on `rs/fix-chat-recovery-e2e-selector`.
...
- If more chatbots need this workflow, first confirm the second use case and then extract only the generic bundle primitive described above; do not promote the I&W payload into a general seed script.

...
- **Active slice:** R2 (route one named model through gateway) is next per plan order.
...
- **D1 completed:** 93a09254 on rs/byok-isolated-litellm in ai/deployment worktree trees/rs/byok-isolated-litellm. 124 litellm tests pass; both validators green; negative tests prove database_url/success_callback violations rejected; callback check tightened to forbid all callback vectors. Review: PASS-WITH-NOTES at project/_local/ within the deployment worktree.
- **I1 completed:** dea37145 on rs/byok-gateway-vault in df-cloud worktree trees/rs/byok-gateway-vault (base 17141b28 = origin/stg head). Infra script 72 pass / 0 fail; klicker suite 27 pass / 0 fail; tsc clean both packages; prettier clean; gitleaks staged scan clean. Known unrelated pre-existing failure on pristine origin/stg: db-credentials-v2 single-writer guard. Authoritative preview deferred pending push authorization.
- **R1 completed:** fc93d6c08 on rs/ai-provider-credentials-design. ByokCapability schema + migration; reservation/consume/settle/cancel/expire service with Serializable transactions; bearer-hash-only storage proven by test; 11 concurrency/property tests pass; tsc clean across all 26 packages; gitleaks clean.
- **C1 completed: 8e70a3f2f on rs/byok-gateway-caller in klicker-uzh-catalyst worktree trees/rs/byok-gateway-caller (base origin/main 18380d188). Tutoring-engine suite 33 pass / 0 fail; tsc clean; prettier clean; gitleaks staged scan clean. Request credential mode renamed to gateway mode targeting the fixed AI Credential Gateway origin; deployment env vars renamed CATALYST_ENGINE_GATEWAY_*; startup allowlist must include the gateway origin or fails closed.

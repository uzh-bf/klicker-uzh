...
Planner construction and challenge: **approved** in the second round. The user approved execution with a goal on 2026-09-06. The active goal covers this draft-only scope through verification, reviews, and local commits. Implementation uses the preserved baseline; the initial refreshed refs showed 0 ahead and 46 behind origin/v3. No upstream integration is authorized.
...
Implementation record: the plan is committed at `745fb939fb`, signup isolation at `1ced720f2`, lecturer dialogs at `1a630a1e5`, and participant journeys at `7d65348a9`. All nine development-only surfaces are implemented in German and English with the existing app presentation components. Retained personal points count immediately on first join and rejoin; leaving preserves them and historical rank awards are never added.

Verification covers 17 browser cases, nine server isolation checks, and 54 screenshots across both languages and desktop, mobile, and LMS iframe sizes. Full repository commit checks pass: 25 package checks, seven lint tasks, formatting, schema/policy checks, and staged secret scans. Container checks use `NODE_OPTIONS=--max-old-space-size=3072`; the synthetic analytics environment uses Python 3.12 and its existing frozen lockfile. A development file-watcher route registration issue was resolved without source changes, and all affected browser checks passed on rerun.

The signup and lecturer slice reviews are dispositioned in `project/_local/reviews/`. The lecturer's redundant callbacks were removed and consequential reopening/scenario checks were expanded. The remaining slice and integrated review reports use the same directory. Current verification and final runtime/review receipts are maintained in `project/_local/dpo-draft/verification.md`; the offline bilingual screenshot gallery is `project/_local/dpo-draft/review.html`. These local artifacts distinguish completed evidence from any outstanding gate.

The container mounts only the task worktree, so the guide and three spreadsheets are copied unchanged into ignored `project/_local/dpo-draft-assets/` for the fixed development-only asset handler. The preserved shared originals remain unchanged. Main owns integration and local commits. The exact task runtime is stopped and its provider/routes verified after the last runtime-dependent check; no runtime or worktree deletion is authorized.

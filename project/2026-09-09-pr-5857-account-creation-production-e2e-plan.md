...
### Hook-environment fixture isolation and portable test Node

Resuming delivery through the repository pre-commit hook surfaced two consolidation defects outside the completed review range conditions. First, the hook environment exports `GIT_DIR`, and `playwright-cache.test.ts` spawned its fixture `git init` with inherited environment. Git reinitialized the caller's worktree administration directory as a bare repository, wrote `bare = true` into the shared repository config, and the suite then failed with exit 128 while every checkout and linked worktree lost work-tree discovery. The failure reproduced on demand by setting `GIT_DIR` and disappeared in standalone runs, which is why prior direct `check:playwright-ci` evidence stayed green. Second, the plan test hardcoded a machine-specific absolute Node 24.16.0 path that cannot exist on CI runners, where `check.yml` runs the same contracts.

Both are fixed in the consolidation's own style: the cache test now spawns fixture git and the CLI under test with a scrubbed `GIT_*` environment, matching the documented pattern in the plan and workflow tests, and the plan test uses `process.execPath`. With the hook environment reproduced, the complete CI (69) and host (49) suites pass and the shared config stays `bare = false`; the previously failing cache test under `GIT_DIR` is the regression proof. The finding postdates the used correction-review budget, so it is recorded here for fresh scope and risk reassessment rather than an additional review pass. The consolidated branch had not been pushed when this was found, so no CI run ever executed the affected tests.

### Fresh browser proof at the pushed fix head

The previously blocked fresh browser rerun completed at the pushed head. The blocked runtime start was resolved in two steps. First, the user-approved temporary `172.30.240.0/24` Compose override let the retained workspace's private network be recreated; an interrupted launcher start then began a default-profile startup, and the subsequent managed stop captured a degraded baseline whose generated managed Dev Container configuration no longer matched any fresh generation, so stopped-workspace repair refused with "Repair requires the recorded profile, resource sets, and unchanged managed configuration." Second, diagnosis showed the recorded source configuration hash still matched while only the generated `.devcontainer/devrouter` configuration file on disk — a regenerable, ignored artifact — had been replaced by the interrupted run's full-profile content. Restoring that file's recorded bytes (the production profile's seven run services, without LiteLLM) made `devrouter ensure --repair` succeed with zero drift; no state file, container, volume, or committed configuration was modified.

At the fix head the canonical production launcher then passed all nine Chromium tests in 15.1 seconds with zero skipped, unexpected, or flaky results, verified the retained production artifacts, and produced independent `--list` discovery of the same nine tests across the three required specs. The production selection marker is absent after the successful run, confirming the reviewed cleanup correction on the success path. The managed stop released all four routes, the provider reports `stopped` with zero exact-source routes and no drift, and the temporary subnet override is removed with runtime data retained. Browser evidence now stands at the pushed implementation head rather than the pre-correction head.

Status: fresh_proof_delivered. The approved consolidation scope is implemented, reviewed, corrected, hook-proven, and browser-proven at the pushed head. Outstanding: fresh scope and risk reassessment for the two post-review delivery fixes, the trusted-selector bootstrap limitation that still blocks merge readiness, and no successful dedicated GitHub workflow run is claimed.

### Target integration and fresh proof at the merged head

The branch integrated the latest `origin/v3` (28 commits: the Playwright local-fast and shard-rebalancing rework, required-CI qualification, promoter v2, the devrouter 0.0.72 pin, and Next.js upgrades) in merge commit `e8702b4917`, committed green through the full pre-commit hook. Conflict resolutions kept the consolidation intact while porting every upstream behavioral change into the consolidated TypeScript modules: drafts now always take the ready-state hosted full plan in `playwright-plan.ts` with its tests, the workflow validator gained v3's permission/telemetry/artifact-wildcard checks with widened types, the launcher combines v3's phase-timed reconciliation, profile inference, and Postgres container resolution with this package's production selection lifecycle, and the staging promoter composes v3's v2 CI-evidence receipt with this package's account-production evidence. The three upstream-modified CJS modules this consolidation had replaced stay deleted with their upstream changes ported.

Fresh verification on the merged head required three environment repairs, all recorded for reuse: an in-container `pnpm install` to satisfy the verify-deps guard after the workspace settings changed, clearing stale mixed `.next/dev` and production type validators that the Next.js upgrade tripped over, and restoring the retained runtime's service aliases onto its project network — the earlier manual attachment repair had left them on the shared devnet, so `postgres` resolved to seven foreign devnet endpoints and the hatchet migration could not connect. After those repairs the canonical launcher rebuilt all four production Webpack artifacts under the upgraded Next.js and passed all nine Chromium tests in 13.1 seconds with zero skipped, unexpected, or flaky results, verified the rebuilt artifacts, and finished with a managed stop, zero exact-source routes, and no drift.

### Bootstrap diagnosis and trusted-selector remedy

The first full CI run on the integrated head reproduced the documented bootstrap limitation precisely: the ordinary hosted lane built its plan from the trusted control's inventory, so candidate-only production specs kept the conservative maximal-profile fallback and leaked into ordinary shards, where `A-account-registration.spec.ts` failed in shard two of eight while the five shards without production specs passed. The dedicated `account-production` lane passed on the same head.

The remedy landed as PR #5953 against `v3`: the trusted manifest designates the three account specs with `runtime: "production-webpack"` before their files exist there, the trusted selector validates the runtime value, tolerates production-designated specs absent from the control inventory, excludes them from every ordinary plan, and ignores their file changes because the dedicated production workflow owns them; ordinary specs keep failing closed unchanged. The contract suites on that PR pass 45 tests including the new production-lane cases. This branch mirrors the same semantics in its consolidated TypeScript selector with equivalent tests (74 CI and 66 host tests green), so the designate-first flow keeps working after the TypeScript modules replace the upstream scripts. Ordinary CI on this PR stays red by design until #5953 merges; the selector and manifest land together as the plan requires, and the dedicated production lane is already established on the integrated head.

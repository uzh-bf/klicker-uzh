...
Implementation active under the user-requested goal. Generator draft [six versioned generation domains](https://gitlab.uzh.ch/uzh-bf/tc/kg-content-generation/-/merge_requests/15) is at `d9934498e671de986176f5215a82ea56840a2cf3`; [exact-head CI](https://gitlab.uzh.ch/uzh-bf/tc/kg-content-generation/-/pipelines/663029) passed, including 120 graph/policy tests and a no-push ARM image build. The pre-existing legacy quality audit remains allowed to fail. Generator slice review corrections passed; integrated final review remains pending.

Foundation is committed and under independent slice review. Catalog is bundled directly, with policy digests required. Generated migration `20260911204500_kb_graph_domain_policy` contains only three nullable columns; Prisma schema-to-schema diff generated it after migrate-dev detected unrelated existing default/index drift. Guarded additive schema push and client regeneration passed; analytics mirror is synchronized. Knowledge-graph build/check and 80 tests, Hatchet check and 122 tests, GraphQL domain reservation and four configuration regressions passed. All 39 container typecheck tasks and 101 host contract checks passed. Full build compiled Manage, then failed on duplicate generated development/production route types; final build remains pending. The accepted simplifier removed redundant persisted-selection presence branching and its four config tests passed.

UI and browser tests are in progress. Synthetic lecturer fixture was restored after database tests removed seeded rows; DB suites no longer overlap browser verification. Main owns exact runtime shutdown, remaining source reviews and draft stack delivery. No paid generation, deployment, worker image publication or real-data access occurred.

...
Agreed scope: Finance, Economics, Business Administration, Mathematics, Informatics, and General / Mixed. The user confirmed support for all six domains on 2026-09-11. Complete each domain's English and German policy before making it selectable. Preserve Finance for legacy builds and omitted arguments. Keep the generation language explicit and independent of the UI locale. This package originally left the language selector outside its scope; it was approved and delivered separately on 2026-09-14 (see the execution status below). Editable prompts, custom node types and wider question-generation settings remain outside this package.
...
The generator owns the catalog. Generate a small versioned JSON manifest from its policy definitions for Klicker's server to consume, and return supported options through the existing authorized graph-config query. The browser must not own a second category map or receive prompt bodies. The catalog includes stable ID, policy version, localized label keys, supported languages and category names/definitions. Category chips show the selected version's category names in the selected generation language; translate surrounding labels and explanations, not the values that generation uses. Unsupported or retired selections remain visible as historical values, disable submission, and require an explicit replacement selection. Reload and catalog refresh must not silently substitute versions.
...
Source implementation is delivered under the user-requested goal. Generator draft [six versioned generation domains](https://gitlab.uzh.ch/uzh-bf/tc/kg-content-generation/-/merge_requests/15) is at `d9934498e671de986176f5215a82ea56840a2cf3`; [exact-head CI](https://gitlab.uzh.ch/uzh-bf/tc/kg-content-generation/-/pipelines/663029) passed, including 120 graph/policy tests and a no-push ARM image build. The pre-existing legacy quality audit remains allowed to fail. Generator slice review corrections passed.

Klicker foundation [PR #5904](https://github.com/uzh-bf/klicker-uzh/pull/5904) and selector [PR #5906](https://github.com/uzh-bf/klicker-uzh/pull/5906) form the approved two-layer native stack targeting `v3-ai`. Both remain draft. Foundation and UI slice reviews, including accepted corrections, passed. Generated migration `20260911204500_kb_graph_domain_policy` contains only three nullable columns; Prisma schema-to-schema diff generated it after migrate-dev detected unrelated existing default/index drift. Guarded additive schema push, client regeneration and analytics mirroring passed.

Foundation verification passed: 80 knowledge-graph tests, 122 Hatchet tests, focused GraphQL reservation/configuration tests, 39 container typecheck tasks and 101 host contract checks. Source CI and clean builds passed at foundation `4626a045` and UI `71d31c1`, superseding the earlier local generated-route collision. All three domain browser scenarios passed through the required host runner with synthetic provider/catalog GraphQL mocks. English/German desktop/mobile screenshots are published and decoded in PR #5906. They show selected categories; published-domain and coexisting-version behavior have interaction evidence rather than separate screenshots. The CI-only correction `736ba99` adds the spec to targeted Manage and shared UI selections; 30 selector/runtime contracts, direct selection checks and formatting passed. Current-head codebase, GraphQL, MCP and locale checks passed at `b294ea4`; Playwright execution skipped on that head, with the prior full CI and host browser evidence reused for unchanged application source.

Integrated final review passed with no findings across all 47 changed paths at Klicker `b294ea4` and generator `d9934498`. The user approved a trusted native final reviewer after Claude failed authentication and AGY denied source reads. The canonical review response validated successfully; it confirmed policy consistency, migration equivalence, legacy behavior, frozen dispatch and strict category failure semantics. Additional runtime catalog validation was rejected because the trusted committed artifact matches the provider export and no current defect was established. All slice reviews and the integrated source review are complete.

The reviewed implementation is delivered in the three drafts above. GitHub OCR still reports HTTP 402. The separate stack final-review workflow requires ready PRs, beyond this draft-delivery authorization. These forge statuses remain unresolved; do not claim all checks green or merge readiness. No source correction is required by the completed integrated review.

The exact managed runtime was stopped and its four routes removed; source and synthetic database state are retained. Separate provider-status confirmation is unavailable because the installed DevPod CLI symlink target is missing. No paid generation, deployment, worker image publication or real-data access occurred. The required source review and verification are complete; retain the remaining forge and provider-status limitations for subsequent readiness and rollout work. No capability configuration was changed to bypass these limitations.


### Approved live evaluation extension — 2026-09-12

The user approved synthetic German teaching material and a USD20 maximum for real graph generation, including embeddings. This supersedes the earlier paid-generation exclusion for this bounded comparison only. Five specialist-versus-general pairs plus one mixed/general case, each repeated three times, give 33 builds. Begin with the two-build Finance pilot; continue only after provenance, cost control and usable outputs pass. Keep graph outputs local and leave application databases, publication and deployments untouched. Methodology planner Ohm approved the corrected proposal. The concrete gateway preflight passed after including JSON-schema bytes in admission cost.

Execution artifacts and reproducible runner: [`_local/kg-domain-evaluation/README.md`](_local/kg-domain-evaluation/README.md). Main owns credentials, cost admission, integration and output assessment; one trusted worker owns synthetic source preparation. Independent source checking precedes generation. The terminal condition is a local comparative graph report or an explicit evidence-backed capability/budget blocker, followed by stopping the exact evaluation runtime. This diagnostic uses the real provider pipeline; it does not claim unmocked UI-to-worker proof.

Live evaluation completed after the user restored account credits: all 33 fixed synthetic builds ran at generator `d9934498`. Eleven produced category-valid cleaned graphs; 22 failed category validation. Specialist/general successes (out of three each) were Finance 3/1, Economics 0/2, Business 0/0, Mathematics 2/3, Informatics 0/0; mixed/general was 0/3. Provider-reported cost was USD0.65058847, plus USD0.1256024 retained conservative charges from the historic HTTP 402 attempts; no unsettled holds remain. The approved USD20 ceiling was preserved.

The [interactive comparison](_local/kg-domain-evaluation/comparison.html) and [verified findings](_local/kg-domain-evaluation/FINDINGS.md) retain raw/cleaned graphs, per-run failures, fixed sources and costs. Real runs prove domain-sensitive types, but expose an Economics whitespace-normalization mismatch, UNKNOWN relationship endpoints, invented categories, incorrect synonym merges and short-input cleaning loss. No generator changes or selective retries were made during evaluation. Lexical coverage is a proxy; qualitative review was not blinded or exhaustive. This schema-v2 direct-provider test does not replace Klicker's unmocked schema-v1 UI-to-worker proof.

The exact standalone evaluation container is stopped (`running=false`, `status=exited`), the host batch completed, the gateway exited and port 18439 has no listener. Desktop/mobile report interactions passed; the isolated browser is closed. Synthetic artifacts and caches are retained locally. No application database, deployment or remote graph was changed. The source-delivery goal's separate forge/readiness limitations are unchanged by this successful evaluation run.

### Broader quality roadmap — 2026-09-12

The [knowledge graph quality roadmap](2026-09-12-kg-quality-roadmap.md) carries the requested direction for generation, content use, chat retrieval, correction, and freshness improvements. It incorporates the completed comparison and existing consumer work. It is a planning proposal; additional implementation, provider spend, and live rollout require their named execution scope.

### Corrected evaluation and comprehensive review — 2026-09-12

The approved goal completed the corrected 33-build comparison at `13fe172` and source corrections at `67a6646`. All 33 builds succeeded; four cross-category merge records motivated the guarded correction. Whole malformed classifier batches now fail closed and category mutation counters reconcile. The combined offline suite passed 1,111 tests with one skip excluding pre-existing DOCX host failures; CI 663516 and same-reviewer correction review passed. The [roadmap status](2026-09-12-kg-quality-roadmap.md) records partial W1–W3 qualification, remaining dispatch/legacy diagnostics, and W4 as the next dependency-ordered package. The [corrected findings](_local/kg-quality-corrected-evaluation/FINDINGS.md) separate stochastic results from the later deterministic fixes and correct prior cost reporting. Evaluation runtime and browser are stopped. No merge, deployment, publication or capability activation occurred.

### Generation language selector — 2026-09-14

The user asked whether the graph's generation language can be chosen as well. The answer is yes, through seams this package already delivered: `KBGraphBuild.domainPolicyLanguage`, the `resolveKBGraphDomainSelection` validation in `packages/knowledge-graph`, the `domainPolicyLanguage` mutation variable and the per-language `categories` on the domain-config query all exist, and all six catalog policies ship German and English category sets. A separate plan was therefore unnecessary; the UI was the only missing piece.

Approved scope was a language selector beside the domain dropdown, initialized from the persisted build language with German for rows written before explicit languages, submitted atomically with the `(policyId, version, language)` triple, and with the category preview following the selected language. A catalog that no longer serves the selected language keeps the pair visible and blocks the rebuild until the lecturer replaces the choice explicitly, matching the retired-version rule. Extending the control to builds without an explicit domain was excluded.

Source is in `packages/kb-management` (panel, with the shared `DOMAIN_GENERATION_LANGUAGES` guard and no silent rewrite on domain change), `packages/i18n` (label, placeholder, served-language message and corrected note) and the domain-selection Playwright spec (a new scenario for restore, catalog refresh and unblocking, plus language assertions in the two existing scenarios). No schema, server, generator or catalog change was needed.

Verification: container typecheck and formatter checks passed; the four browser scenarios passed against the seeded synthetic stack in the `rs-kg-question-focus-ui` runtime. The whole spec cannot run in one sandboxed process on this machine, because macOS denies the Mach-port rendezvous Chromium's normal multiprocess launch needs, so the four scenarios were executed individually with the committed configuration plus an untracked local launch override; the suite's normal path remains the host launcher and CI. Escalated host-runner invocations of `playwright:host` were blocked by the automatic approval review's deadline, which is recorded in the draft PR rather than presented as a pass.

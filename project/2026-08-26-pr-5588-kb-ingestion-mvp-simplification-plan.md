...
Klicker sends data-ingestion one Resource API request whose source is the original public URL. The request also carries MIME, display, and digest integrity metadata required by the deployed contract; it does not carry the page body. Klicker validates and retains the observed byte size locally for quota accounting, but the Resource API request has no size field. Klicker has no direct web-scraping API, credential, configuration, or network dependency.
...
- 2026-08-27: Provider MR !126 merged as `af38e52c5f1e37742219309860baacaa3b6595ec`; pipeline 651971 passed and published immutable worker/API images. Deployment MR !693 merged from reviewed head `e640d801a9298ae6282c53d46d2bc66620e57bd1` as squash commit `d503344c0afda69bb11bb76051a227c7f9748c42`; pipeline 651975 passed with both images pinned to provider source `af38e52c` and Klicker's exact PDF/plain-text/HTML policy.
- 2026-08-27: Klicker review correction `f6afb4a0d583f9ab01e6f616eca3ee359ddf77c0` keeps null-start legacy operations reconcilable while preserving the five-minute callback grace for timestamped operations. The Node 24 Hatchet suite passes 112/112 with package check, build, Biome, and host pre-commit checks passing. The exact task runtime is stopped with zero routes. PR publication, feedback disposition, exact-head CI, and merge remain; deployment reconciliation, live proof, retry, cleanup, graph action, and production remain separately gated.

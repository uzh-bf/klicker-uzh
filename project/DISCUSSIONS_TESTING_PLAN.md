...
- **Browser screenshot**: exercised against the isolated real app and routing
  through `agent-browser` or the focused Playwright workflow.
- **Focused Chromium**: Playwright workflow completed against the isolated app.
...
|---|---|---|---|---|
| `QA-001` | PASS | Rollout and runtime gates hide Q&A in Manage until enabled | Rollout Playwright `7/7` plus six gate-state screenshots | None |
| `QA-002` | PASS | Course overview integrates Q&A only when both gates allow it | Course Playwright `14/14` plus final desktop and mobile captures | None |
| `QA-003` | PASS | Participant creates a course thread and reply in place | Course Playwright `14/14` plus final thread/reply capture | None |
| `QA-004` | PASS | Thread and reply upvotes toggle without counter drift | Course Playwright plus backend `39/39` and selected-vote capture | None |
| `QA-005` | PASS | Evaluated practice and microlearning stacks expose contextual Q&A, while answering does not | Practice Playwright `7/7` plus test-published microlearning browser proof | Production-like microlearning publication blocked by missing Hatchet workflow |
| `QA-006` | PASS | Lecturer overview groups and paginates course and stack threads | Course and practice Playwright plus final inline thread/reply capture | None |
| `QA-007` | PASS | Lecturer generates external-block and course-wide embed links | Embed Playwright `7/7` plus final integrated controls capture | None |
...
| `QA-009` | PASS | Tampered or stale embed scope/token fails closed without persistent side effects | Backend suite `39/39`; embed Playwright fragment/history and tampered-token proof | None |
| `QA-010` | PASS | Anonymous rate limits reject repeated posting and bound audit writes | Backend suite plus typed browser rejection screenshot | None |
...
The current Playwright baseline is course `14/14`, practice `7/7`, embed `7/7`,
...
Current result: `39/39` passing across authorization, gates, rate limits,
...
Historical screenshots `06` and `07` remain additional runtime evidence for
lecturer pagination from 20 to all 27 threads. The gallery and images are
...
1. The backend suite passes all `39/39` scenarios.
2. Course, practice, embed, and rollout Playwright workflows pass `14/14`,
   `7/7`, `7/7`, and `7/7`.
3. Fresh desktop `1280x900` and mobile `390x844` browser proof covers the
   integrated course, Manage review/embed controls, and fallback surfaces.
4. The existing gallery retains practice, microlearning, anonymous embed,
   rollout-gate, pagination, and legacy live-feedback browser evidence.

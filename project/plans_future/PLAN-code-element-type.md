...
Note 2026-07-12: the Mastra chat migration (PR #5126/#5129) does NOT affect this plan — the grading path is backend GraphQL + Hatchet, untouched by the chat-stack change. Only touchpoint: the codeapi client + JWT minter should be the shared `packages/` module from the chat-tool plan (build once, consume in the Hatchet worker here).
...
6. **Hatchet**: new `gradeCodeSubmission` durable task (new file in `apps/hatchet-worker-general` or a dedicated worker; NOT the live-quiz response-processor) + the shared `packages/` codeapi client module (JWT mint, exec, parse — same module the chat tool uses) + pubSub publish on completion.

...
- One codeapi `/v1/exec` call runs the invocations in isolated child processes and returns raw values, exceptions, stdout/stderr, and timeout markers. The Klicker worker treats that response as untrusted, validates its schema, and compares each raw outcome with the server-held expectation.
...
- One exec per submission, not one exec per test. This keeps cost and latency O(1) per submission and stays inside the 20 exec/30s per-user limit.
...
3. A Hatchet durable task (`durableTask` + `retries: 3` pattern, `apps/hatchet-worker-response-processor/src/index.ts:28-34`) claims the submission with a random claim token and expiry, executes the sandbox call with its own timeout budget (≥ JOB_TIMEOUT), and parses raw outcomes. A duplicate worker exits when it cannot claim the row; an expired claim can be recovered.
...
6. **Hatchet**: new `gradeCodeSubmission` durable task in `apps/hatchet-worker-general` or a dedicated worker, not the live-quiz response processor. It claims with token+expiry, calls the shared `packages/` codeapi client, sends invocation-only runner input, validates raw output, finalizes once, and publishes completion. Add recovery for expired claims and an on-failure transition to `FAILED`.
...
3. codeapi client + runner: JWT mint, exec call, invocation-only Python runner, strict raw-result parser, and worker-side comparator; unit fixtures + one live test vs stg codeapi. Check: outbound requests contain no expected values/assertion source; known-good/known-bad/timeout/malicious-output submissions grade correctly.
...
- Unit: worker-side comparator, invocation runner generator, strict result parser, options validation/sanitization, outbound-request snapshot proving expected values/assertions are absent, JWT claims fixture.

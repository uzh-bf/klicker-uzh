...
- 2026-08-31: S1 is committed as `6a9515eaf` (`feat(embed): add resize-aware quiz foundation`). Its dedicated slice review found no blocking issues; the simplifier's low-severity import-churn concern was restored before S2.
- 2026-08-31: S2 is implemented locally: focused presentation is negotiated through the shared protocol, the child reports whether the current action is valid, and the host advance request is accepted only for the matching origin and state. The child keeps the existing action implementation behind a hidden controller so the integration remains small and the legacy embed fallback remains usable.

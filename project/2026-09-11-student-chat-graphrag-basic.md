...
Both map and retrieval default off. Preserve the already-applied policy migration's checksum; add one generated forward migration to change the map default and turn existing live/draft map visibility off for this first opt-in rollout. No other revision settings change; advance the optimistic-lock version for changed rows to reject stale editors. This second migration is justified by the locally applied first migration, not an avoidable split of a new schema change. Mirror schema to analytics. Student map reads reject stale builds and revalidate publication before returning; client errors clear loaded graph state. Lecturer stale inspection remains unchanged, documented as a student-serving exception to ADR0009. Already-rendered content cannot be recalled; subsequent requests must not expose withdrawn content.

Continuation delegation/acceptance:
- Main: integration, GrowthBook cross-system policy (`packages/feature-flags/src/contracts.ts`, existing Chat featureFlags/scope and GraphQL chatbot service, Manage ChatbotDetails); mutation/scope/default-off tests. Commit gate/default slice after types, focused tests and browser flag-off/on proof. Retain main ownership because cohort/authorization semantics are coupled.
- Trusted continuity worker: only `apps/chat/src/services/graphAssistedDocQuery.ts` and its existing unit test; stable provider resource/chunk identity with exact content/provenance safeguards, explicit metadata variation/conflict tests. No new production file, no commit or external action.
- Main: diagnostic correction in Chat route and existing/new diagnostic test (new file allowed `apps/chat/test/tool-diagnostics.test.ts` if no existing seam), map stale read fence and existing route/state tests, Chat Helm configuration using existing backend graph values. These files are disjoint from worker.
- Main: schema generation, docs, full checks, UI gallery, reviews and ordinary branch push/draft description correction. New migration directory is generated by Prisma; private report artifacts stay under existing project/_local.

Test obligations: extend existing scope tests for flag-off/on and revocation; shared flag-client tests already protect refresh/expiry, extend with this flag if needed. Extend revision tests for direct false-to-true denial, approval under owner cohort, unchanged-enabled edits and disabling. Extend map route/state tests for stale pre/post-read rejection and clearing. Extend fusion test for metadata-only duplication versus conflicting content/resource identity. Diagnostic test distinguishes call/result/error, null versus absent versus valid empty; retain existing persistence/abort and strict malformed-evidence tests. Run relevant host browser controls/reload tests and capture synthetic screenshots. No new test pins prose or seed contents.

Terminal is a reviewed, verified dark-rollout source candidate on the existing draft PR, or concrete external blockers. Actual-service staging E2E, deployed credentials, representative capacity and real-data quality evaluation are next adoption gates and are not claimed from local tests. Existing heldout remains frozen. Hosted OCR/final review are readiness gates; repair source-owned issues, report external tool/provider blockers. Keep the exact local runtime under the user's testing lease.

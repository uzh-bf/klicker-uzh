...
Add focused backend operations instead of reusing broad frontend payloads:

- a candidate lookup query/mutation that reuses the practice-pool service and returns answer-safe candidate metadata,
- a safe stack render query for a signed question ref,
- a submission mutation that validates the signed ref and calls existing stack response logic.

If we can implement the safe stack render and submission as wrappers around existing backend functions, do that. Avoid duplicating grading or SRS logic in MCP.

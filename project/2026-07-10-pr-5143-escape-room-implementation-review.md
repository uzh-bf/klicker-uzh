...
- **Playwright e2e suite (release gate):** `playwright/tests/Z-escape-room.spec.ts` is still the broken pre-roadmap draft (references a nonexistent `insert-stack-display-name-0` selector). Rewrite against the real drag/drop stack-builder flow (mirror `Q-practice-quiz.spec.ts`), then extend: create an escape-room quiz with ≥3 stacks + intro story + per-element hints via the wizard; participant runs the full loop (start overlay with intro markdown → sequential gating → wrong-answer lockout countdown → hint reveal charges penalty → completion stats); lecturer dashboard tab shows the attempt and reset works. Follow `.agents/skills/klicker-playwright-e2e`.
- **apps/docs documentation (release gate):** no user-facing docs exist for escape rooms. Add a lecturer-facing page (what the mode does, wizard settings incl. time limit/hint penalty/intro story, hint authoring, dashboard + reset) and participant-facing coverage (one attempt, timer, hints cost time, lockout), wired into `sidebars.js`, matching existing docs style/tone.
- **Process guard:** extend `AGENTS.md`/`CLAUDE.md` so every PR adding new functionality must extend the Playwright suite and the `apps/docs` documentation (alongside the existing wiki requirement).

...
- **Current:** takeover audit completed 2026-07-13 on `codex/escape-room-production` at `trees/escape-room-production`. The branch includes `origin/v3` at `3872caee7`, the exact-workspace/shared-types/Hatchet runtime checkpoints, and the validated review-cleanup slice. The QR unique-index migration split is the final code checkpoint before Slice 10c runtime evidence and the release gate.
...
- **Next:** Split and validate the QR unique index as its own non-transactional Prisma migration, then execute Slice 10c against the branch-correct real browser stack: two concurrent GroupActivity participants plus monitoring, and complete LiveQuiz participant/cockpit/reset/reload runtime with locale/viewport screenshots and QR evidence.

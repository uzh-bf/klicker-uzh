...
Progress: implementation committed; verification and delivery in progress. Prior planner approved the content/design plan after one correction; this amendment changes delivery authority and records the user's requested separate PR.
...
The simplifier found no justified net reduction. The claim-risk reviewer accepted release, access and feedback wording. Its missing-sign-in concern is rejected: the global navigation retains the existing Manage destination, labelled Lecturer sign-in. The tutorial-first primary action is intentional. A main landmark was added during browser verification; the affected Biome check and production rebuild pass. Browser evidence must use the teaching-section image selector and explicitly scroll lazy-loaded images into view; the earlier empty image collection does not prove image loading.

Execution pause: automatic permission review repeatedly timed out for host browser commands and the native browser alternative. The main-landmark commit also hit an approval-review timeout. This is a capability failure, not a safety rejection. No new PR has been created. The corrected image audit, remaining browser interactions and integrated final review are pending; earlier empty-image results are not accepted as image proof. The final production build and the refreshed 71-page static audit pass. Task container klicker-website-v34-refresh is verified exited, with the exact worktree mounted at /workspace. The task browser could not be closed through the unavailable approval route; no authenticated data was used. Resume the same branch and worktree, complete the pending checks and review, then publish the approved new draft upper-stack PR.

User resumed on 5 September and explicitly requested stacking above the website refresh and retaining a docs-only preview for Browser annotations. Native local topology is verified: v3 -> rs/website-v3-4-refresh (PR 5791) -> rs/landing-page-v3-4, with no rebase needed. Approval service recovered for inspection and native Browser access. The same task container is running only the docs static preview at http://127.0.0.1:15500/; the visible Browser tab is retained for the user. Keep-running lease lasts through the annotation review and will be reconsidered after feedback is handled.

## Stacked draft and annotation preview

Draft PR https://github.com/uzh-bf/klicker-uzh/pull/5794 targets rs/website-v3-4-refresh (PR5791). Native stack5795 contains both PRs; lower branch remains unchanged at d7ff68275d2064d35d249ea31a86e01de3bb8ffd. Final review passes at 9f209157bc82e40a124023908122494c1fe4ad63 with no actionable findings. This rename is metadata only. Mobile and tablet image checks confirm all three story images load and remain visible; native Browser displays the current page and sign-in. Full keyboard/zoom qualification and user annotations remain acceptance inputs, and broader workspace checks retain the recorded limitations. Docs-only container klicker-website-v34-refresh remains running at http://127.0.0.1:15500/ under the user’s annotation lease; the Browser tab is retained. No merge, readiness promotion, upstream integration or deployment occurred.

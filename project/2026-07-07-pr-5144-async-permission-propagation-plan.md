...
- 2026-07-30 (Slice 4 prototype): the opt-in database-backed harness uses only synthetic identifiers and its own `permission_propagation_prototype` schema, defaults to a no-connection dry run, requires a separate `:run` command plus explicit local confirmation for writes, rejects `NODE_ENV=production`, rejects non-local database hosts, and drops the schema in `finally`. The first database run exposed Prisma's inability to deserialize the advisory-lock function's `void` result; using tagged `$executeRaw` preserved parameter safety without result decoding. The second exposed ambiguous timestamp-minus-interval inference; binding a TypeScript-computed cutoff removed it. Two complete confirmed-local runs through the final `:run` interface then passed, proving both cleanup and repeatability. The scenarios proved atomic rollback of source plus durable work, blocking between an in-flight grant worker and synchronous revoke, serialization of different roots that share one derived row, coalescing of an old task onto the latest generation, duplicate-task no-op behavior, rediscovery one cadence after commit-before-enqueue, a five-minute `recoverBy`, and a persistent sanitized failure row while failed work remains unresolved. Focused Prettier and GraphQL typecheck pass, as does the repository-wide typecheck (25/25 tasks). The prototype commit's pre-commit hook passed the complete `pnpm run check:all`, including Syncpack, AGENTS.md validation, and Prisma-schema sync. An earlier uncached DevPod attempt hit only the unrelated Analytics environment path—CPython 3.14 could not install pinned pandas 2.2.2 without a C compiler—and no Analytics file changed. ADR-0001 now records the proposed global advisory fence, canonical generation-keyed work record, at-least-once dispatch, one-minute reconciliation cadence, persistent failure sink, and five-minute maximum grant-convergence SLO. This is prototype evidence, not production approval.
...
- Active: commit the verified Slice-4 prototype and proposed ADR contract, then obtain exact-commit correctness and simplification reviews.

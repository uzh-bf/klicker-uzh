...
3. Batch ops already filter through the Prisma `permissions` relation, which is `DerivedPermission[]` on elements, courses, and all activity models (`element.prisma:53`, `course.prisma:60`, `quiz.prisma:52,126,181,287`). RESOLVED by live fact-check (2026-07-23): no query rewrite or access widening is needed. Permanent regression coverage must retain the actual propagation rules: course WRITE can reach activities when propagation is enabled; elements inherit ADMIN, not WRITE, from parent activities; READ can change element review status.
...
- Complete exact-commit review and simplification for Slice 2, then integrate any accepted findings.

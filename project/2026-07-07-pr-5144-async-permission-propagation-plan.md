...
- 2026-07-30 (Slice 4 review): exact-range correctness and simplification reviews accepted three contract corrections and two reductions. The work key is now a collision-safe deterministic encoding that the database constrains to the independently unique object-type/object-ID/mode/user tuple. Repeated unresolved mutations preserve the first `dirtyAt` and `recoverBy`, preventing continuous churn from postponing the five-minute target. A stale task's persistent failure row records the latest generation observed under the fence rather than its stale input generation. The test pause now occurs inside the actual full-rederivation source read, and unused prototype columns were removed. ADR-0001 distinguishes the deterministic database proof from production obligations: scheduler and queue instrumentation, lock-wait and throughput measurements, SLO breach alerting, and an external alert path when database failure recording is unavailable. Two consecutive confirmed-local runs of the review-adjusted harness passed all scenarios and cleaned up their isolated schema. Focused Prettier, GraphQL typecheck, and diff hygiene also pass. No production schema or call-site conversion is authorized.

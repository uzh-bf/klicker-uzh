...
- 2026-07-29 (Slice 3 practice-quiz review): exact-commit correctness and simplification reviews found no defect or material simplification. They independently verified the practice-quiz identifiers, fail-closed group guards, course mapping and provenance, deterministic direct-over-course precedence, user/object convergence, soft-deletion contract, exact conflict target, tagged SQL safety, access-request sequencing, and downstream element propagation. The SQL remains explicit because a polymorphic shared helper would obscure security-relevant identifiers. A stale JSDoc cross-reference was replaced with the actual course-to-activity mapping; residual practice-specific soft-deletion and malformed-row tests are non-blocking because the shared activity suite covers the contract and production enforces `UserOrGroupRequired`.
- 2026-07-29 (Slice 3 microlearning implementation): user-scoped and object-wide microlearning recomputation now share the reviewed tagged set-based activity pattern with explicit microlearning columns and the exact microlearning/user conflict target. It preserves the required course relation, course mapping and provenance, deterministic direct-over-course ties, fail-closed group expansion, soft deletion, access-request updates, and downstream stack-element propagation. A permanent exact-row oracle proves scoped isolation, group-owner/admin expansion, non-propagating course WRITE to EXECUTE, object-wide convergence, and scoped stale-row deletion without disturbing other users. The freshly rebuilt util bundle contains the expected SQL, and both the focused oracle (1/1) and complete activity-permission suite (51/51) pass.
- 2026-07-29 (Slice 3 microlearning review): the exact-commit correctness review found no defect and verified the required course relation, entity identifiers, fail-closed group expansion, complete course mapping and provenance, deterministic precedence, user/object deletion scope, soft-deletion behavior, exact conflict target, tagged SQL safety, access-request sequencing, and downstream stack-element propagation. The simplification pass found no safe SQL reduction; its proposed removal of expected-row sorting was rejected because generated UUIDs have no guaranteed lexical fixture order. Residual entity-specific tie, soft-deletion, and malformed-row cases are non-blocking because the shared activity tests cover those contracts and production enforces `UserOrGroupRequired`.
- 2026-07-29 (Slice 3 group-activity implementation): user-scoped and object-wide group-activity recomputation now share the reviewed tagged set-based activity pattern with explicit group-activity columns and the exact group-activity/user conflict target. It preserves the required course relation, course mapping and provenance, deterministic direct-over-course ties, fail-closed group expansion, soft deletion, access-request updates, and downstream stack-element propagation. A permanent exact-row oracle proves scoped isolation, group-owner/admin expansion, non-propagating course WRITE to EXECUTE, object-wide convergence, and scoped stale-row deletion without disturbing other users. The freshly rebuilt util bundle contains the expected SQL, and both the focused oracle (1/1) and complete activity-permission suite (52/52) pass.
- 2026-07-29 (Slice 3 group-activity review): exact-commit correctness and simplification reviews found no defect or material reduction. They verified the required course relation, exact entity identifiers, fail-closed group-owner/member/admin expansion, complete course mapping and provenance, deterministic precedence, user/object deletion scope, soft-deletion behavior, exact conflict target, tagged SQL safety, access-request sequencing, downstream stack-element propagation, and UUID-independent exact-row assertions. Residual entity-specific tie, soft-deletion, and malformed-row cases are non-blocking because reviewed sibling coverage exercises the shared branches and production enforces `UserOrGroupRequired`.
- 2026-07-29 (Slice 3 course implementation): user-scoped and object-wide course-root recomputation now share one tagged set-based statement for owner, direct-user, and group-owner/member/admin sources, with propagation-aware deterministic tie-breaking, scoped stale deletion, and the exact course/user conflict target. The thin wrappers fetch only child IDs, update root access requests after convergence, and retain the existing sequential cascade through every activity type and its set-based descendants. A permanent exact-row oracle proves scoped isolation, object convergence, propagation-enabled equal-level precedence, group-admin removal across course and child rows, and fallback from propagating group WRITE to non-propagating direct WRITE with child EXECUTE provenance. The built bundle contains the expected SQL; the focused oracle (1/1), complete course suite (29/29), and combined activity/course-sharing/group set (142/142) pass.
- 2026-07-29 (Slice 3 course review): the exact-commit correctness review found no defect and verified exact identifiers and conflict target, fail-closed group expansion, owner/direct precedence, deterministic propagation-aware provenance, scoped and object-wide deletion, missing-course behavior, access-request ordering, every activity cascade, tagged SQL safety, and UUID-independent assertions. The simplification review identified one safe reduction: course candidates are always non-derived, so the final SQL now emits literal `false` instead of carrying that constant through the candidate and desired-row CTEs; the complete course suite remains green (29/29). Residual explicit four-child, malformed dual-principal, missing-course, and access-request sequence cases are non-blocking because existing course/activity suites cover the cascade behavior, production enforces `UserOrGroupRequired`, and the wrappers retain the reviewed ordering.
- 2026-07-29 (Gate A): five write-mode runs completed against the seeded disposable devcontainer database after a fresh util build per run. The counted reports are `20260729194349-f761b5bd`, `20260729194444-9791ae0e`, `20260729194531-93916e5c`, `20260729194618-8cffcb90`, and `20260729194708-08997dfe`; all share payload hash `9c5b3429c172212f0d7858da55b808726840432e09fdfda0f2db1f1b47006cf8`, recorded every scenario as successful, and finished with 8 cleanup successes and 0 mismatches. The Testkurs and synthetic cardinalities match the approved baseline. The payload hash differs from the earlier disposable-database run; because it includes seed-specific source identifiers, cardinality plus intra-run hash stability are used for this comparison rather than cross-seed hash equality. Nearest-rank p95 over five samples equals the observed maximum.

  | Scenario | Total median / p95 / max ms | Tx median / p95 / max ms | Queries | Rows changed | SLO result |
  |---|---:|---:|---:|---:|---|
  | Testkurs-sized user share | 1,103.76 / 1,138.20 / 1,138.20 | 1,099.72 / 1,135.34 / 1,135.34 | 1,533 | 52 | pass |
  | Testkurs-sized user revoke | 739.68 / 751.37 / 751.37 | 738.55 / 748.46 / 748.46 | 791 | 52 | pass |
  | Testkurs-sized ownership transfer | 1,587.62 / 1,623.68 / 1,623.68 | 1,586.01 / 1,621.76 / 1,621.76 | 2,321 | 104 | fail: transaction p95 |
  | Testkurs-sized group share | 1,098.42 / 1,751.91 / 1,751.91 | 1,097.25 / 1,750.67 / 1,750.67 | 1,902 | 52 | fail: transaction p95 |
  | Testkurs-sized group revoke | 1,455.87 / 1,729.47 / 1,729.47 | 1,453.44 / 1,726.65 / 1,726.65 | 1,579 | 52 | fail: transaction p95 |
  | Synthetic-large user share | 4,062.51 / 4,270.75 / 4,270.75 | 4,062.19 / 4,270.48 / 4,270.48 | 6,317 | 1,551 | fail: both p95 targets |
  | Synthetic-large user revoke | 3,275.30 / 3,686.85 / 3,686.85 | 3,274.89 / 3,686.20 / 3,686.20 | 3,213 | 1,551 | fail: both p95 targets |
  | Synthetic-large ownership transfer | 7,332.39 / 8,219.11 / 8,219.11 | 7,331.74 / 8,218.30 / 8,218.30 | 9,527 | 3,102 | fail: p95 and hard ceiling |
  | Synthetic-large group share | 6,132.08 / 8,866.20 / 8,866.20 | 6,131.73 / 8,865.58 / 8,865.58 | 7,867 | 46,530 | fail: p95 and hard ceiling |
  | Synthetic-large group revoke | 14,690.52 / 16,543.72 / 16,543.72 | 14,685.28 / 16,538.71 / 16,538.71 | 6,423 | 1,551 | fail: p95 and hard ceiling |
  | Add member to group with 20 object grants | 58.17 / 67.58 / 67.58 | 55.13 / 62.30 / 62.30 | 51 | 20 | pass |

  Compared with the Slice-1 baseline, median latency improved by 2.17×-9.79× and query counts fell by 75%-97%; the former 60-second synthetic group-share timeout now converges successfully. Gate A still fails the approved p95 mutation-latency and transaction-duration targets, and three supported large-fixture mutations breach the 5-second hard ceiling. The slowest mutation is synthetic-large group revoke at 16.54 seconds p95/max; the widest fan-out is synthetic-large group share with 46,530 changed rows. Phase B is therefore triggered, but Slice 4 has not started because its database-backed fencing/durable-dispatch prototype requires maintainer architecture approval.
- 2026-07-30 (Slice 4 start): maintainer approved the bounded architecture prototype only; production schema, sharing call-site conversion, deployment, and PR readiness remain unauthorized. Slice 4 is validating a conservative fail-closed contract: every permission source mutation, synchronous revoke, and propagation worker acquires one shared PostgreSQL transaction-level advisory lock before reading or writing permission state; the source mutation atomically increments a durable work generation; workers full-rederive the latest source state and mark the observed generation processed in the same fenced transaction; dispatch and reconciliation provide at-least-once delivery; and worker failures reach a persistent database sink while leaving the durable work unresolved. The prototype must exercise rollback atomicity, a grant-worker/revoke race, different roots that overlap one derived row, commit-before-enqueue recovery, duplicate delivery, and persistent failure reporting. The initial recovery target under test is five minutes, with a one-minute reconciliation cadence. The global fence deliberately favors a small proof surface over parallel propagation throughput; Slice 4 will stop for maintainer approval after measuring and documenting the contract.
- 2026-07-30 (Slice 4 prototype): the opt-in database-backed harness uses only synthetic identifiers and its own `permission_propagation_prototype` schema, defaults to a no-connection dry run, requires a separate `:run` command plus explicit local confirmation for writes, rejects production and non-local database targets, refuses a pre-existing prototype schema, and drops its schema in `finally`. Two complete confirmed-local runs through the committed `:run` interface passed, proving cleanup and repeatability. The scenarios proved atomic rollback of source plus durable work, blocking between an in-flight grant worker and synchronous revoke, serialization of different roots that share one derived row, coalescing of an old task onto the latest generation, duplicate-task no-op behavior, rediscovery one cadence after commit-before-enqueue, a five-minute `recoverBy`, and a persistent sanitized failure row while failed work remains unresolved. Focused Prettier and GraphQL typecheck, the repository-wide typecheck (25/25 tasks), and the prototype commit's complete pre-commit `check:all` hook passed.
- 2026-07-30 (Slice 4 review): exact-range correctness and simplification reviews accepted three contract corrections and four reductions. The work key is now a collision-safe deterministic encoding constrained to the object-type/object-ID/mode/user tuple and used as the primary key, which enforces tuple uniqueness without a redundant composite index. Repeated unresolved mutations preserve the first `dirtyAt` and `recoverBy`, preventing continuous churn from postponing the five-minute target. A stale task's persistent failure row records the latest generation observed under the fence rather than its stale input generation, with one combined scenario proving unresolved work, reconciliation eligibility, and recovery. The test pause now occurs inside the actual full-rederivation source read, and unused prototype columns were removed. ADR-0001 distinguishes the deterministic database proof from production obligations: scheduler and queue instrumentation, lock-wait and throughput measurements, SLO breach alerting, and an external alert path when database failure recording is unavailable. Two consecutive confirmed-local runs of the first review-adjusted harness and two more after the final simplifications all passed every scenario and cleaned up their isolated schema. Focused Prettier, GraphQL typecheck, and diff hygiene also pass. No production schema or call-site conversion is authorized.
...
- Completed: Slice 4 has a reviewed, repeatable database proof and proposed ADR contract.
- Pause after Slice 4: maintainer approval of the selected fence, durable-work data shape, concurrency semantics, persistent failure sink, and recovery SLO. No production schema or call-site conversion is authorized yet.

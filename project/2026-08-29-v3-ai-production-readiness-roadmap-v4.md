...
`v3.5.0-ai-rc`, qualify that exact tree on shared staging, merge it into `v3`,
...
- `v3` head `0892b61dc` includes PR #5634 (Final-AI-review trusted-workflow
  fix). `v3-ai` head `edae58628` is **88 ahead / 0 behind** `v3` — the G0
  merge of `v3` into `v3-ai` is done, and the Final AI review has **passed on
  `v3-ai`** (runs 33211452106 and 33211991290, 2026-08-28). The
  trusted-policy assumption is closed.
...
- Active rulesets today are tag-only; verify at G0 that the release captain
  can create `v3.5.0*` tags (add a bypass if not).
...
v3 ────────────────────────────────────────────────────────────┐
                                                               │
v3-ai ── normalization + core ──┬── v3.5.0-ai-rc ── qualify ──┴──> v3
        (incl. participant      │
...
9. Cut `v3.5.0-ai-rc` from the exact normalized commit.
...
1. Cut `v3.5.0-ai-rc` from the exact content-gated commit; extend the
   ruleset to it; enable the back-merge automation.
2. Set `STG_SOURCE_BRANCH=v3.5.0-ai-rc` **and repoint the ArgoCD staging
...
| RC cut + staging reset | Sep 10–11 | `v3.5.0-ai-rc` promoted from clean reset |
...
Location: **`project/_local/releases/v3.5.0-ai/manifest.md`** (gitignored —
...
3. Create the branch ruleset per §5.1 (approvals 0, promoter bypass,
   PR-only `v3-ai`); verify the tag rulesets allow `v3.5.0*` creation.
...
path-scoped equality, tag `v3.5.0-alpha.N`, deploy dark, lift the hold.

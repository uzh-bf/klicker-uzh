...
- 2026-07-23: Final integration-gap UI slice active. Q&A-only courses now enter the integrated course layout even without description, assessment, or gamification content, and the Manage Q&A tab is hidden unless the current course user has editor rights matching the overview/embed WRITE checks. Cypress adds explicit Q&A-only and READ-only course cases; PWA, Manage, and Cypress type checks pass. Browser execution reached the seeded cleanup but the first pre-existing lecturer test redirected to Auth before the new cases despite three isolated runtime configurations; one separate Chromium attempt also crashed at startup under host memory pressure. The failure is confined to the local Auth test-session harness, not a UI assertion. Next: commit and review the slice with runtime evidence explicitly pending.

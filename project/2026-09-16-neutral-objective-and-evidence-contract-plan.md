...
- K4 slice review: read-only slice-reviewer returned DONE_WITH_CONCERNS with no blocker. Proven by execution: the emission gate is OFF (the constant is false, the only production caller passes no override, and no env/GraphQL-input/config-hash route can set it); the `graph_resolution` privacy boundary holds (only `evidenceEntityIds` leaves, the internals-leak guard is unweakened and re-verified); `schema.graphql` is byte-identical under the repo's own regeneration check; all five i18n keys exist in both locales and are referenced.
- K4 review finding fixed (`4ccf4bcba7`): `designSummary` is a schema-less `Json?` column written by whichever parser version was live at parse time, and the design-review transition does not re-parse it. A build still in design review across the deploy would serve a summary without `slots`, and the new non-null GraphQL field would fail the whole `elementGenerationBuild` query for it. The resolver now defaults the slot list and each slot's evidence list to empty, the type says the field is optional, and `resolves a design summary persisted before the slot evidence field` reproduces the crash without the default (confirmed by temporarily reverting the fix: "Cannot read properties of undefined (reading 'map')").
- K4 checks after that fix: graphql focused suites 115 passed; frontend-manage 10 passed; typecheck 35 baseline errors, none in changed files; Biome clean; regeneration still byte-identical.
- PR #6088 description rewritten to cover the whole branch (both the digest scope fix and the objective/evidence work), including the gate and the rollout order.
- Deploy-time note accepted, no action: old-format worker artifacts degrade gracefully (absent `graph_resolution` yields an empty evidence list and cannot trigger a false concentration notice).
- Next: K4 final-reviewer on the integrated package; then merge and deployment authority for the worker MR !20 and this PR, which remain separately gated.
- K4 final-reviewer gate: **capability blocker**. The `final-reviewer` route errored with the account usage limit ("try again at Sep 20th, 2026 10:36 AM"), the same limit that blocked the planner and simplifier routes. Per the routing continuity rule the integrated package-level pass was completed in the main session instead: the marker journey is coherent across configuration normalization (`objectiveSource: 'neutral'` for synthesized rows, `'provided'` for lecturer rows) → blueprint emission gate → parser → GraphQL views → generated `schema.graphql`/`ops.ts` → UI, with no lossy seam; the gate is enforced in code, not merely documented (the constant is false and the only production caller passes no override); `designSummaryView` is the only path that could fail on legacy data and now defaults; the digest suite passes 5/5 with the `materialType` filter asserted; all five i18n keys exist in both locales and are referenced. This is a main-session pass, not an independent review, and should be treated as such.
- Rollout-order enforcement, verified: the ordering is enforced by the code (the gate constant), so the Klicker side cannot emit the marker before the worker release is deployed. The plan's binding order is therefore a real control, not a convention.
- Remaining honesty gap carried in the PR body: the changed review surface has no browser capture, so its rendered layout and locales are unverified until a real UI session runs (which needs a deployment).
- Exact-head review finding fixed (`d089967628`): OpenCodeReview flagged the generated-default flag as derived from an inline `'neutral'` literal while its parameter was a bare `string`, so a typo or backend rename would type-check silently. Verified and fixed at the root rather than at the call site: `NEUTRAL_OBJECTIVE_SOURCE` is now a shared constant in `@klicker-uzh/types`, `designReviewObjectives` accepts `QuestionGenerationObjectiveSource`, and the GraphQL view exposes `objectiveSource` as a real `ElementGenerationObjectiveSource` enum instead of `String`. The enum change was the actual root cause — `exposeString` erased the union from the generated client types, which is why the raw literal reached the UI at all. `schema.graphql` and the client documents were regenerated.
- Exact-head checks after that fix: graphql 140/140 (configuration, artifacts, lifecycle, blueprint, accounting), frontend-manage 10/10, `tsc` clean for graphql/types/frontend-manage, `format:check` clean, schema regeneration byte-stable.
- Verification-environment note: the earlier `tsc -p tsconfig.check.json` EXIT=0 was misleading; `tsc -p` skipped re-checking via the committed `dist/tsconfig.check.tsbuildinfo`. Re-run with an explicit out-of-tree `--tsBuildInfoFile` it is genuinely clean, and the GraphQL suites require the disposable test database on the devrouter Postgres port (`127.0.0.1:32796`, `klicker_test`) — the default `localhost:5432` route hangs.
- Remaining: exact-head CI is re-running on `d089967628`; merge and deployment authority for worker MR !20 and this PR stay separately gated.
- Rollout enablement: `BLUEPRINT_OBJECTIVE_SOURCE_ENABLED` is flipped to `true` so the neutral-objective marker reaches the worker by default. Precondition enforced before merge: the companion worker pin (helm-charts `rs/pin-kg-workers-evidence-anchoring`, build `f884cbe`, per-slot evidence anchoring) is deployed and verified in PRD. The withheld path stays covered through the explicit `emitObjectiveSource: false` option.

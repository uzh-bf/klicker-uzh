...
|---|---|---|---|
| D1 | i18n locale resolution | Cookie `NEXT_LOCALE`, **no** switcher. Resolve in root layout, `setRequestLocale`, pass `locale`+`messages` explicitly, fallback `en`. | Slice 2 |
...
- Do: sweep remaining hardcoded user-facing strings into `chat` namespace (de+en), same convention.
- Check: `pnpm --filter chat check`; browser spot-check en + de.
- Commit: `refactor(chat): externalize remaining strings to i18n`.
...
- [ ] Slices 3–9
- Next: Slice 3 — migrate remaining chat strings (settings-panel "Settings"/"Chat Mode"/"AI Model"/"Select AI Model"/reasoning/credits copy, + any other hardcoded UI). Slice 1 evidence: after-screenshots in session scratchpad; runtime tokens `--primary`/`--ring` #0028a5, `--sidebar-accent` #ccd4ed, `--radius` .5rem.

## Open Risks

- Slice 8: RAGToolUI may have been disabled for a real defect → downscope path noted.
- Slice 2: RESOLVED — de cookie flips `<html lang>` at runtime; `setRequestLocale`↔`requestLocale` share one request cache (source-verified) so server APIs stay cookie-consistent. Authed string screenshot deferred to Slice 4.

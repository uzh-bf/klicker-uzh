...
- [ ] Slices 2–9
- Next: Slice 2 — i18n runtime wiring + first migrated string. Slice 1 evidence: after-screenshots (disclaimer modal, main chat, blue send button, active-thread `bg-primary/15` highlight) in session scratchpad; runtime tokens `--primary`/`--ring` #0028a5, `--sidebar-accent` #ccd4ed, `--radius` .5rem; prettier clean.

## Open Risks

- Slice 8: RAGToolUI may have been disabled for a real defect → downscope path noted.
- Slice 2: App Router next-intl is net-new here; verify de cookie flips output at runtime, not just tsc.

...
- [ ] Slices 6b–9 (6b next: message-turn restyle. Surveyed — off-brand targets: edit-composer send/cancel buttons hardcode `#000000`/`#ffffff` inline (thread.tsx ~L1024-1047) → send should use primary/UZH-blue fill + cancel muted; assistant avatar `hover:bg-uzh-red-20` (~L1140/L1154) → neutral; slate left-borders in `PartGroup` L1109 + `AssistantReasoningPart` L215 → `border-border`. Action-bar/branch-picker icon buttons already on-brand (`bg-accent`/`ring-ring`). Edit buttons are direct design-system `Button`s (no Slot) so `className.root` *may* apply — but browser-verify the fill lands, or fall back to plain `<button>` per Slice 6a's Slot finding.)

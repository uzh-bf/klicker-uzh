...
| 22 | Audit infrastructure is provisioned through the existing df-cloud Pulumi infrastructure in the shared `DF_Klicker_RG`, using dedicated audit storage and workload identities. No separate resource group is created. Existing group ownership is unchanged; inherited group/subscription data access must be reviewed alongside account-scoped evidence grants. The audit ingress runs in the same AKS cluster as a separate deployment with its own route; cluster-level shared fate is an accepted, documented limitation. |

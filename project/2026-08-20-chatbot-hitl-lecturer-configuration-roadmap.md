...
| 2026-08-24 | M1/U4 — final model registry policy | The user explicitly authorized one fifth layer above ready PR #5490 as an exception to the normal four-layer stack preference. The approved plan is `project/2026-08-24-chatbot-u4-model-registry-policy-plan.md`; branch `rs/chatbot-u4-model-registry-policy` starts at exact published U3 head `8b17ba74b`. U4 verifies every stored input/output rate, makes Luna the sole base model and current same-class fallback, and uses the user-approved rounded Auto rate of 1/5 derived from the approximate 90% Luna / 10% Sol generation mix. | Commit the approved plan, implement and review U4, publish one draft PR above #5490, account for CI, and retain the exact local runtime for the user's in-app Browser validation. Merge and deployment remain withheld |

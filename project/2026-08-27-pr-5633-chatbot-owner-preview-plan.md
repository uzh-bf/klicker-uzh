...
- Status: P1–P4 and the post-integration final review are complete in draft PR [#5633](https://github.com/uzh-bf/klicker-uzh/pull/5633). The approved one-time integration of `origin/v3-ai` is committed, and the combined owner-preview and Manage-assistant behavior passes local verification. The PR publication refresh remains.
- Completed: research, planner review, P0 blocker ruling, P1 owner authorization and stateless preview route, P2 owner-preview page, P3 Manage entry point, both simplifier passes, both slice reviews, their corrections, the same-reviewer correction passes, the P4 browser gate, and the integrated final review with its correction pass.
- Verification: the full Chat suite passes (83 files and 728 tests passed; 13 integration tests skipped by their existing environment gate). Chat and Manage type checks pass; package lint has no errors; repository formatting and `git diff --check` pass. Focused Manage access-scope and URL-helper assertions pass through the installed TypeScript runner. Review corrections constrain preview tools to `doc_query`, add executable route coverage, disable attachment drops, preserve run-status announcements, keep the signed-out preview recoverable, hide the Manage action from `READ_ONLY` and `SESSION_EXEC` scopes, remove brittle source-string coverage, and keep package-local tests within their owning app.
...
- Final review and publication: head `5fa7b9da2` is pushed and the PR description carries exact-head evidence and screenshots. Only the repository-wide `trusted_policy` check failed on this head; the automated review, build, codebase check, GraphQL and lecturer MCP suites, and all eight hosted Playwright shards passed. The native final reviewer found no blocking issue, corrected two low test-maintainability findings in `ad629a16e`, and passed the correction range with no remaining findings; the report stays under the gitignored `project/_local/reviews/` path.
- Publication pass and readiness: the publication pass verified the pushed head `d244ade3d` (description accuracy, screenshot-caption match, plan/report consistency, no merge or live claims) and the PR is now ready for review. `trusted_policy` remains red as the owner-approved repository-wide infrastructure recurrence. Merge, deployment, staging/production mutation, and live activation remain withheld and now await merge authority.
- Delivery: achieved layer is a published, ready-for-review PR package. Terminal condition reached; the only open item before merge is merge authority plus green required checks at merge time.

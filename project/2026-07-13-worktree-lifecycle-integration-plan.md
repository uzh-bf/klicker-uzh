...
- Target: `v3`
- Dependency: devrouter `0.0.28` (worktree lifecycle hardening, packaged-prompt resolution, and refreshable worktree-aware agent guidance).
...
- Require devrouter `0.0.28` in `.devrouter.yml`.
...
- Current: implementation, published-package live gate, browser smoke, and final review complete locally.
- Next: push this branch, open a draft PR to `v3`, and monitor CI.
...
- Result: devrouter `0.0.28` and `devrouter workspace ensure .` are canonical for existing linked worktrees. Unsupported primary-checkout proxy instructions were removed; the primary checkout remains the one-at-a-time localhost fallback.
...
- Result: passed on 2026-07-13 with published `@devrouter/cli@0.0.28`.
...
- Runtime: final state `22079 22079 3348913417-1223`; `WORKSPACE` and `DEVROUTER_WORKSPACE` both equal the persisted identity; in-container Git resolves `/workspaces/klicker-uzh`. One Turbo `dev` task set runs the routed apps and workers; no duplicate `dev:lti` tasks or `EADDRINUSE` failures remain.
- Routing: published `@devrouter/cli@0.0.28` verified nine HTTPS routes plus the PostgreSQL TCP route. A warm ensure preserved both app container `8d80516f34ce` and owned process group `22079`.

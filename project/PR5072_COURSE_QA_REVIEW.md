...
and the DB-backed discussion suite now passes `32/32`. The remaining rollout
...
| State-creating embed query | Resolved: embed links are generated by one authenticated, permission-checked mutation with bounded inputs and persisted-manifest regression coverage |
...
- Fresh real-browser evidence covers course, practice, test-published
  ...
  `42` and `43`. The backend suite passes `32/32`.
...
- **Query with side effects (resolved)**: `generateCourseDiscussionEmbeddingInfo` is now a GraphQL mutation with authenticated `WRITE` access and bounded inputs. It still creates `DiscussionScope` rows by design; scope cleanup/list-existing-scopes UX remains a post-alpha improvement for lecturer typos and reuse.

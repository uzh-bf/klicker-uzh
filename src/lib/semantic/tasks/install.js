...
module.exports = function (callback) {
  let currentConfig = requireDotFile('semantic.json'),
...
---------------*/
  ...
  gulp.task('run setup', () => {
    ...
    if (install.shouldAutoInstall()) {
      ...
      return gulp.src('gulpfile.js').pipe(
        prompt.prompt(questions.setup, (setupAnswers) => {
          // hoist
  ...
  gulp.task('create install files', (callback) => {
    /*--------------
   Exit Conditions
  ---------------*/
  ...
  ---------------*/
    ...
    gulp.task('create theme.config', () => {
      let // determine path to site theme folder from theme config
        ...
        pathToSite = path
          .relative(path.resolve(installPaths.themeConfigFolder), path.resolve(installPaths.site))
          .replace(/\\/g, '/'),
  ...
  ---------------*/
    ...
    gulp.task('create semantic.json', () => {
  ...
  gulp.task('clean up install', () => {
    ...
    return gulp.src('gulpfile.js').pipe(
      prompt.prompt(questions.cleanup, (answers) => {

...
module.exports = function () {
  ...
  gulp.watch(
    ...
    (file) => {
      ...
      isConfig =
        file.path.indexOf('theme.config') !== -1 || file.path.indexOf('site.variables') !== -1
  ...
  gulp.watch([`${source.definitions}/**/*.js`], (file) => {
    gulp
      .src(file.path)
      .pipe(plumber())
      .pipe(gulpif(config.hasPermission, chmod(config.permission)))
  ...
  gulp.watch([`${source.themes}/**/assets/**/${globs.components}?(s).*`], (file) => {
    // copy assets
    gulp

  ...
  $.fn.dropdown = function (parameters) {
    ...
    $allModules.each(function (elementIndex) {
      ...
      module = {
        ...
        filter(query) {
          let searchTerm = query !== undefined ? query : module.get.query(),
        ...
        fuzzySearch(query, term) {
          ...
          search: for (
            let characterIndex = 0, nextCharacterIndex = 0;
        ...
        event: {
          ...
          test: {
            toggle(event) {
              const toggleBehavior = module.is.multiple() ? module.show : module.toggle
          ...
          item: {
            ...
            mouseleave(event) {
              const $subMenu = $(this).children(selector.menu)
              if ($subMenu.length > 0) {
                clearTimeout(module.itemTimer)
                module.itemTimer = setTimeout(() => {
        ...
        get: {
          ...
          value() {
            let value = $input.length > 0 ? $input.val() : $module.data(metadata.value),
          ...
          selectValues() {
            ...
            if (settings.sortSelect) {
              select.values.sort((a, b) => (a.name > b.name ? 1 : -1))
        ...
        add: {
          label(value, text, shouldAnimate) {
            let $next = module.is.searchSelection() ? $search : $text,
        ...
        is: {
          ...
          verticallyScrollableContext() {
            const overflowY = $context.get(0) !== window ? $context.css('overflow-y') : false
            return overflowY == 'auto' || overflowY == 'scroll'
          },
          horizontallyScrollableContext() {
            const overflowX = $context.get(0) !== window ? $context.css('overflow-X') : false
        ...
        escape: {
          value(value) {
            let multipleValues = $.isArray(value),

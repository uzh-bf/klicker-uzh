...
test('telemetry is values-free and preserves measurable fields', (t) => {
  ...
  Object.assign(process.env, {
    ...
    PLAYWRIGHT_BUILD_GRAPH_MODE: 'bounded',
    PLAYWRIGHT_BUILD_GRAPH_REASON: 'bounded-profile',
  ...
  assert.deepEqual(buildTelemetry(), {
    ...
    route: 'public-pr',
    mode: 'full',
    buildGraphMode: 'bounded',
    buildGraphProfile: null,
    buildGraphReason: 'bounded-profile',
    runner: 'runner-01',
    platform: 'Linux',
    architecture: 'ARM64',
    cacheEnabled: true,

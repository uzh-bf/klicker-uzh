...
describe('Performance', () => {
  ...
  describe('Logging Overhead', () => {
    it('should meet <1ms per log call requirement in production', () => {
      ...
      consoleLogSpy.mockRestore()
      console.log(`Performance: ${perCallMs.toFixed(3)}ms per call (${iterations} calls in ${durationMs.toFixed(1)}ms)`)
      consoleLogSpy = jest.spyOn(console, 'log').mockImplementation()
    ...
    it('should handle high-volume logging efficiently', () => {
      ...
      const expectedOperations = Math.floor(operationsPerMinute * testDurationMs / 60000)
      ...
      while (Date.now() < deadline) {
        logger.info('Operation processed', createOperationContext(`op-${operations}`, 'TEST_OPERATION'))
        operations++
      ...
      consoleLogSpy.mockRestore()
      console.log(`High-volume test: ${operations} operations in ${durationMs.toFixed(0)}ms (${opsPerSecond.toFixed(1)} ops/sec)`)
      consoleLogSpy = jest.spyOn(console, 'log').mockImplementation()
  ...
  describe('Context Creation Performance', () => {
    ...
    it('should calculate performance metrics efficiently', () => {
      ...
      const avgDurationNs = durations.reduce((a, b) => a + b, 0) / durations.length

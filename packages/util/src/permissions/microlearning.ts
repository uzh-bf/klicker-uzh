...
async function recomputeMicroLearningPermissionsSetBased(
  ...
  await prisma.$executeRaw(
    DB.Prisma.sql`
      ...
      expanded_group_users AS (
        ...
        WHERE permission."microLearningId" = ${id}::uuid
      ...
      deleted_permissions AS (
        ...
        WHERE derived_permission."microLearningId" = target_activity."id"
          AND (
            target_scope."userId" IS NULL
      ...
      SELECT
        ...
        CURRENT_TIMESTAMP
      FROM desired_permissions
      ON CONFLICT ("microLearningId", "userId")
      DO UPDATE SET
 ...
 *   includes the considered microlearning:
...
export async function recomputeMicroLearningPermissionsUser(
  ...
  await recomputeMicroLearningPermissionsSetBased({ id, userId }, prisma)
 ...
 *
 * The set-based recomputation converges all rows for the microlearning, choosing
 ...
 *   microlearning, according to the same rules as the user-specific recomputation
 *   above.
 *
 * Additionally, a recomputation of the derived permissions on all elements
 * used in the activity is triggered.
...
export async function recomputeMicroLearningPermissionsObject(
  ...
  await recomputeMicroLearningPermissionsSetBased({ id }, prisma)

...
import { lockAdaptiveLearningCourseEnabled } from '@klicker-uzh/adaptive-server/services/adaptiveLearningRollout'
import {
  assertAdaptivePublishedPool,
  clearAdaptivePublishedPool,
  materializeAdaptivePracticeQuizPool,
} from '@klicker-uzh/adaptive-server/services/adaptivePracticeQuizPublication'
import { purgeAttemptFreeAdaptivePublications } from '@klicker-uzh/adaptive-server/services/adaptivePracticeQuizPublicationCleanup'
import {
...
} from '@klicker-uzh/adaptive-server/services/adaptivePracticeQuizRepository'
import { withAdaptiveOperationalTransaction } from '@klicker-uzh/adaptive-server/services/adaptiveTransactions'
...
import { UNPUBLISHED_ACTIVITY_STATUSES } from './activities.js'
...
export async function deletePracticeQuiz(
  ...
  const deletion = await withAdaptiveOperationalTransaction(
    ...
    async (prisma) => {
      ...
      if (
        onlyIfUnpublished &&
        !UNPUBLISHED_ACTIVITY_STATUSES.includes(practiceQuiz.status)
      ) {

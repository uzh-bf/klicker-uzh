...
async function run() {
  ...
  for (const response of responses) {
    ...
    // make sure that question detail responses are sorted by creation date
    detailResponses.sort((a, b) => {
    ...
    } else if (response.elementInstance!.elementType === ElementType.CONTENT) {
      // loop over the detail responses and aggregate the correctness values
      const {
        correctCount,
        correctCountStreak,

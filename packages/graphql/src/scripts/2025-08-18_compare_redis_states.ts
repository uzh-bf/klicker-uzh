...
function generateReport(results: ComparisonResult[]): void {
  ...
  for (const result of results) {
    ...
    if (summary.discrepancies === 0) {
      ...
      if (keyComparison.lqOnly.length > 0) {
        console.log('\n  Keys only in lq:')
        keyComparison.lqOnly.slice(0, 5).forEach(k => console.log(`    - ${k}`))
      ...
      if (keyComparison.lqV2Only.length > 0) {
        console.log('\n  Keys only in lqV2:')
        keyComparison.lqV2Only.slice(0, 5).forEach(k => console.log(`    - ${k}`))
      ...
      if (valueDifferences.length > 0) {
        ...
        if (valueDifferences.length > 3) {
          console.log(`    ... and ${valueDifferences.length - 3} more differences`)
        }
      }
    }
  ...
  console.log(`  - Quizzes with discrepancies: ${results.length - perfectMatches}`)
  ...
  if (totalDiscrepancies === 0) {
    ...
    console.log('⚠️  WARNING: Discrepancies detected. Review the differences above.')
  }
}

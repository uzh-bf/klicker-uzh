...
import {
  type ParticipantDataUseContext,
...
function participantContext(participantId: string): ParticipantDataUseContext {
  return {
    prisma,
...
async function readAnalyticsCounts(participantId: string) {
  ...
  const [course, performance, activity, competency] = await Promise.all([
    prisma.participantCourseAnalytics.count({ where: { participantId } }),
    prisma.participantPerformance.count({ where: { participantId } }),
    prisma.participantActivityPerformance.count({ where: { participantId } }),
    prisma.competencyAnalytics.count({
      where: { participantAnalyticsId: { in: analytics.map(({ id }) => id) } },
    }),
  ])
...
async function holdSharedAnalyticsLock() {
  ...
  const transaction = prisma.$transaction(
    async (tx) => {
      await tx.$executeRaw`SELECT pg_advisory_xact_lock_shared(${LEARNING_ANALYTICS_ADVISORY_LOCK.classId}, ${LEARNING_ANALYTICS_ADVISORY_LOCK.objectId})`
      acquired.resolve()
...
describe('participant analytics withdrawal PostgreSQL integration', () => {
  ...
  it('records a revisioned withdrawal through the choice writer before deleting analytics', async () => {
  ...
  it('processes only pending requests at or before the scheduled cutoff', async () => {
    ...
    expect(await readAnalyticsCounts(oldFixture.participantId)).toEqual({
      participant: 0,
      competency: 0,
      course: 0,
      performance: 0,

...
/**
 * Reads the actor's product update feed: the catalog entries that may be shown
 * to the given audience on the given surface right now, each paired with the
 * actor's stored read state, plus the write calls that record what happened to
 * a card.
 *
...
export function useProductUpdates({
  ...
  // did not change the eligible set.
  ...
  return {
    ...
    // way, so the unread indicator stays quiet instead of flashing on every
    // cold load.

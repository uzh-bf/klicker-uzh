...
export function KnowledgeGraphDetails({
  ...
  const heading = node?.displayLabel ?? edge?.label ?? 'Details'
        ...
        <div className="mb-5 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-[#4C4C4C]">
              {node === undefined ? 'Relationship' : 'Concept'}
            </p>
        ...
        {node === undefined ? null : (
          <div className="space-y-5">
            <dl className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <dt className="font-semibold text-[#4C4C4C]">Type</dt>
                <dd className="mt-0.5 text-[#121212]">{node.kind}</dd>
              </div>
              <div>
                <dt className="font-semibold text-[#4C4C4C]">Connections</dt>
            ...
            >
              {isExpanding ? 'Loading connections…' : 'Expand connections'}
            </button>
          </div>
        )}
          ...
          <div className="space-y-5">
            <dl className="grid grid-cols-1 gap-3 text-sm">
              {edgeEndpoints === undefined ? null : (
                <>
                  <div>
                    <dt className="font-semibold text-[#4C4C4C]">From</dt>
                    <dd className="mt-0.5 break-words text-[#121212]">
                      {edgeEndpoints.source}
                    </dd>
                  </div>
                  <div>
                    <dt className="font-semibold text-[#4C4C4C]">To</dt>
                    <dd className="mt-0.5 break-words text-[#121212]">

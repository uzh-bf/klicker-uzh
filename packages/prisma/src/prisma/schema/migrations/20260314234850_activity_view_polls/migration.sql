...
SELECT
  ...
  -- Activity features
  po."areInstancesOutdated",
  false as "isGamificationEnabled",
  false as "isAssessmentEnabled",
  1 as "pointsMultiplier",
  ...
  -- Type-specific scheduling fields
  CAST(NULL AS TIMESTAMP(3)) as "availableFrom",
  CAST(NULL AS TIMESTAMP(3)) as "scheduledStartAt",
  CAST(po."endDate" AS TIMESTAMP(3)) as "scheduledEndAt",

  -- Course information
  CAST(NULL AS UUID) as "courseId",
  CAST(NULL AS TEXT) as "courseName",
  CAST(NULL AS "Locale") as "courseLanguage",
  CAST(NULL AS TIMESTAMP(3)) as "courseStartDate",
  ...
  -- Permission counts for numSharedUsers and isActivityReviewer calculation
  COALESCE(activity_perm_counts.num_permissions, 0) as "numActivityPermissions",
  false as "isUserCourseAdmin"

FROM "Poll" po
JOIN "DerivedPermission" dp ON dp."pollId" = po.id
LEFT JOIN (
  SELECT p.id, p."userGroupId" as user_group_id
  FROM "Permission" p
) p_direct ON p_direct.id = dp."directPermissionId"
LEFT JOIN "ActivityTemplate" at ON at."pollId" = po.id
LEFT JOIN (
  SELECT 
    es."pollId",
  ...
  WHERE es."pollId" IS NOT NULL
  GROUP BY es."pollId"
) stack_counts ON stack_counts."pollId" = po.id
LEFT JOIN (
  SELECT 
    p."pollId",
    COUNT(p.id) as num_permissions
  ...
  GROUP BY p."pollId"
) activity_perm_counts ON activity_perm_counts."pollId" = po.id

...
export function getChatModelRegistry(): ChatModelCapability[] {
  ...
  try {
    ...
    console.warn(
      '[api] Invalid CHAT_MODEL_REGISTRY_JSON; falling back to built-in defaults.',
      error
    )
    cachedChatModelRegistry = DEFAULT_CHAT_MODEL_REGISTRY
    return cachedChatModelRegistry
...
export function resetChatModelRegistryCacheForTests() {
  cachedChatModelRegistry = null
}

export function getPublicChatModelRegistry() {
  return getChatModelRegistry().map((model) => ({
    id: model.id,
    name: model.name,
...
type ChatbotServiceContext = {
  prisma: PrismaClient
  userId: string
}

export async function getChatbotsInfo({
  prisma,
  userId,
}: ChatbotServiceContext) {
  ...
  return chatbots.map((chatbot) => {
    const {
      course,
      disclaimer,
      mcpConfigurations: rawMcpConfigurations,
      ...chatbotFields
    } = chatbot
    ...
    const disclaimerSummary = disclaimer
      ? {
          ...disclaimer,
          acceptedCount,
          declinedCount,
    ...
    const mcpConfigurations = rawMcpConfigurations.map((config) => ({
...
type ChatbotModelSettingsUpdateArgs = {
  chatbotId: string
...
export async function updateChatbotModelSettings(
  args: ChatbotModelSettingsUpdateArgs,
  { prisma, userId }: ChatbotServiceContext
) {
  ...
  if (unknownAllowedModelIds.length > 0) {
    throwBadRequest(`Unknown model id(s): ${unknownAllowedModelIds.join(', ')}`)
  }
  ...
  for (const entry of args.allowedReasoningEffortsByModel ?? []) {
    ...
    if (!model) {
      throwBadRequest(`Unknown model id in reasoning config: ${entry.modelId}`)
    }
    if (!model.supportsReasoning) {
      throwBadRequest(
  ...
  const { course, ...updatedFields } = updated

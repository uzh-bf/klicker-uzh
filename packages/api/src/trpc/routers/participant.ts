...
export const participantRouter = router({
  ...
  respondToElementStack: publicProcedure
    .input(participantRespondToElementStackInput)
    .mutation(async ({ ctx, input }) => {
      const prisma = getPrisma(ctx)
      const participantId =
        ctx.user?.role === UserRole.PARTICIPANT ? ctx.user.sub : undefined

      return respondToElementStack({
        courseId: input.courseId,
        isOwner: input.isOwner ?? false,
        participantId,
        prisma,
        responses: input.responses,
        stackAnswerTime: input.stackAnswerTime,
        stackId: input.stackId,
      })
    }),
  ...
  startGroupActivity: participantProcedure
    .input(participantStartGroupActivityInput)
    .mutation(async ({ ctx, input }) => {
      const prisma = getPrisma(ctx)
  ...
  setLiveQuizPin: publicProcedure
    .input(participantSetLiveQuizPinInput)
    .mutation(async ({ ctx, input }) => {
      const prisma = getPrisma(ctx)

      return setLiveQuizPinCookie({
        liveQuizId: input.liveQuizId,
        pin: input.pin,
        prisma,
        res: ctx.res,

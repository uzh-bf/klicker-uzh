...
describe('adaptive-learning production-shaped simulations', () => {
  ...
  it.each(
    CANONICAL_PRODUCT_SCENARIOS
  )('$config.label satisfies every Phase 11 engineering regression gate', (scenario) => {
  ...
  it.each(
    stressScenarios
  )('$config.label produces complete, stratified evidence', (scenario) => {
    const result = resultFor(scenario)
    ...
    for (const trace of result.learnerTraces) {
      ...
      expect(trace.selectedItemIds.every((itemId) => itemIds.has(itemId))).toBe(
        true
      )
  ...
  )('keeps a %s pool within the analytical reachability range', (type, count) => {

...
describe('adaptive item calibration', () => {
  ...
  it.each([
    ...
    'FLAGGED',
    'RETIRED',
  ] as const satisfies readonly AdaptiveCalibrationStatus[])('excludes %s parameters from Diagnostic estimation', (status) => {
  ...
  it.each([
    ...
    ['MC', 4],
    ['KPRIM', 4],
  ] as const)('derives fixed guessing for provisional %s items', (type, count) => {
    expect(
      resolveEffectiveItemParameters({
        calibration: null,
        provisionalDifficulty: 0,
  ...
  ] as const)('accepts a compatible calibrated %s model', (itemType, model, guessing) => {
    expect(
      resolveEffectiveItemParameters({
  ...
  it.each([
    ...
    string,
    AdaptiveItemModel,
  ])[])('rejects incompatible %s and %s combinations', (itemType, model) => {
    expect(() =>
      resolveEffectiveItemParameters({
  ...
  it.each([
    ...
    1.5,
    Number.NaN,
  ])('rejects malformed choice counts before deriving guessing: %s', (choiceCount) => {
    expect(() =>
      resolveEffectiveItemParameters({
        calibration: null,
        provisionalDifficulty: 0,
  ...
  it.each([
    2, 3, 5,
  ])('requires exactly four statements for KPRIM: %s', (choiceCount) => {
    expect(() =>
      resolveEffectiveItemParameters({
        calibration: null,
        provisionalDifficulty: 0,

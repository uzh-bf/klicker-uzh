...
describe('Create different types of elements (with and without sample solution) and edit them', () => {
  ...
  it('Create a Selection question', () => {
    ...
    cy.get('[data-cy="configure-number-of-inputs"]')
      .click()
      .clear()
      .type(String(SESolutions.length + SESolutionsNotChosen.length)) // maximum number of inputs = options - 1
    cy.get('[data-cy="save-new-question"]').should('be.disabled')
    cy.get('[data-cy="configure-number-of-inputs"]')
      .click()
      .clear()
      .type(String(SESolutions.length + SESolutionsNotChosen.length - 1))
    cy.get('[data-cy="save-new-question"]').should('not.be.disabled')
    cy.get('[data-cy="configure-number-of-inputs"]')
      .click()
      .clear()

...
- The B1 export reads only the generation-scoped `LiveQuizRespondent.exportLabel` values written by A5 finalization. It does not persist source-identity hashes, assign labels during export, or depend on the cleared `exportSalt`; it rejects incomplete settlement, remaining active bindings, or unfinalized respondents before loading response rows.
...
- **Correlated response export is a separate read boundary**: export locks the ended quiz, requires settlement-complete receipts and finalized generation-scoped respondent labels, excludes free-text instances before size preflight, and never loads source identity keys into the CSV builder. Keep correlated quizzes out of the legacy identifiable course export.

...
- **Correlated response acknowledgement uses a transactional outbox**: `LiveQuizPendingResponse.id` is the stable Hatchet message id and its unique `responseKey` enforces one pending response per respondent and quiz execution. The response API creates the encrypted validated response, admitted identity, and acceptance-time instance snapshot while holding the quiz row lock before enqueue acknowledgement. Hatchet receives only the message id; the worker loads the authoritative row and retries until terminal handling deletes it. Correlated export takes the same quiz lock and refuses to run while any outbox row remains. Deploy the migration before response API or worker code that reads this table.
- **Plan a lock budget for the correlated-response migration**: it adds constraints and a non-concurrent index to `LiveQuizResponse`. Apply it in a maintenance window or after confirming the target table size and acceptable write-lock duration; do not treat the migration-first rollout order as permission for an unbounded production lock.

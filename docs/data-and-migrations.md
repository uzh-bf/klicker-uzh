...
- The A1 domain migration `20260815090000_live_quiz_respondent_bindings` adds the persisted monotonic publication generation alongside the generation-scoped respondent and binding shape. The A5 publication migration `20260812010000_live_quiz_publication_materialization` adds nullable materialization/retry timestamps and the reconciliation lookup index. Publication remains backward-compatible: the database generation is recorded before Redis metadata is written, incomplete rows are retried by the general worker, and an abort tombstone prevents a delayed older publication from recreating its Redis generation without rejecting an immediate republish in the same millisecond.
- The B1 export migration `20260812030000_live_quiz_response_export_labels` is an interim implementation that stores HMAC-derived identities and assigns labels during export. Before the stack is ready, replace that shape with the ADR-0006 contract: persist the immutable label on the respondent during A5 finalization and make B1 render finalized labels without retaining identity hashes or assigning labels lazily.
...
- **Live-quiz response ownership is mode-specific**: assessment responses retain `participantId`; standard correlated responses use `respondentId` even when the browser is logged in. The A1-A5 adaptation scopes correlated respondents and active bindings by `publicationGeneration`, enforcing one binding per respondent and one respondent per participant or anonymous credential in that generation. Keep the exclusive-owner checks, unique indexes, and service lookups aligned with [ADR-0005](./adr/0005-separate-live-quiz-response-identity-policies.md). Correlated publication remains disabled until the downstream export/UI cascade and finite-retention gate are complete.
- **Correlated credentials are active-only data**: account mappings and respondent-token hashes belong in a separate binding that is deleted only after the generation is ended and every receipt has `settledAt` set with no payload or retry schedule remaining. Admission holds a shared quiz-row lock through receipt insertion; ending/finalization takes the exclusive lock before checking that predicate. Finalization keeps the minimal respondent parent for referential integrity, allocates immutable labels, clears transitional identity fields, deletes settled receipt metadata, removes the export salt, and is irreversible ([ADR-0006](./adr/0006-finalize-correlated-identities-after-settlement.md)). The current A1-A5 implementation remains expand-contract compatible; do not drop the existing owner relation while old writers can still be present.
- **Correlated live-quiz responses have exclusive owners**: the identity check requires exactly one of `participantId` and `respondentId`, and separate unique indexes protect each nullable owner. Keep the check, both indexes, and the two Prisma compound-unique lookups aligned with the mode-specific ownership rule above.
...
- **Correlated response export is a separate persistence boundary**: `LiveQuizResponseExportLabel` contains no source identity, account, email, or response data. Export locks the ended quiz, refuses unsettled receipts, excludes free-text instances before size preflight, and strips identity keys before CSV generation. Keep correlated quizzes out of the legacy identifiable course export.

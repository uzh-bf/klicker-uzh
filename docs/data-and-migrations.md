...
- The export slice adds `20260812030000_live_quiz_response_export_labels` after the A5 migrations. `LiveQuizResponseExportLabel` stores only an HMAC-derived identity hash and a positive, quiz-scoped label; its composite primary key and per-quiz label uniqueness keep later downloads stable. The export service locks the ended quiz row before reading or appending labels, so concurrent downloads cannot assign the same label twice.

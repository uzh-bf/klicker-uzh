...
- **Correlated response acknowledgement uses a transactional outbox and receipt**: `LiveQuizPendingResponse.id` is the stable Hatchet message id and its unique `responseKey` permanently records the first accepted response per respondent and quiz execution. The response API creates the encrypted validated response, admitted identity, and acceptance-time instance snapshot while holding the quiz row lock before enqueue acknowledgement. Hatchet receives only the message id; the worker loads the authoritative row and retries until terminal handling sets `settledAt` and erases the ciphertext. Correlated export takes the same quiz lock and refuses to run while any unsettled outbox row remains. Deploy the migration before response API or worker code that reads this table.
- **Correlated-response migration avoids a table-write outage**: the new respondent uniqueness boundary uses `CREATE UNIQUE INDEX CONCURRENTLY`, and the respondent foreign key is added `NOT VALID` before explicit validation. Prisma PostgreSQL migrations are not transaction-wrapped by default, which is required for concurrent index creation. Run the migration through the production deployment path before enabling correlated publication, monitor concurrent-index progress/failure, and never wrap this migration in a transaction.

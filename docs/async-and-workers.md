...
- `build-kb-knowledge-graph` dispatches the active KB's immutable source manifest to the external graph workflow, correlates the returned run id, and never publishes an unverified graph or artifact path.
- `monitor-kb-graph-builds` runs every minute, reconciles graph workflow status, cancels timed-out runs, and requires a versioned terminal-result callback before settlement or publication. Provider `COMPLETED` is not sufficient. A missing callback, malformed result, failure, cancellation, or timeout clears the active slot without moving the published pointer and holds the reservation as `NEEDS_HUMAN_REVIEW`; automatic release applies only to an ordinary `RESERVED` build. A late valid success callback may settle the existing held build once, but a malformed or late failure result remains held and every callback still passes the same identity, artifact, and metering checks.
...
The interim backend kill switch `KB_INGESTION_DISABLED=true` is checked by `packages/graphql/src/services/knowledge.ts:assertKbIngestionEnabled`. It refuses new upload tickets, URL resources, and ingestion attempts while leaving reads, deletion, cleanup, and already-queued worker reconciliation live. The separate `KB_GRAPH_DISABLED=true` switch refuses graph opt-in and rebuild dispatches; it does not revoke an already published graph. The graph switch and cost settings must be injected into the GraphQL backend's environment; the chart currently maps only the non-secret external graph-worker connection settings.

...
For `CORRELATED_EXPORT`, initialization is the only route that may mint an anonymous quiz-scoped token. Submission uses the dedicated `/AddCorrelatedResponse` endpoint, resolves the token from its HttpOnly cookie or the PWA's in-memory fallback header, checks for an existing durable response, and acquires a five-minute per-identity Redis lease before enqueueing. The lease only provides synchronous duplicate feedback at ingress; it is not a delivery prerequisite after Hatchet accepts the versioned durable event. Old response API replicas do not implement the dedicated endpoint and therefore cannot silently accept a new correlated submission through the legacy aggregate path.

The worker's correlated path is isolated in `processors/correlatedResponse.ts`. It validates the signed owner and event claim shape, serializes duplicate processing with a lease, writes the `LiveQuizResponse`, then applies all Redis hash mutations plus the processed marker in one Lua operation. The script validates every key type and integer before its first write. Network-ambiguous retries read the processed marker; a lease abandoned before persistence expires and can be retried.

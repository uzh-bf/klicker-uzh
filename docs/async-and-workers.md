...
CODE questions use a separate receipt-backed path. The participant mutation creates or returns the reusable `CodeSubmission`, then enqueues `grade-code-submission` on the general worker. An enqueue error leaves the accepted receipt pending instead of deleting it; `recover-code-submissions` runs every minute in bounded batches to enqueue due pending or expired work and closes expired receipts that exhausted three claims. A CodeAPI `429` releases the claim to `PENDING` with a database-held `retryAt` derived from `Retry-After` (30-second default, bounded to 1 second through 5 minutes), and the current Hatchet attempt returns successfully; recovery will not re-enqueue it before that timestamp. The worker claims the receipt for seven minutes, while CodeAPI applies one configurable deadline (at most five minutes) across the public and hidden batches and Hatchet permits six minutes. PracticeQuiz and MicroLearning finalization locks the matching claim and shared aggregate row, then commits the normal response effects and completion together. LiveQuiz receipt creation locks the quiz lifecycle row so it cannot race block closure, and quiz end is rejected while a block is active or any CODE receipt is pending/running. Finalization binds the captured block execution, stores one `LiveQuizResponse` and the sanitized grader result, then projects Redis counters outside the database transaction with a receipt marker. The atomic projection first verifies the current block-execution marker and refuses stale workers before any leaderboard, XP, or result write; a missing or mismatched marker is a recoverable projection failure and cannot complete the receipt. Once that durable result exists, projection retries no longer consume the receipt's grading-failure budget: recovery keeps the receipt pending until the idempotent Redis projection succeeds. Assessment closure aggregates every result-bearing LiveQuiz CODE submission, including receipts still waiting for that projection, so a transient Redis outage cannot erase durable grading results. The claim/status transition remains the idempotency authority; a duplicate, overlapping, or stale worker cannot finalize after losing its claim. Completion is published through Redis-backed GraphQL pub/sub; participant-owned polling remains the durable fallback if an event is missed, with an explicit retry action when polling itself fails.

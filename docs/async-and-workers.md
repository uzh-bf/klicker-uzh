...
Before enqueue acknowledgement, the response API locks the `LiveQuiz` row, rechecks that it is still published in correlated mode, and creates a `LiveQuizPendingResponse` outbox row keyed by the event message id. A unique response key makes this row the authoritative first-response gate even after the short Redis ingress lease expires. Its Hatchet payload, including an acceptance-time instance metadata snapshot, is encrypted at rest with an `APP_SECRET`-derived AES-GCM key. The snapshot keeps accepted responses processable after live Redis metadata expires. Quiz ending and export therefore serialize against ingress. A non-blocking single-flight dispatcher reserves due rows with `FOR UPDATE SKIP LOCKED` and republishes them with the stable message id until terminal worker settlement deletes the row. An ambiguous immediate Hatchet push is acknowledged as queued and left for that dispatcher rather than discarded.

The worker's correlated path validates the signed owner and event claim shape, serializes duplicate processing with a lease, writes the `LiveQuizResponse`, then applies all aggregate Redis hash mutations plus the processed marker in one Lua operation. It deliberately skips participant response hashes, leaderboard scores, and XP for correlated quizzes so those identity-bearing surfaces cannot be matched back to exported rows. Export holds the quiz lock and returns `LIVE_QUIZ_CORRELATED_EXPORT_NOT_READY` while outbox rows remain, so an acknowledged queued response cannot be silently omitted.

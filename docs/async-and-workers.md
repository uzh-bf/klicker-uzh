...
- `monitor-kb-graph-builds` runs every minute, reconciles graph workflow status, cancels timed-out runs, and accepts a late completion only when the pinned digest is still current and no newer build or retention cleanup has taken ownership.

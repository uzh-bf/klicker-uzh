...
- `monitor-kb-graph-builds` runs every minute, reconciles graph workflow status, cancels timed-out runs, and requires a versioned terminal-result callback before settlement or publication. Provider `COMPLETED` is not sufficient. A missing callback or malformed result clears the active slot without moving the published pointer and holds the reservation as `NEEDS_HUMAN_REVIEW`; a valid non-success result with metering settles actual usage without publishing, while a non-success result without metering releases only an ordinary `RESERVED` build. A late valid success callback may settle the existing held build once before cleanup, but a malformed or late failure result remains held and every callback still passes the same identity, artifact, currency, counter, and metering checks.

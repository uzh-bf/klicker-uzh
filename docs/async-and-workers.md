...
- `process-course-duplication` — async course duplication worker. The GraphQL mutation stores job state in Redis and returns a job id; it retries an ambiguous Hatchet event publication with the same job id so a lost acknowledgement cannot open a second copy. The worker uses a renewable, token-checked process lease, rethrows generic failures for Hatchet retries, and records only access or partial-copy failures as terminal. The manage frontend polls `courseDuplicationStatuses` until the job completes or fails.

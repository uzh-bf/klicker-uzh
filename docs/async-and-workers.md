...
Live Quiz Escape Room responses take a synchronous validation branch before the ordinary event path. Only a regular participant with an explicitly started attempt may answer. The response API applies the same exact-origin credentialed CORS policy as the ordinary path, verifies the instance-to-active-block binding, current stage, lockout, expiry, and shared lifecycle claim, then grades the answer and records cleared stages. Accepted events use deterministic message IDs; the response processor wraps their Redis mutations, pending-event removal, and completion marker in one deduplication transaction so concurrent delivery or retries cannot apply statistics twice.

Block closure first sets a Redis gate that rejects new Escape Room response slots. It drains in-flight grading and accepted pending worker events before `updateLiveQuizBlockResultsFromCache` snapshots the block. A timeout fails closure and reopens the gate, preserving the accepted response instead of silently closing with incomplete results.

...
Before enqueue acknowledgement, the response API locks the `LiveQuiz` row, rechecks that it is still published in correlated mode, and creates a `LiveQuizPendingResponse` outbox row keyed by the event message id. Its Hatchet payload is encrypted at rest with an `APP_SECRET`-derived AES-GCM key. Quiz ending and export therefore serialize against ingress. An in-process dispatcher reserves due rows with `FOR UPDATE SKIP LOCKED` and republishes them with the stable message id until terminal worker settlement deletes the row. An ambiguous immediate Hatchet push is acknowledged as queued and left for that dispatcher rather than discarded.

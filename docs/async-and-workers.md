...
- `process-course-deletion` — event-driven permanent deletion of an accepted course request. The course marker is the durable visibility boundary; a task-local concurrency bucket serializes deletion processing, retries ordinary failures, and rechecks the request token, requester permission, and published-live-quiz safety condition inside the deletion transaction. Requests remain hidden after exhausted retries and are republished by the sweep.
- `sweep-course-deletions` — cron task (every 5 minutes) republishing the oldest pending deletion markers in a bounded batch and warning for requests pending longer than 75 minutes.
...
remain unprocessed; the marker must only be cleared or permanently deleted by
the feature-aware worker.

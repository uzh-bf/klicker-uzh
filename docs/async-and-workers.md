...
Bare `http.createServer`, two routes: `GET /healthz` and `POST /AddResponse`. Non-assessment responses (`handleAddResponse`) emit `response-received:authenticated|anonymous`. The assessment path (`handleAddAssessmentResponse`) verifies a JWT correlation key, dedupes via `hget` on the assessment Redis, then emits `response-received:assessment`; audit-log events (`create-audit-log-entry`) are emitted throughout. Live-quiz vs assessment behavior switches on the `ASSESSMENT_MODE` env var. Both handlers apply the participant account gate (`src/participantAdmission.ts`) before publishing any event, so an account that has not completed the current data-use disclosure can never have a response collected: assessment responses are always attributed and therefore always gated, while regular responses are gated only for a `participant_token` that verifies as a registered participant. Anonymous and temporary participants carry no account whose disclosure could apply, and an unverifiable cookie keeps its pre-existing anonymous attribution. The gate reads the shared backend database, which both response-api deployments receive through `DATABASE_URL`.

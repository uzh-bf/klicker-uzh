...
For `CORRELATED_EXPORT`, initialization is the only route that may mint an anonymous quiz-scoped token. The dedicated correlated submission endpoint validates the complete response against the acceptance-time metadata before acknowledging it and rejects free-text questions before identity admission or outbox creation. The locked transaction rechecks the published mode, active block execution, and current quiz PIN, then stores the accepted identity, response key, encrypted snapshot, and `LiveQuizPendingResponse` row. Hatchet receives only `{ messageId }`; the worker loads that row and retries until terminal handling settles the receipt and erases the ciphertext. After the quiz ends, the general-worker reconciliation path finalizes only generations whose receipts are fully settled, assigning stable respondent labels and removing active identity bindings. The same minute-level task expires finalized respondent datasets after 90 days in bounded batches, cascading responses and correction rows while leaving rows with a binding or pending receipt for a later retry. Keep `LIVE_QUIZ_CORRELATED_RESPONSES_ENABLED` disabled until the response API, worker, finalization lifecycle, and migrations are deployed together.
...
- `reconcile-live-quiz-publications` — every minute, repairs published quizzes whose Redis publication metadata or scheduled task cleanup is incomplete, retries settlement-gated correlated finalization, and expires finalized correlated datasets after 90 days
...
Publication is deliberately two-phase: the database transaction records the published generation first, then the backend materializes the matching Redis metadata and marks it complete. The reconciliation cron retries incomplete materialization and removes stale scheduled-publication tasks idempotently. It also retries correlated finalization after `ENDED`; incomplete or retryable receipts leave the bindings, encrypted receipt data, and export salt in place. After finalization, it deletes each generation's respondent, response, and correction rows once `finalizedAt` is at least 90 days old and no binding or pending receipt remains. A persisted monotonic generation counter, rather than timestamp precision alone, identifies each publication. Aborting a quiz records a Redis generation tombstone and clears only the aborted generation, so a delayed older publication cannot recreate stale metadata and an immediate republish cannot be rejected merely because it shares the same millisecond timestamp.

...
`packages/kb-management/src/components/KnowledgeBaseChatbotBindings.tsx:KnowledgeBaseChatbotBindings` owns the single-enabled-KB binding UI. Replacing an existing chatbot binding requires an explicit warning step; detach is available from the current KB. `apps/frontend-manage/src/components/resources/chatbots/ChatbotDetails.tsx:ChatbotDetails` shows the reciprocal linked-KB state or an actionable no-KB warning, and `ChatbotKnowledgeBaseSetup.tsx` connects, replaces, disconnects, or creates and connects a KB from the chatbot side. Bindings are not part of the chatbot revision, so a change applies to a published chatbot immediately; the setup shows that note for published chatbots. After creating a KB there, the lecturer lands on its detail page with `?chatbotId=`, which renders a back link only when the id matches one of the lecturer's own chatbots.

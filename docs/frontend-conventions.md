...
- **Rich content persists as Markdown.** Expose only `ContentInput` behavior that survives Markdown serialization and the shared `Markdown` preview renderer. Tables are pipe-table compatible: insert, add/delete rows or columns, and delete the table; merged cells and persisted column widths are intentionally unavailable. Code fences retain their language tag; the editor registers Lowlight's common languages while the preview keeps a safe Prism superset. Pages-router initialization sets `immediatelyRender: false`, and programmatic content or disabled-state synchronization must not emit a Formik update. (`apps/frontend-manage/src/components/common/ContentInput.tsx:ContentInput`, `packages/markdown/src/Markdown.tsx:Markdown`)
- **Clipboard input stays within the Markdown storage boundary.** `ContentInput` treats plain clipboard text containing Markdown link syntax as Markdown so supported `[video](...)` and `[embed](...)` links keep their preview behavior. Ordinary plain text and rich HTML keep Tiptap's default paste path, except that pasted merged cells expand into explicit unit cells before parsing because GFM tables cannot persist row or column spans. (`apps/frontend-manage/src/components/common/ContentInput.tsx:ContentInput`)

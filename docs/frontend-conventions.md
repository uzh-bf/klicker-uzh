...
- A non-modal assistant dock must use a labelled complementary region, omit modal semantics and focus trapping, and leave the page root interactive and exposed to assistive technology. Connect its launcher with `aria-controls` and `aria-expanded`, keep Escape and the close control available, and restore focus to the launcher on close. `apps/frontend-manage/src/components/assistant/ManageAssistantWidget.tsx` follows this pattern so lecturers can keep working in Manage beside the dock.

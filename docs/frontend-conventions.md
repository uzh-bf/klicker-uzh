...
`apps/frontend-manage/src/components/common/Header.tsx:Header` shows Knowledge Bases and Chatbots inside one top-level AI dropdown, placed between Resources and Analytics and visible only when the client gate `useAiFeaturesEnabled()` is open (GrowthBook `ai-beta` AND the account's `aiFeaturesEnabled`). Both menu items carry the localized Beta features badge. The Resources dropdown no longer contains those routes, and its active state excludes them; the AI dropdown owns them. That client gate is discoverability only: the AI routes render a localized unavailable state when the gate is closed, while GraphQL independently applies the same two-condition gate and returns `AI_BETA_ACCESS_REQUIRED` for denied management calls.

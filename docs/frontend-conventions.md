...
- Keep the closed Manage assistant launcher centered above the non-interactive footer rather than over page-level actions. Clamp both stored and default desktop dimensions to the current viewport, and treat browser storage as an optional enhancement so privacy settings cannot prevent the dock from opening.

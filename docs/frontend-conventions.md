...
`apps/frontend-manage/src/components/common/Header.tsx:Header` shows Knowledge Bases and Chatbots inside one top-level AI dropdown between Resources and Analytics. Both menu items carry the localized Beta features badge. The Resources dropdown excludes those routes and their active states. The complete client and server gate contract lives in [Feature Flags](./feature-flags.md#active-flags).

...
- Keep the element-creation action in the creation strip outside activity editing. While an activity wizard is open, move the same secondary action into the centered slot between Cancel or Back and the primary, next-step-specific Continue action; explain the temporary detour with a concise hover/focus tooltip instead of persistent text, and do not duplicate the strip above the wizard. Opening and closing the element modal must preserve the active wizard (`apps/frontend-manage/src/pages/index.tsx:Index`, `apps/frontend-manage/src/components/activities/creation/WizardNavigation.tsx:WizardNavigation`).

...
- Generated-element review is an inbox, not a second element editor. Render drafts as a compact semantic table with explicit open, kept, and discarded states; resolve the pinned original source title and kind from the build and pair it with citation page ranges rather than showing normalized `.md` artifact names. Render a stored source URL as a link only when it parses to an HTTP or HTTPS URL; otherwise keep the source title as plain text. Open one draft in the canonical `ElementEditForm`, default its status to Review, and make Keep an atomic persistence action. Discarded drafts stay visible and restorable, while kept rows link to the ordinary Element editor. Keep the old batch save control only when an accepted-but-unsaved legacy draft requires recovery.
- Custom modal drawers must use a neutral element with `role="dialog"` and `aria-modal="true"`, connect the launcher with `aria-controls`, `aria-expanded`, and `aria-haspopup="dialog"`, trap keyboard focus, and restore focus on close. When the drawer lives beside the page root, portal it to `document.body` and make the page root `inert` plus `aria-hidden` while open; restore the exact previous state on cleanup. The participant course-chat drawer implements this contract in `apps/frontend-pwa/src/components/chatbot/CourseChatDrawer.tsx`.
- A non-modal assistant dock must use a labelled complementary region, omit modal semantics and focus trapping, and leave the page root interactive and exposed to assistive technology. Connect its launcher with `aria-controls` and `aria-expanded`, move focus into the dock when it opens, keep Escape and the close control available across embedded origins, and restore focus to the launcher on close. Below the desktop breakpoint, the dock becomes a full-viewport modal sheet: trap focus and mark only the app-content sibling inert and `aria-hidden`, then restore its exact state on close or when the breakpoint changes. Persist desktop dimensions only from the desktop layout so mobile clamping does not overwrite the lecturer's preferred size. `apps/frontend-manage/src/components/assistant/ManageAssistantWidget.tsx` follows this pattern so lecturers can keep working in Manage beside the dock.
- For a cross-origin assistant iframe, a validated child handshake is the readiness signal; `load` alone is not sufficient. After a bounded deadline, say that loading is taking longer rather than claiming failure, keep the current frame alive for a late handshake, and offer a remount retry. Reserve failure copy for an actual iframe error. Every recovery state keeps close and Escape available and labels a standalone fallback as a new conversation without the current page context.
- Keep the closed Manage assistant launcher as a compact icon-only control in the bottom-right viewport corner without reserving global Layout padding. List pages with controls at the bottom use local collision clearance: leave horizontal space beside the launcher on desktop and only the necessary bottom space on compact layouts. Clamp both stored and default desktop dimensions to the current viewport, and treat browser storage as an optional enhancement so privacy settings cannot prevent the dock from opening (`apps/frontend-manage/src/components/Layout.tsx:Layout`, `apps/frontend-manage/src/pages/index.tsx:Index`, `apps/frontend-manage/src/pages/activities.tsx:Activities`).
- Manage assistant actions that cross the Chat iframe boundary must use the validated parent origin and identity-only payloads. The parent owns same-origin editor navigation, closes the dock before routing, and validates the positive integer element id; standalone Chat does not expose the Manage-only action.
...
`apps/frontend-manage/src/components/common/Header.tsx:Header` shows Knowledge Bases and Chatbots in Resources, with their existing capability gates and localized Beta features badges. Generate elements is an action in the element library. Chatbot authoring additionally requires Catalyst entitlement and full-access or account-owner scope; local seed setup grants these capabilities to the test lecturer. The complete client and server gate contract lives in [Feature Flags](./feature-flags.md#active-flags).
...
The detail resource workspace presents that loaded window as a semantic design-system table with source, material category, operation, serving, update, selection, and contextual action metadata. Use one `+ Add resource` action that opens the chooser for Website or Document; keep Video visibly unavailable until its ingestion contract exists. Keep destructive resource deletion in the row overflow menu, and preserve the explicit inspector Ingest action for resources that are only `ADDED`. When a file upload has requested its ticket, keep the chooser open and non-dismissible until upload confirmation or a terminal failure.

File rows expose `Replace file` in their overflow menu; URL rows do not. The replacement modal first selects one supported file and then requires the explicit `Replace and ingest` action. Confirmation keeps the resource id, title, and material category, immediately queues the new canonical source, and leaves the previous AI-serving version available until success. Explain before confirmation that the prior source file is deleted best-effort and cannot be restored; after a queue failure, refresh the row so the lecturer can use its ordinary retry action without uploading again.

Material category is a controlled single-select with `Unclassified`, `Course content`, and `Administrative`. Creation forms submit the selected value, while the inspector can update an existing resource without changing its source or ingestion state. The category filter is server-backed and does not change the complete-KB ingestion summary. That summary reports resources needing ingestion, failed resources, and in-progress resources across the whole KB, even when the table is paginated or filtered. `Ingest all` opens one confirmation, asks the server to reconcile the complete KB, and reports queued, retried-failed, already-current, already-in-progress, and queue-failure counts; it never refreshes an already-current URL and never fabricates progress percentages.
...
`packages/kb-management/src/components/KnowledgeBaseChatbotBindings.tsx:KnowledgeBaseChatbotBindings` owns the single-enabled-KB binding UI. Replacing an existing chatbot binding requires an explicit warning step; detach is available from the current KB. `apps/frontend-manage/src/components/resources/chatbots/ChatbotDetails.tsx:ChatbotDetails` shows the reciprocal linked-KB state or an actionable no-KB warning, and `ChatbotKnowledgeBaseSetup.tsx` connects, replaces, disconnects, or creates and connects a KB from the chatbot side. Bindings are not part of the chatbot revision, so a change applies to a published chatbot immediately; the chatbot setup and the KB bindings panel both warn about that for published chatbots. The chatbot list and the KB list summarize material readiness through `KBMaterialsReadiness`: a resource counts as available while it has a serving revision, even when a newer revision is processing or has failed. That summary covers retrieval only and says nothing about question generation. A KB created there is proposed in the chatbot course's language (`en` English, `de` German) when the suggested subject serves it. After creating a KB there, the lecturer lands on its detail page with `?chatbotId=`, which renders a back link only when the id matches one of the lecturer's own chatbots; if the connection fails, the created KB stays selected with a Retry connection action instead of prompting a second creation. In the other direction, the KB bindings link to `/resources/chatbots?createForKb=`, which opens chatbot creation only after `getKb` confirms the lecturer owns that KB, requires an explicit course choice when several courses are eligible, and connects the KB to the new chatbot.
...
- **Feature flags gate alone.** Don't combine a flag with data-dependent counts (`flag && count > 0`) — that creates chicken-and-egg visibility problems.
  ...
  - `@klicker-uzh/feature-flags` is the shared GrowthBook integration. The `ai-beta` and `learning-analytics` flags default to false. Manage leaves analytics controls visible but disabled when `learning-analytics` is off. Use stable internal actor IDs for targeting. Browser evaluation is presentation state, while a backend-enforced feature entitlement is an additional fail-closed condition that never replaces role, scope, or resource permissions. See [Feature Flags](./feature-flags.md), [ADR 0008](./adr/0008-use-growthbook-for-feature-flags.md), and [ADR 0038](./adr/0038-backend-enforced-feature-entitlements.md).

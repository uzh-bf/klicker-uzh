...
In the participant PWA, CODE stacks use the asynchronous receipt lifecycle instead of the synchronous stack-response mutation. Persist the submitted code and receipt id under a separate activity-and-stack-scoped local-storage key, include and verify the authenticated participant id before restoring either receipt or completion state, recover it after reload, subscribe with polling fallback while it is active, and write normal practice-quiz completion state only after the receipt reaches `COMPLETED`. Terminal receipts are monotonic: a stale active poll or subscription result must not regress `COMPLETED` or `FAILED`. A `FAILED` receipt must leave the editor enabled for a new attempt. CODE submission is enabled for practice quizzes; keep shared microlearning rendering read-only until its grading/evaluation slice is complete. Participant rendering may show public test inputs, expectations, and results only; hidden test metadata and execution output never cross the GraphQL boundary.

...
`packages/kb-management/src/components/KnowledgeBaseChatbotBindings.tsx:KnowledgeBaseChatbotBindings` owns the single-enabled-KB binding UI. Replacing an existing chatbot binding requires an explicit warning step; detach is available from the current KB. `apps/frontend-manage/src/components/resources/chatbots/ChatbotDetails.tsx:ChatbotDetails` shows the reciprocal linked-KB state or an actionable no-KB warning, and `ChatbotKnowledgeBaseSetup.tsx` connects, replaces, disconnects, or creates and connects a KB from the chatbot side. Bindings are not part of the chatbot revision, so a change applies to a published chatbot immediately; the setup shows that note for published chatbots. A KB created there is proposed in the chatbot course's language (`en` English, `de` German) when the suggested subject serves it. After creating a KB there, the lecturer lands on its detail page with `?chatbotId=`, which renders a back link only when the id matches one of the lecturer's own chatbots; if the connection fails, the created KB stays selected with a Retry connection action instead of prompting a second creation. In the other direction, the KB bindings link to `/resources/chatbots?createForKb=`, which opens chatbot creation only after `getKb` confirms the lecturer owns that KB, requires an explicit course choice when several courses are eligible, and connects the KB to the new chatbot.

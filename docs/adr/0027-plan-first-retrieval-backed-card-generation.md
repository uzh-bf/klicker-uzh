...
2. `generate_cards` performs, per planned card, one retrieval call and one
   ...
   that retrieval. Grounding is enforced by the pipeline, not by the prompt.
   Cards stream as preliminary results; the final result carries card-local
   sources in the retrieval result shape.

...
| Student-initiated practice candidates | **Working simple version**                   | Inside the tutoring conversation, the chat route drafts flashcards (later questions) as candidate elements, each grounded by its own retrieval call and saved only by the student into their personal elements (ADR 0027). Needs a retrieval server; without one the feature is absent. KG-grounded generation is the Catalyst upgrade behind the same contract |

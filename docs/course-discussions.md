...
The Playwright journeys cover course integration, evaluated practice scopes, embeds, and rollout gates (`playwright/tests/Y-course-qa-course.spec.ts:Course Q&A course-level workflows`, `playwright/tests/Y-course-qa-practice.spec.ts:Course Q&A practice workflows`, `playwright/tests/Y-course-qa-embed.spec.ts:Course Q&A embed workflow`, `playwright/tests/Y-course-qa-rollout-gates.spec.ts:Course Q&A rollout-gate workflow`). New end-to-end scenarios should follow the Playwright-first policy in [Testing](./testing.md).

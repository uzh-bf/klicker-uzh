...
![Audience View Interaction](/img/audience_view_desktop.png)
...
3. **Lecturer Cockpit:** In the described case, whenever a lecturer is supported by an assistant, it might be beneficial to reduce the workload on the teacher by only forwarding selected student inputs to him. The Lecturer Cockpit (shown at the bottom of this page), which can be reached via the highlighted button, provides the lecturer on his/her device only with the questions that have been pinned by a moderator. However, it is important to notice that moderator and lecturer both have to be logged in with the same account in order to access these teacher-related functionalities. In principle, this view is also available without enabled 'moderation' and can be used by the teacher to organize questions or show a selection of inputs to the audience via projection or streaming (e.g. also during hybrid lectures).
...
9. **Confusion Barometer:**: The confusion barometer displays a real-time aggregate of feedbacks provided by students. The average is computed over all feedbacks from the past 10 minutes regarding speed and difficulty of lecture contents. To showcase the reliability of the measures, the number of feedbacks in the aggregate are also displayed.
...
Finally, the screenshot below shows the referenced lecturer cockpit, which can be used if only the results of the confusion barometer and pinned questions together with their upvotes shall be shown on a screen (e.g. on a second screen of the lecturer). To allow for a quick grasp of the situation, the colored bar will turn red if one of the metrics is in a red zone, or yellow if one of the metrics is in an orange zone.

![Lecturer Cockpit](/img/lecturer_cockpit.png)

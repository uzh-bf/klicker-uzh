...
- Correlated-response changes run the response API, response worker, export, `liveQuizResponseCollection.test.ts`, database-backed `liveQuizResponseCollectionMode.test.ts`, `liveQuizAbort.test.ts`, `liveQuizPublication.test.ts`, and `correlatedLiveQuizResponseExport.test.ts` suites together. Cover the discriminated acceptance metadata contract, authored response-shape bounds, atomic active-generation and PIN admission, normal-end settlement, direct-insert `P2002` same-event recovery, the Redis mutation ceiling, and export blocking on unsettled receipts. Abort must prove both transactional dataset deletion and preservation of a newer Redis publication generation. Publication changes require a clean migration run, including successful `CREATE UNIQUE INDEX CONCURRENTLY`, and a disabled/enabled capability-gate case.

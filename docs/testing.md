...
- Correlated-response changes run the response API, response worker, export, `liveQuizResponseCollection.test.ts`, database-backed `liveQuizResponseCollectionMode.test.ts`, `liveQuizAbort.test.ts`, `liveQuizPublication.test.ts`, and `correlatedLiveQuizResponseExport.test.ts` suites together. Cover authored response-shape bounds, atomic active-generation admission, normal-end settlement, same-event uniqueness recovery, the Redis mutation ceiling, and export blocking on unsettled receipts. Abort must prove both transactional dataset deletion and preservation of a newer Redis publication generation. Publication changes require a clean migration run and a disabled/enabled capability-gate case.

...
**Git hooks run no application test suites** (pre-commit = `check:all`, pre-push = `build`). The pre-commit check does run fast compiler-generation invariants such as the Prisma namespace patch tests. The expectation before a PR: `check:all` + build + targeted vitest for touched logic + browser evidence for UI changes; CI is the real e2e gate.

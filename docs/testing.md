...
- The Chat-local docs search tool is covered in `apps/chat/test/docs-search.test.ts`: deterministic ranking with stable route tie-breaks, exact/closest/ambiguous/no-result honesty, query and output bounds, authoritative source URLs and media, sentinel fencing of results with forged-fence neutralization, the reserved-name collision assertion against lecturer MCP tools, and a bundled-manifest sanity check against the generated artifact.

...
| Assessment response acceptance and correction races                  | GraphQL service tests plus the response processor against the same Postgres/Redis/Hatchet stack | run the focused correction tests and `pnpm --filter @klicker-uzh/hatchet-worker-response-processor test:run`        |

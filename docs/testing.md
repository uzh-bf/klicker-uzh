...
- Correlated-response changes run the response API, response worker, export, `liveQuizResponseCollection.test.ts`, database-backed `liveQuizResponseCollectionMode.test.ts`, `liveQuizAbort.test.ts`, `liveQuizPublication.test.ts`, and `correlatedLiveQuizResponseExport.test.ts` suites together. The abort case must prove that a published correlated quiz removes its temporary respondents, responses, receipts, and labels transactionally. Publication changes also require a clean migration run because the durable reconciliation marker and retry timestamp are part of the recovery contract.

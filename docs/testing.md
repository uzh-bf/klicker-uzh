...
- Correlated export tests cover stable label reuse, free-text exclusion, permission/state readiness, unsettled-response blocking, identity stripping, formula-safe CSV encoding, combined CSV header/response and respondent-by-column matrix bounds before row materialization, and fail-closed size limits. The correlated abort integration test also verifies that stable export labels are removed with the temporary response dataset. The GraphQL package imports the export build, so `.github/workflows/test-graphql.yml` builds `packages/export` before `packages/graphql`.

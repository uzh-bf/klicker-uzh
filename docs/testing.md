...
| Live Quiz response mode or evaluation export UI                      | Playwright e2e plus a real browser pass                                                         | verify mode selection/locking, the student privacy notice, cookie-blocked respondent-token fallback, and the ended-quiz export action     |
...
- Correlated export tests cover persisted finalized labels without lazy writes, free-text exclusion, permission/state readiness, unsettled-response blocking, invalid-owner rejection, formula-safe CSV encoding, combined CSV header/response and respondent-by-column matrix bounds before row materialization, and fail-closed size limits. The correlated abort integration test verifies removal of the temporary response dataset, while labels remain on the retained respondent rows until finite-retention deletion. The GraphQL package imports the export build, so `.github/workflows/test-graphql.yml` builds `packages/export` before `packages/graphql`.

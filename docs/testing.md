...
- Correlated export tests cover stable label reuse, free-text exclusion, permission/state readiness, unsettled-response blocking, identity stripping, formula-safe CSV encoding, CSV header and respondent-by-column matrix bounds before row materialization, and fail-closed size limits. The GraphQL package imports the export build, so `.github/workflows/test-graphql.yml` builds `packages/export` before `packages/graphql`.

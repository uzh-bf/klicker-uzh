...
**Superseded on 2026-09-17.** A cited source card no longer shows the retrieved envelope. `extractCitedPages` (`apps/chat/src/lib/markdown/remarkCitationMarkers.ts`) reads the labelled page detail the answer itself carries, and `formatCitedPageRanges` (`apps/chat/src/lib/sources/sourceDisplay.ts`) renders only the smallest ranges covering exactly those pages: an answer citing pages 6 and 7 shows `S. 6–7`, not the `2–95` span of every chunk retrieval happened to return. `getPageEnvelope` keeps its derivation and stays the fallback line for a source the answer cites without a page detail; `page`, `pageEnd` and the `#page=` anchor still use the lowest retrieved page. See [Chat Platform](../docs/chat-platform.md). The envelope semantics above are kept for the record.

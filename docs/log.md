...
- **Update**: [domain-model](./domain-model.md), [data-and-migrations](./data-and-migrations.md), [graphql-api-layer](./graphql-api-layer.md), and [frontend-conventions](./frontend-conventions.md) document the standalone QR Scan authoring/print foundation, exact-owner answer protection, fail-closed activity placement boundary, CSPRNG print ordering, and isolated replay requirements for its concurrent unique index. The matching data-model, GraphQL, frontend, and verification procedures were updated in the same change.

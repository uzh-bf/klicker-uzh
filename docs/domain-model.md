...
LiveQuiz Escape Rooms are scoped to an `ElementBlock`. Temporary participants are ineligible; a regular participant must explicitly start the attempt through GraphQL before response-api accepts an answer. The attempt remains in progress until every answerable instance in the block has been cleared, with attempt-scoped progress discarded naturally when a lecturer reset creates a new attempt identity.

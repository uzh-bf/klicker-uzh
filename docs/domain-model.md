...
Uploaded file resources can be replaced in place from the resource workspace: `replaceKbResourceFile` consumes a fresh upload ticket under the KB lock, swaps the blob identity, resets the resource to `ADDED` with its stored attempt cleared, and preserves the material category, resource id, and version for the next `ingestKbResource` dispatch. The previous blob is deleted best-effort after the transaction commits; a deletion failure only orphans the old blob and never fails the confirmed replacement. The previously active serving version remains available until the replacement's own ingestion succeeds.

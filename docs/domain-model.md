...
The resource workspace's `ingestAllKbResources` mutation reconciles the complete owned KB, not the currently loaded table page or its filters. It claims only resources whose latest lecturer revision is not the active serving revision, skips current or already active resources, and skips in-flight attempts. Claims are made under the KB lock with conditional resource updates, then dispatched through the existing ingestion workflow in bounded batches after the transaction commits. A queue failure is recorded only for the still-current attempt, so repeated clicks and concurrent per-resource actions converge without creating duplicate external operations.

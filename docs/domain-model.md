...
- `Achievement` (`gamification.prisma`) has `type` PARTICIPANT/GROUP/CLASS and `scope` GLOBAL/COURSE, with per-subject instance models; `Level` defines XP thresholds as a linked list; `Title` and `AwardEntry` complete the set.
  Six achievements currently have an earnable award path: live quiz podium places (Champion, Vice-Champion, Vice-Vice-Champion, awarded in `liveQuizzes.ts` to the top three finishers) plus Dream Team and Team Spirit (awarded during group activity grading in `groups.ts`). Explorer is additionally granted manually by staff. All other seeded achievements are marked `isDiscoverable=false` so students never see entries they cannot earn; new award implementations flip the flag at seed time via `ACHIEVEMENT_AWARD_PATHS`.

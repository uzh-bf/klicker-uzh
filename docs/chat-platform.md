...
**Two GrowthBook flags gate the assistant, and both are enforced server side.** `manage-assistant` covers the surface as a whole: the launcher in `apps/frontend-manage` (`src/components/Layout.tsx`, `src/components/assistant/ManageAssistantWidget.tsx`), the `/manage` page in `apps/chat`, and `POST /api/manage/chat`. `manage-assistant-mcp-tools` separately covers whether that assistant receives the lecturer MCP tools, and therefore whether `POST /api/manage/proposals/confirm` will redeem a proposal token — a token minted while the flag was on stops being redeemable the moment it is turned off. The API routes evaluate their flag per request through `src/lib/server/featureFlags.ts`, so hiding the launcher is not what protects them.

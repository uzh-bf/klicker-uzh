...
degraded no-save limits. Once the preflight settles, explicit no-save authoring
remains available in degraded states, and the
...
Documentation help is a Chat-local composite tool, not a lecturer MCP tool.
`klicker_docs_doc_query` is reserved by the chat route and merged into the
request's tool set after asserting the reserved name does not collide with any
lecturer tool. When `MCP_KLICKER_PUBLIC_DOCS_URL` and
`DOC_QUERY_JWT_TOKEN_KLICKER_PUBLIC_DOCS` are configured, its first call in a
request opens the dedicated multi-tenant Doc Query path, verifies that the
remote inventory contains only the retrieval tool and the runtime's optional
same-tenant `_chunk_topics` companion, and invokes only the retrieval tool.
Initialization, inventory, and retrieval share a four-second deadline. Any
configuration, initialization, call, timeout, malformed-output, non-canonical
source, or empty-result failure disables the remote client for that request and
uses the release-bundled fallback instead. The remote client and lecturer
client have independent idempotent teardown on finish, error, and abort.

The fallback runs the existing deterministic keyword search over the checked-in
docs manifest (`apps/docs/src/generated/docs-manifest.json`, generated by
`apps/docs` from the current v3 public docs and the use-case catalogue). It
requires every query term to match, ranks title over tags, headings, and
summary, breaks ties by route, and reports whether the outcome is a single
match, closest match, ambiguity, or no result. It emits the same documents-mode
shape as the remote path, including canonical source URLs and an explicit
`bundled_release_snapshot` provenance marker. Query length, result count,
per-result context, and total output remain bounded. Both paths return through
the same per-request data fence as lecturer MCP output, while source
normalization removes only a valid outer fence before parsing. Documentation
help therefore stays available when either remote Doc Query or lecturer MCP is
degraded, and no assistant turn fetches the website directly.

...
Documentation help is a Chat-local tool, not a lecturer MCP tool:
`klicker_docs_search` is reserved by the chat route and merged into the
request's tool set after asserting the reserved name does not collide with any
lecturer tool. It runs a deterministic keyword search over the checked-in docs
manifest (`apps/docs/src/generated/docs-manifest.json`, generated by
`apps/docs` from the current v3 public docs and the use-case catalogue) that is
bundled with the chat release at build time — it never fetches live
documentation, and its answers say so when freshness could matter. The search
requires every query term to match, ranks title over tags, headings, and
summary, breaks ties by route, and reports honestly whether the outcome is a
single match, the closest match, several equally good matches, or no result.
Query length, result count, per-result context, and total output are bounded.
Results return through the same per-request data fence as lecturer MCP output,
so text stored in the docs stays evidence and can never grant tool authority.
Because the tool is Chat-local, it stays available even when lecturer MCP is

...
Escape Rooms expose `startEscapeRoomAttempt`, `requestEscapeRoomHint`, `resetEscapeRoomAttempt`, `escapeRoomProgress`, and attempt/config fields on Practice Quiz, Microlearning, and Group Activity data. Start and hint operations accept exactly one activity ID. Group resets additionally require the server-returned `groupId`; progress returns one row per course group, including `NOT_STARTED`. The Prisma model retains a dormant Live Quiz block reference, but that mode has no GraphQL entry point yet.
...
Group Activity submission has a separate atomic path because one group answers one stack together. `services/groupEscapeRoomSubmissions.ts:submitEscapeRoomGroupActivityDecisions` rechecks the activity instance, group membership, schedule, active attempt, lockout, exact answer set, and sample solutions inside a serializable transaction. It updates aggregate instance results and either completion or lockout together. Group membership and the lifecycle actor are resolved from the authenticated participant and course; client-provided progression state is never trusted.

...
- Validate element-type placement in the service after resolving new, retained, and duplicated instances, not only in the authoring UI. QR Scan is accepted only when a Practice Quiz, Microlearning, or Group Activity is configured as an Escape Room; Live Quiz and live-quiz-template inputs still reject it with `BAD_USER_INPUT`. Group Escape Room authoring additionally rejects an empty answer set or any answerable non-QR instance without a sample solution before writing.

Escape Rooms expose `startEscapeRoomAttempt`, `requestEscapeRoomHint`, `resetEscapeRoomAttempt`, `escapeRoomProgress`, and attempt/config fields on Practice Quiz, Microlearning, and routed Group Activity details. Start and hint operations accept exactly one activity ID; Group Activity calls additionally require the routed `groupId`, which the server verifies against the activity course and authenticated membership. Group resets likewise require the server-returned `groupId`; progress returns one row per course group, including `NOT_STARTED`. The Prisma model retains a dormant Live Quiz block reference, but that mode has no GraphQL entry point yet.

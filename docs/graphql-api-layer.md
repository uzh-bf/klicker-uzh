...
Direct media upload is a two-operation authenticated contract owned by `packages/graphql/src/schema/mediaUpload.ts`. `getFileUploadSas` creates the owner-bound `MediaFile` and upload target with create-only blob permission; the still-live SAS therefore cannot overwrite bytes after classification. After Azure accepts the browser upload, full-access mutation `finalizeFileUpload(mediaFileId: ID!): Boolean!` delegates to `packages/graphql/src/services/importExportFingerprints.ts:finalizeUploadedMediaFingerprintV1`. The service rechecks the current user as owner and classifies the canonical stored bytes before returning `true`; a missing/unowned record or a blob visibility race cannot authorize the href for editor use. Classification and the owner-scoped invalidation of any already-authored null-fingerprint element that references the href share one transaction. Invalidation increments the element version and writes the database-valid `NULL` fingerprint/version retry marker, so a concurrent pre-finalization snapshot loses its conditional write. Finalization is idempotent: Manage retries the same `mediaFileId` through a bounded three-attempt policy, and an already-classified retry skips both the download and the version bump. When import/export is enabled, the generic `userMediaFiles` query delegates to `packages/graphql/src/services/elements.ts:getUserMediaFiles` and returns only version-1 classified rows; the disabled gate preserves legacy visibility until the rollout media backfill is complete. Keep finalization server-side—browser MIME metadata is not the import/export media identity.

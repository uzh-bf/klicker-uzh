...
New Manage clients use the additive `assessmentParticipantInvitationPage` field with finite `numEntries` and `offset` arguments. The service clamps page sizes to 50 and orders by `invitedAt DESC, id DESC`, returning both the page and `totalCount`. Assessment invitation imports are limited to 200 rows at the GraphQL service boundary. The original list field and persisted operation remain unchanged during the rolling-client window; retire that unbounded compatibility path only after deployed clients have migrated.

...
- Capability-gated reads (precedent: `getChatAccountUsage` with the `ai-beta` flag) keep authorization first, then await the fail-closed helper `lib/featureFlags.ts:isFeatureFlagEnabled` before any domain data query, and return `null` when the flag is unavailable or false. The helper reads the authenticated account's beta preference with request-local reuse; opt-out overrides even a forced-on rollout. Visibility gates hide results; they never replace authorization and never gate administrative mutations that must stay reachable.
...
Lecturer knowledge-base and generation service entry points start with `packages/graphql/src/lib/manageAiFeatureGate.ts:assertManageAiEnabled`. The gate first reads the current `User.aiFeaturesEnabled` entitlement and `User.betaEnabled` preference. If either is absent or false, it returns `AI_BETA_ACCESS_REQUIRED` without evaluating GrowthBook. Otherwise, it evaluates `ai-beta`: an explicit denial returns `AI_BETA_ACCESS_REQUIRED`, while an unusable decision returns `AI_FEATURE_TEMPORARILY_UNAVAILABLE`. Database access withdrawal therefore takes effect on the next request, including during a GrowthBook outage. Chatbot authoring instead requires the database beta preference, `ai-beta`, Catalyst, and an allowed login scope; publication and model usage additionally require `User.aiFeaturesEnabled`. Participant `courseChatbots` queries and worker-only knowledge-graph settlement remain outside this lecturer management gate. The separate `assertKbIngestionEnabled` admission evaluates the per-actor `kb-ingestion` GrowthBook flag at call time and blocks only upload-ticket issue, URL-resource creation, file replacement, and Ingest/Retry/Re-ingest with `KB_INGESTION_DISABLED`; reads, upload confirmation, deletion, and chatbot binding remain available while ingestion is disabled.

The graph lifecycle has two additional gates. The per-actor `kb-graph-builds` rollout flag blocks graph opt-in and rebuild mutations, while `KB.knowledgeGraphEnabled` is required before a build can reserve quota or be served to chatbot students. The worker rechecks both gates and a complete `RESERVED` cost ledger, including its linked quota identity, immediately before starting the external run; it claims `dispatchClaimedAt` before the provider call, and an accepted-but-uncorrelated run is held for review rather than externally retried. An unstarted gated build fails closed and releases its ordinary reservation, while an incomplete pre-accounting row is held for review. An accepted-but-uncorrelated build keeps the active KB build slot fenced, and a rebuild mutation refuses to start a second external run; that hold is a waiting state rather than an operator task, because `packages/hatchet/src/kbGraphIngestion.ts:resolveAmbiguousKBGraphDispatch` asks the provider again on every graph-monitor tick and either correlates the recovered run or, once the provider definitively reports no run for that build id, releases the reservation and frees the slot as an ordinary `KB_GRAPH_DISPATCH_FAILED`. `setKbKnowledgeGraphEnabled` validates the cost configuration before enabling a KB. `rebuildKbKnowledgeGraph` is a thin wrapper around `packages/graphql/src/services/knowledge.ts:startKbKnowledgeGraphBuild`, which also accepts a trusted `{ kind: 'system', ownerId }` trigger. That trigger evaluates the stored owner's entitlement and rollout without a session (`evaluateKBGraphSystemOwner`), always uses the Standard tier, returns `UNSUPPORTED_SETTINGS` without reserving when the stored subject and language do not resolve to an explicit selection, returns `NOT_DUE` without reserving when the published graph already matches, records `origin: SYSTEM` on the build and enqueues its dispatch at Hatchet `Priority.LOW`. Its only caller is the worker's scheduled sweep, `packages/graphql/src/services/knowledgeGraphPreparation.ts:sweepKbGraphPreparation`; no GraphQL field exposes it. Rebuild reserves the configured estimate in minor currency units under the owner-semester quota lock, and the external monitor never publishes from provider status alone: `settleKbKnowledgeGraphResult` accepts only a W1-versioned terminal result whose build, KB, owner, run, source digest, graph name, artifact, currency, bounded counters, and metering match the reservation. A valid success settles and publishes; a valid non-success result with metering settles actual usage without publishing; an unmetered non-success releases only an ordinary `RESERVED` build. A timed-out success may publish only after settlement atomically confirms no newer build and a matching current KB digest; stale or superseded late results settle usage without publication. Settlement is fenced by `KBGraphBuild.costStatus` and cleanup claims; invalid results become `NEEDS_HUMAN_REVIEW` and retain the reservation. The config query selects the newest graph attempt for lifecycle and cost fields, while it resolves `isStale` only from a verified successful published build, so a held or charged rebuild remains visible without changing the served pointer. The lecturer config reports persisted quota-currency/limit drift as unavailable and keeps historical build-cost currency separate from quota display.

The element-generation API exposes SC, MC, KPRIM, and flashcards as peer Klicker elements over one `ElementGenerationBuild` lifecycle. `elementGenerationCapabilities.configured` requires both the dedicated Hatchet/Blob runtime and complete fixed-price configuration. `elementGenerationSources` lists every KB the lecturer owns with its derived preparation state and, while one is eligible, a source basis: the published build id and a fingerprint over that build's preparation identity and whether recent changes are excluded from it. A basis stays eligible when sources were only added or updated, and is withdrawn after a settings change or when any snapshot source stops serving. Because the fingerprint covers the recent-changes flag, a start is rejected when the recent-changes notice the lecturer saw would now differ, but not on each further upload while that notice stays the same. `startElementGeneration` must return the KB id, build id and fingerprint; the server recomputes the basis and rejects any mismatch, including a foreign or missing KB, with `KB_GRAPH_BASIS_CHANGED` instead of substituting a newer graph. Initial generation and flashcard retries reserve the shared `KBGraphQuota` under its owner-semester lock, persist one `ElementGenerationSpend` per durable dispatch UUID, finish deterministic runtime validation, and only then claim immediately before the provider call. Acceptance or exact-run recovery settles the spend. Concurrent use of one client idempotency key returns the committed build without another reservation. Definite pre-claim failure releases atomically with terminal build state. An accepted-but-not-yet-visible attempt is never redispatched: its claim stays fenced for 15 minutes, after which an exact provider lookup may release it only when no matching run exists. Review and incomplete-publication events do not reserve cost. Provider success without the required output artifact is terminal instead of polled forever, and buffered artifact reads request at most 10 MiB plus one byte before rejecting oversized output.

`keepGeneratedElementDraft` is the canonical review persistence seam. It accepts the draft revision plus the visible Element editor payload, validates the owned terminal draft and unchanged type, then creates and links exactly one ordinary Element in the same transaction. The canonical form's choice IDs accompany assessment options so reorder, deletion, and replacement preserve generated-draft identity; ordinary Element options remain unchanged and do not persist those IDs. The selected Element status, tags, scoring, question options, and edited content are persisted together; Flashcard card type remains pinned to generation metadata. A network retry carrying the consumed revision returns the existing linked draft without creating a duplicate, while any other stale revision fails. Build reads expose pinned original source title, kind, and URL so clients can pair citation page ranges with the source lecturers recognize. `saveGeneratedElements` remains only as recovery for accepted drafts produced by the earlier two-step workflow.
...
The same parent lock serializes quota allocation. A KB permits 1000 retained-or-reserved resources and defaults to 500 MiB of storage, with an administrator-controlled `storageLimitMiB` override. Storage usage includes retained resources and reservations. Upload requests reserve their exact count and bytes, confirmation consumes the matching reservation, and URL creation reserves one resource plus the conservative 25 MiB unknown-size claim before the worker measures its exact size. Mutation failures use stable `KB_RESOURCE_LIMIT_REACHED`, `KB_STORAGE_LIMIT_REACHED`, and `KB_UPLOAD_TICKET_MISMATCH` codes. Klicker derives ingestion `kb_id` only from owner-checked persisted relations; platform-side validation against a registered per-project set remains a separate deployment gate.

KB metrics byte fields (`pendingCleanupSizeBytes`, `quotaSizeBytes`, `reservedSizeBytes`, `storageLimitBytes`, and `visibleSizeBytes`) use GraphQL `Float!` instead of `Int!` to support capacities above 2 GiB. This changes the public schema contract: clients with generated bindings or Int-specific scalar validation must regenerate or update them before adopting this schema. Resolvers return whole byte counts; the configured limit must remain a safe JavaScript integer. JavaScript clients continue to receive numeric values.

...
- Validate element-type placement in the service after resolving new, retained, and duplicated instances, not only in the authoring UI. QR Scan is accepted only inside Escape Room activities or Live Quiz Escape Room blocks; ordinary Live Quiz blocks and template inputs reject it with `BAD_USER_INPUT`. Group Escape Room authoring additionally rejects an empty answer set or any answerable non-QR instance without a sample solution before writing.

Escape Rooms expose `startEscapeRoomAttempt`, `requestEscapeRoomHint`, `resetEscapeRoomAttempt`, `escapeRoomProgress`, and attempt/config fields on Practice Quiz, Microlearning, routed Group Activity details, and Live Quiz blocks. Start and hint operations accept exactly one activity target. Group Activity calls additionally require the routed `groupId`, which the server verifies against the activity course and authenticated membership; Live Quiz calls use `elementBlockId` and require that block to be active. Progress binds both `liveQuizId` and `elementBlockId` before returning participant identities. Group resets require the server-returned `groupId`; Live Quiz resets require WRITE permission on the owning quiz.
...
Group Activity submission has a separate atomic path because one group answers one stack together. `services/groupEscapeRoomSubmissions.ts:submitEscapeRoomGroupActivityDecisions` rechecks the activity instance, routed group membership, schedule, active attempt, lockout, exact answer set, and sample solutions inside a serializable transaction. It updates aggregate instance results and either completion or lockout together. The routed group ID selects the shared resource; authenticated membership and the activity-course match authorize it. Client-provided progression state is never trusted.

Live Quiz answers cross the response API rather than a GraphQL submission mutation. `apps/response-api/src/escapeRoom.ts` synchronously validates regular-participant identity, quiz/block/instance binding, the active attempt, current stage, lockout, and expiry before grading or emitting a deterministic Hatchet event. The response processor deduplicates that event before updating ordinary Live Quiz statistics, and block closure drains accepted events before snapshotting results. GraphQL remains authoritative for explicit start, hints, current-stage payload masking, course-roster cockpit progress (including `NOT_STARTED` participants), and reset.

...
**Work back-to-front, and run codegen as part of every API step.** The canonical order below matches how shipped features actually land — worked example: commit `ff61d9bc7` (#4951, read-only feature) and its schema-changing sibling `38c92d035` (#4958, migration + mutations + heavy vitest).
...
5. **Ops + codegen**: add `Q*/M*/S*/F*.graphql`, run `pnpm --filter @klicker-uzh/graphql generate`, and commit the handwritten operations plus the tracked `public/schema.graphql` snapshot. The typed documents and persisted-query maps are regenerated by the package build and are not committed.
...
`packages/types` (new + renamed types) → `schema/assessment.ts` + `query.ts` (object type, two fields, one arg) → `services/courses.ts` (new service fn) → auth (`asUser` + `withPermission(...ADMIN)` lecturer-side) → 2 new + 2 extended `.graphql` ops → regenerated typed documents, persisted-query maps, and SDL snapshot → new manage page + component + gated nav button → +6/+6 lines de/en.

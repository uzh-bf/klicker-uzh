...
`preview`/`pilot` maturity labels, and the "Show me where" button — lives under
`shared.productUpdates` in `packages/i18n/messages/de.ts` and `en.ts`; each
application keeps only its own surface labels (the archive page title and link
and the spotlight popover buttons on manage, the menu label on the PWA) under
`manage.productUpdates` and `pwa.productUpdates`.

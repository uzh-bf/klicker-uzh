...
- **Offline qualification**: Public-safe synthetic fixtures and a dependency-free evaluator live under `.github/open-code-review/qualification/`. They cover blocker and false-blocker adjudication, prompt-injection-as-data handling, valid and invalid stack topology, path ownership, incomplete coverage, and token-counter consistency. Run `node --test .github/open-code-review/qualification/final-review-qualification.test.js` and `node .github/open-code-review/qualification/final-review-qualification.js`; this proves deterministic local contracts only and makes no OpenRouter call or live merge-readiness claim.

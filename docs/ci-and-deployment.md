...
- **Topology spread contract**: prd values define zone + hostname `topologySpreadConstraints` for the three Hatchet workers and the two assessment workloads (frontend + backend). The chart renders the value at each pod spec only when non-empty (chart defaults are `[]` and render nothing), and every constraint selector must match the workload's own `app.kubernetes.io/component` pod label — the assessment frontend selects `frontend-assessment`, not `frontend-pwa-assessment`. After changing scheduling values, verify the rendered output with `node deploy/scripts/verify-topology-spread.mjs`: it renders the prd chart, asserts that exactly the five intended Deployments carry their constraint lists at the pod-spec level with selectors matching the pod component labels, and that chart defaults render no spread field. A passing render is still source-level proof — live spread behavior must be confirmed against the synced cluster.

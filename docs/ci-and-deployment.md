...
- **Required status contexts**: Dedicated status checkers preserve stable required names. Playwright drafts skip the reusable execution workflow before preparation, builds or shards; `test-playwright-status` explicitly reports the draft skip. Ready PRs require successful execution, full mode and the complete eight-shard matrix. Unexpected skips, missing outputs, failures and cancellations without proven supersession fail. The route-neutral concurrency group on the reusable caller cancels its complete predecessor regardless of backend. Conversion back to draft or PR closure joins that same group through a checkout-free no-op job. The status reporter stays outside that group so an older reporter cannot prevent new execution. Drafts also skip queue telemetry. GraphQL retains its own path-filter and cancellation policy.

...
The standard response API imports the shared Prisma client for correlated-response admission and its encrypted delivery outbox. Its deployment currently receives `DATABASE_URL` from the backend GraphQL database secret; the assessment response API does not run this outbox. A dedicated database role limited to the live-quiz admission, respondent, response, and outbox tables remains required hardening before this credential boundary is considered least privilege. Roll out the matching Prisma migration before the response API and response-worker images, with an explicit lock budget for its non-concurrent `LiveQuizResponse` index. Keep `backendGraphql.correlatedResponsesEnabled` false for that first rollout; after every old PWA, response API, backend, and worker replica is gone, set it true in a second backend and general-worker configuration rollout. Do not rotate `APP_SECRET` while unsettled `LiveQuizPendingResponse` rows contain ciphertext; drain those deliveries first. Settled receipt rows have no ciphertext and do not block rotation.

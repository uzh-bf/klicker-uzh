...
- **Security analysis**: `codeql-analysis.yml` analyzes JavaScript/TypeScript, the analytics Python sources, and the GitHub Actions workflows with SHA-pinned CodeQL v4 actions, and runs the `security-extended` suite as a pilot; the job is not a required check. `v3_sonarcloud.yml` pins the Sonar scanner, derives the project version from the root `package.json`, waits for the quality gate, and imports LCOV coverage only from a test run that belongs to the analyzed head and recorded the same tested source tree (`.github/scripts/sonar-coverage-inputs.cjs`); a missing or unverified report leaves coverage "not computed" in SonarCloud instead of reporting a satisfied metric, and no coverage threshold is armed yet. Fork and Dependabot pull requests cannot receive `SONAR_TOKEN`, so their analysis fails closed with a named unavailable result rather than an authorization error. `dependency-review.yml` reviews changed dependencies and fails on high severity, but pnpm 11 dependency-graph support is unverified, so it is not yet complete CVE coverage. Trivy scanning is piloted on the staging backend-docker image and migrator by `v3_backend-docker-stg.yml`; promotion does not consume those receipts yet.

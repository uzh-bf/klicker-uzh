...
- **Public PR ARM64 runners**: The reusable `public-pr-playwright-shards.yml` workflow may run Playwright shards on the `public-pr-arm64` organization runner group only for non-draft, non-bot PRs whose head branch belongs to this repository. Fork PRs, pushes, and disabled rollouts retain the GitHub-hosted shard job. The reusable workflow repeats the eligibility check before requesting a self-hosted runner, uses read-only contents permission, does not persist checkout credentials, and receives no secrets. `PUBLIC_PR_ARM64_PLAYWRIGHT_CANARY_PR` enables one exact PR; `PUBLIC_PR_ARM64_PLAYWRIGHT_ENABLED=true` enables all eligible same-repository PRs. Do not set either variable until the reusable workflow exists on `v3` and the runner group allows only `uzh-bf/klicker-uzh/.github/workflows/public-pr-playwright-shards.yml@refs/heads/v3`. The three persistent public runners are disposable, have no private repository or private-network access, and require scheduled rebuilds plus immediate replacement after anomalies. Disk cleanup is not compromise recovery.
- **Organization ARM64 pool provisioning**: `util/provision-hetzner-arm64-runner.sh` provisions a fresh ARM64 VM as either `public-pr-arm64-01` through `-03` or `trusted-arm64-01` through `-02`. The matching runner groups must exist before provisioning. Both groups use selected repositories and selected reusable workflows only. The public group may select public repositories but never private ones, and its workflow allowlist must contain only the KlickerUZH reusable workflow at `refs/heads/v3`; every trusted workflow must belong to one of that group's selected private repositories. Pool assignment is immutable because a persistent runner may retain job data; rebuild the VM instead of moving it between groups. Public PR services are reachable only inside each job's Docker network, and each Playwright shard uses a run-specific Hatchet volume. The script enables UFW with deny-by-default inbound traffic and OpenSSH as the only inbound allowance. Threshold cleanup removes unused Docker volumes as well as old containers, images, and builder data. Use the local 80 GB disk initially and add a protected volume only after monitoring shows sustained disk pressure.

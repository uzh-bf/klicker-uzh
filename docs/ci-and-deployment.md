...
The standard response API imports the shared Prisma client for correlated-response admission and its encrypted delivery outbox. Its deployment receives `DATABASE_URL` explicitly from the backend GraphQL database secret; the assessment response API does not run this outbox. Roll out the matching Prisma migration before the response API and response-worker images. Do not rotate `APP_SECRET` while `LiveQuizPendingResponse` contains rows; drain the outbox first so queued ciphertext remains decryptable.

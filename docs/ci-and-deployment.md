...
- **CodeAPI integration**: the staging values target `http://codeapi-api-klicker-test.codeapi.svc.cluster.local:3112` and enable the narrowly scoped Kubernetes-service HTTP exception. The v2 chart emits the five `CODEAPI_JWT_*` signing settings only into the general grading-worker Secret; the v3 chart keeps those keys in its existing externally managed general-worker Secret. Provision the same issuer, audience, tenant, private key, and key ID that correspond to the CodeAPI verifier JWKS. No CodeAPI or Klicker cluster resource is changed by this repository configuration alone.

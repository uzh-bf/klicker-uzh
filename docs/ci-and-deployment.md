...
- **Playwright build handoff**: The single Playwright build job uploads the built GraphQL package, and each shard restores its generated client map into the source path used by the course-sharing specs. This keeps cached Turbo builds and uncached builds equivalent without regenerating the package eight times.

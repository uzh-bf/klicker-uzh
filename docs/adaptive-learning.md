...
Ordinary warnings such as coverage below an authoring target or the conservative time budget do not block publication. Four structural Research diagnostics do: unreachable leaf minimum evidence, minimum evidence blocked by a node cap, minimum evidence blocked by the global cap, and unreachable classification bands. They remain warnings so Research authors can inspect and edit the configuration, but `readiness.ready` stays false and the real-course publication transaction rejects the quiz until they are resolved. Readiness output is part of the owner preview, not transient UI-only text. Every issue carries a stable code and structured numeric/name parameters; Manage maps the allow-listed codes to English/German messages and uses backend prose only as a compatibility fallback.
...
- The existing element editor adds an optional Adaptive mapping section for `NUMERICAL`, `SC`, `MC`, `KPRIM`, and controlled-answer `FREE_TEXT`, including before the element's first save. Authors enable the section and choose one tree, leaf, and level; the primary action then states that it will create and assign the element. The element is persisted first, followed by one narrow assignment mutation after an element id exists, with durable retry or an explicit keep-unmapped recovery when the second write fails. Editing a saved element supports additional tree mappings. The UI shows breadcrumb-labelled leaves, level-derived `b`, fixed/default `a`, inferred `c`, and numerical percent-input behavior. Unsupported element types receive an explanation without assignment controls.

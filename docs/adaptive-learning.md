...
The participant result serializer returns level bands, confidence/near-boundary wording, a normalized overall trajectory, and a nested depth-5 profile. Lecturer cohort analytics include all completed participants without a minimum group size. The configured first/latest-completed-attempt policy selects one attempt per participant. Empty cohorts remain unavailable; in-progress and abandoned attempts are excluded.

Snapshot policy version 3 removes small-group suppression and five-person release boundaries. The aggregate JSON shape remains version 2. Authorized reads refresh aggregates after completed attempts or erasure; snapshot JSON contains only aggregates, without participant/attempt identifiers or raw answers. Canonical attempts are selected in PostgreSQL and aggregated in bounded batches. Existing lecturer access checks and participant ownership checks remain unchanged.

Pilot metrics use canonical response rows and available completion durations. Small categories and known/missing partitions are reportable. Genuinely missing measurements remain unavailable, and residual/misfit calculations still require 30 responses. These diagnostics do not recalibrate questions or rewrite student results. See [Adaptive Learning Operations](./adaptive-learning-operations.md).
...
- Participant and participation deletion deliberately retains the existing erasure cascade for that participant's attempts. The attempt-delete trigger invalidates every affected aggregate snapshot; a later authorized read recomputes all remaining completed participants. Invalidated snapshots are not returned and require an explicit retention decision before any export.
...
- Phase 12/13 browser evidence - **verified 2026-07-14** for the tree library/editor, element mapping, adaptive setup/settings, anonymous evaluation, and student level-band result at desktop, tablet, and mobile widths in English and German. Root document overflow is absent at 390/768/1440 px and the wide coverage matrix remains independently scrollable.
- `npx agent-browser` plus repository-pinned Playwright browser QA - **verified 2026-07-13** for intro, every supported question type, zero-answer resume, atomic restart, classified/insufficient completion, depth-5 expansion, 390 px English/German layouts, payload privacy, and the ready/init/state embed handshake.
- Phase 7 browser evidence - **verified 2026-07-13** for the live question timer, four-point student level-band result, and five-result released lecturer cohort. Six representative PR screenshots are retained in `project/screenshots/adaptive-learning-final/`; the automated browser suites remain the canonical evidence for omitted states, viewports, and locales. This is development evidence, not production or accessibility signoff.

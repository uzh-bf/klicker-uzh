...
- **Anonymous correlated live quiz** — a signed `LIVE_QUIZ_RESPONDENT` JWT scoped to one `liveQuizId`, initialized by `apps/response-api` and kept only in a host-only HttpOnly `live_quiz_respondent_token_<quiz-id>` cookie on the response API. The initialization body never returns the bearer token and the PWA does not read or forward it through JavaScript. This identity creates a `LiveQuizRespondent`, not a participant account, and cannot be reused for another quiz.

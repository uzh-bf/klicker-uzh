...
activity (`apps/frontend-manage/src/components/activities/creation/liveQuiz/submitLiveQuizForm.tsx:submitLiveQuizForm`). Aggregate anonymous collection is the default; assessment courses force that mode, and correlated export cannot be combined with gamification. Once a quiz is published, the wizard keeps the response mode and incompatible course choices locked. The student page (`apps/frontend-pwa/src/pages/session/[id].tsx:ensureCorrelatedResponseIdentity`) first initializes the identity with the HttpOnly respondent cookie, then retries initialization for a memory-only signed bearer token only after `POST /AddCorrelatedResponse` reports a missing identity. This keeps the token out of page JavaScript in the normal cookie-retained path while keeping an admitted cookie-blocked page usable. Aggregate responses continue through `POST /AddResponse`. The ended-quiz evaluation exposes the CSV download only when `canExportCorrelatedResponses` is true, in the footer overflow menu (`apps/frontend-manage/src/components/evaluation/EvaluationFooterMenu.tsx:EvaluationFooterMenu`); the activity list reaches the same download through `liveQuizCorrelatedExportReadiness` (see [Frontend Conventions](./frontend-conventions.md)).

...
CodeAPI rate limits are deferred in the receipt itself. A `429` releases the active claim back to `PENDING`, records a bounded `retryAt` from `Retry-After` (30 seconds by default, clamped to 1 second through 5 minutes), and completes the current Hatchet attempt successfully so it cannot spin. The minutely recovery task re-enqueues only receipts whose deferral is due. Other transient failures still use Hatchet retry behavior, while the database claim budget remains the final fence.

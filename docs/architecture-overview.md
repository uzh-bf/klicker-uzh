...
1. `apps/response-api` accepts aggregate responses on `POST /AddResponse`, correlated responses on the versioned `POST /AddCorrelatedResponse` path, and assessment responses on the separate assessment path. Shared code parses the request and loads instance metadata. Standard live quizzes use one of two explicit modes: `AGGREGATED_ANONYMOUS` keeps only Redis aggregates, while `CORRELATED_EXPORT` initializes a quiz-scoped pseudonymous identity only when the participant first submits. Correlated admission validates authored response dimensions and atomically creates the identity, permanent response key, encrypted acceptance snapshot, and outbox receipt under shared quiz/block locks.
2. `apps/hatchet-worker-response-processor` consumes `response-received:{authenticated|anonymous|assessment}` and `response-received:correlated-v1`. Aggregate and correlated events enter separate processors rather than a mode-flagged orchestrator. Hatchet carries only the correlated outbox message id; the correlated processor loads and decrypts the authoritative accepted event and durably persists it through normal quiz end. Database uniqueness and the atomic Redis processed marker make concurrent redelivery idempotent without a separate lease. The aggregate processor retains the legacy Redis-only path.

...
if [ -z "${CATALYST_FORMATIVE_EVALUATOR_URL+x}" ]; then
	if [ "$PLAYWRIGHT_SEMANTIC_EVALUATOR_STUB" = 'false' ]; then
		unset CATALYST_FORMATIVE_EVALUATOR_URL
	else
		export CATALYST_FORMATIVE_EVALUATOR_URL="http://127.0.0.1:${PLAYWRIGHT_SEMANTIC_EVALUATOR_PORT}/evaluate"
	fi
elif [ -z "$CATALYST_FORMATIVE_EVALUATOR_URL" ]; then
	unset CATALYST_FORMATIVE_EVALUATOR_URL
else
	export CATALYST_FORMATIVE_EVALUATOR_URL
fi
export SEMANTIC_EVALUATION_DISCLOSURE_VERSION="${SEMANTIC_EVALUATION_DISCLOSURE_VERSION:-playwright-v1}"

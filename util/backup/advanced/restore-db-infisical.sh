...
ENVIRONMENT VARIABLES:
    ...
    DATABASE_URL              PostgreSQL connection URL (Infisical environments)
...
restore_database() {
    ...
    if [[ -n "${DATABASE_URL:-}" ]]; then
        ...
        if pg_restore --dbname="$DB_CONN" --no-owner --format="t" --clean --verbose "$DUMP_FILE" 2>&1 | tee "$log_file"; then
            restore_exit_code=0
        else
        ...
        if pg_restore --host="${DATABASE_HOST}" --port="${DATABASE_PORT:-5432}" --user="${DATABASE_USER}" --dbname="${DATABASE_NAME}" --no-owner --format="t" --clean  --verbose "$DUMP_FILE" 2>&1 | tee "$log_file"; then
            restore_exit_code=0
        else
            restore_exit_code=${PIPESTATUS[0]}
        fi
    ...
    echo "  ⚠️  pg_restore completed with exit code $restore_exit_code (some errors may be expected)"
    restore_exit_code=0

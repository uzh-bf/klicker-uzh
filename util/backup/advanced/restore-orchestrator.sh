...
validate_dump_files() {
    ...
    if command -v jq &> /dev/null; then
        local temp_file="${state_file}.tmp"
        jq --arg db_dump "$db_dump" --arg redis_dump "$redis_dump" \
           '.source_dumps = {"db": {"file": $db_dump}, "redis": {"file": $redis_dump}}' \
           "$state_file" > "$temp_file" && mv "$temp_file" "$state_file"
    fi
...
create_pre_restore_backups() {
    ...
    if [[ "$DRY_RUN" != "true" ]] && command -v jq &> /dev/null; then
        ...
        jq --arg db_backup_dir "$db_backup_dir" --arg redis_backup_dir "$redis_backup_dir" \
           '.backup_created = {"db": {"directory": $db_backup_dir}, "redis": {"directory": $redis_backup_dir}}' \
           "$state_file" > "$temp_file" && mv "$temp_file" "$state_file"
    fi
...
execute_coordinated_restore() {
    ...
    local db_restore_cmd="${SCRIPT_DIR}/restore-db.sh $ENVIRONMENT"
    ...
    # Step 2: Restore Redis (only if database succeeded)
    if [[ "$db_restore_success" == "true" ]]; then
        ...
        local redis_restore_cmd="${SCRIPT_DIR}/restore-redis.sh $ENVIRONMENT"
        ...
        # Check final result
        if [[ "$redis_restore_success" == "true" ]]; then
            ...
            log "ERROR: Redis restore failed after database restore succeeded"
            update_operation_state "$state_file" "coordinated_restore" "failed" "Redis restore failed, initiating rollback"
...
find_backup_dump() {
    ...
    if [[ "$service" == "db" ]]; then
        ...
        pattern="${backup_prefix}_redis_dump_*.dump*"
    fi
...
get_backup_prefix_from_state() {
    ...
    for service in db redis; do
        ...
        if [[ "$service" == "db" ]]; then
            ...
            pattern="${pattern}_redis_dump_*.dump*"
        fi
...
rollback_operation() {
    ...
    local db_backup_file redis_backup_file
    ...
    local db_rollback_cmd="${SCRIPT_DIR}/restore-db.sh $ENVIRONMENT"
    ...
    if [[ "$db_rollback_success" == "true" ]]; then
        ...
        local redis_rollback_cmd="${SCRIPT_DIR}/restore-redis.sh $ENVIRONMENT"
        ...
        if [[ "$redis_rollback_success" == "true" ]]; then

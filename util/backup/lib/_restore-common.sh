...
validate_database_env() {
    ...
    if [[ -n "${DATABASE_URL:-}" ]]; then
        ...
        if [[ ! "$DATABASE_URL" =~ ^postgres(ql)?:// ]]; then
            validation_error "DATABASE_URL must start with 'postgres://' or 'postgresql://'"
        fi
    else
...
validate_redis_env() {
    ...
    if [[ -n "${REDIS_URL:-}" ]]; then
        ...
        # validate_env_var "REDIS_PASS" # empty password for dev Redis
    fi
...
cleanup_old_dumps() {
    ...
    local old_files
    old_files=$(find "$dump_dir" -name "*.tar*" -o -name "*.json*" -type f -mtime +$retention_days 2>/dev/null || true)
...
decrypt_dump_if_needed() {
    ...
    local temp_dump
    temp_dump=$(create_secure_temp_file "decrypted_dump" ".json")

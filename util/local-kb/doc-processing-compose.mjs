  ...
  return {
    services: {
      ...
      'doc-processing': {
        ...
        // This endpoint proves API liveness, not extraction readiness.
        healthcheck: {
          test: [
            'CMD',
            'python',
            '-c',
            "import urllib.request; response = urllib.request.urlopen('http://127.0.0.1:8000/health', timeout=3); assert response.status == 200",
          ],
          interval: '5s',
          timeout: '4s',
          retries: 24,

...
export function registerPracticeEscapeRoomTests() {
  ...
  test('Lecturer sees the roster before the first attempt', async ({
    page: testPage,
  }, testInfo) => {
    const page = testPage
    testInfo.setTimeout(600_000)
    ...
    await expect(
      page.getByTestId(`escape-room-attempt-${env('STUDENT_USERNAME')}`)
    ).toContainText(messages.manage.evaluation.escapeRoomStatusNotStarted)
  })

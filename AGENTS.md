...
- **Element semantic writes are not fully centralized**: `manipulateElement` is the main authoring path, but public catalog imports in `packages/graphql/src/services/sharing.ts` create `Element` rows directly, and first-login demo seeds in `packages/graphql/src/services/accounts.ts` do too. Any async work keyed off element creation/update must decide explicitly whether those paths are in scope.

...
- **New packages need a check-types.yml entry**: the root `check` script runs `turbo run check --parallel`, and `--parallel` ignores `dependsOn: ^build`, so CI typechecking relies on the hardcoded package build list in `.github/workflows/check-types.yml`. Any new workspace package that others import from `dist` must be added to that build step or consumer typechecks fail with TS2307. (`.github/workflows/check-types.yml`)
...
- **Active-KB-per-chatbot guard is service-level**: `linkKBChatbot` enforces "max one enabled KB per chatbot" in a single Prisma `$transaction` that demotes other enabled `KBChatbot` rows for the same chatbot before upserting; there is no DB-level partial unique index, so always go through the service to mutate the link. (`packages/graphql/src/services/knowledge.ts`)

...
- **FalkorDB chatbot graph helpers**: Chatbot-specific FalkorDB lifecycle and query helpers live in `@klicker-uzh/falkordb`; future chatbot create/delete lifecycle code should call this package rather than issuing raw `GRAPH.*` commands inline. (`packages/falkordb/src/index.ts`)

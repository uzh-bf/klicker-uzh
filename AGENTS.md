...
- **Next.js 16 Turbopack default**: Turbopack is the default bundler for both `next dev` and `next build` in Next 16. PWA apps need `next build --webpack` because `@ducanh2912/next-pwa` is webpack-only. The `chat` app also uses `next build --webpack` because its Turbopack production build fails inside the Alpine Docker `v3_chat-stg.yml` pipeline (App Router + standalone + Alpine combo); this is empirical, not by design — revisit when Next 16 Turbopack standalone is more stable. Non-PWA apps without that constraint (`auth`) use Turbopack for both `build` and `build:test`. (`apps/frontend-*/package.json`, `apps/chat/package.json`)

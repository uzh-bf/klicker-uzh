...
- **Next.js 16 Turbopack default**: Turbopack is the default bundler for both `next dev` and `next build` in Next 16. PWA apps need `next build --webpack` because `@ducanh2912/next-pwa` is webpack-only. (`apps/frontend-*/package.json`)

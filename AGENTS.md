...
- **Next.js 16 lint migration**: `next lint` was removed in Next 16. Replace it with `eslint .` backed by `eslint.config.mjs` flat config using `import coreWebVitals from 'eslint-config-next/core-web-vitals'`. Delete old `.eslintrc.json` files. `eslint-config-next` 16 bundles React Hooks 7 with new React Compiler rules (`set-state-in-effect`, `preserve-manual-memoization`, `error-boundaries`, etc.) set to error -- these flag pre-existing patterns and should be downgraded to warn until code is migrated. (`apps/*/eslint.config.mjs`)
- **Next.js 16 i18n App Router restriction**: The `i18n` key in `next.config` is forbidden in App Router apps. Destructure it out from the shared `getNextBaseConfig`: `const { i18n: _i18n, ...baseConfig } = getNextBaseConfig(...)`. (`apps/chat/next.config.ts`)
- **Next.js 16 Turbopack default**: Turbopack is the default bundler for both `next dev` and `next build` in Next 16. PWA apps need `next build --webpack` because `@ducanh2912/next-pwa` is webpack-only. The `chat` app also uses `next build --webpack` because Turbopack production builds do **not** produce the `.next/standalone` directory that `apps/chat/Dockerfile` copies into the runner image (the standalone tracing/server.js bundle is webpack-only as of Next 16.2.5; Turbopack writes only `.next/standalone/out/` which has no `apps/chat/server.js`). Without webpack, the Docker `COPY --from=installer /app/apps/chat/.next/standalone` step fails with `failed to compute cache key: ... '/app/apps/chat/.next/standalone': not found` (or similar). Non-PWA apps that don't go through Docker standalone (`auth`) use Turbopack for both `build` and `build:test`. (`apps/frontend-*/package.json`, `apps/chat/package.json`)
- **next-config webpack conditionNames must be kept**: The `webpack()` callback in `packages/next-config/index.js` that sets `config.resolve.conditionNames` is required for ALL webpack builds. Without it, `embla-carousel-react` (from `@uzh-bf/design-system`) resolves to its browser variant in the server bundle, causing `useEmblaCarousel is not defined` during page data collection. Turbopack ignores the callback, so it is safe to keep in the shared config. (`packages/next-config/index.js`)
- **syncpack sortFirst ordering**: Keys not in `.syncpackrc.mjs` `sortFirst` (e.g., `"type"`) must appear after all sortFirst keys in package.json or syncpack lint fails. (`packages/next-config/package.json`)

...
- **OLAT API CI Docker install**: `.github/workflows/test-olat-api.yml` already runs `pnpm/action-setup` on the Linux host; the OLAT test container should reuse mounted `node_modules` when `CI=true` instead of reinstalling the whole workspace. Local Docker runs still install in-container for platform correctness. (`apps/olat-api/test/docker/Dockerfile.test`, `apps/olat-api/test/docker/docker-compose.test.yml`)
...
- **Manage full-page screenshots**: `agent-browser screenshot --full` on Manage pages can include horizontal overflow outside the visual viewport. For fixed overlays like the assistant drawer, confirm viewport screenshots and `getBoundingClientRect()` before treating right-side blank space as a layout bug. (`apps/frontend-manage/src/components/assistant/ManageAssistantWidget.tsx`)
...
- **Course chatbot enrollment check**: `Participation.isActive` is leaderboard participation, not course enrollment. Student course-chatbot visibility should gate on participation existence unless leaderboard opt-in is explicitly part of the feature. (`packages/graphql/src/services/chatbots.ts`)
...
- **Chat Vitest global stubs**: Tests that call `vi.stubGlobal` must clean up with `vi.unstubAllGlobals()` or they can leak constructor replacements into later files under the shared Vitest process. (`apps/chat/test/`)
- **AI SDK MCP tool result rendering**: MCP tools loaded through AI SDK can arrive in assistant-ui as `{ content: [{ type: 'text', text: JSON.stringify(result) }] }`; unwrap this envelope before detecting structured tool-card results. (`apps/chat/src/components/tool-fallback.tsx`, `apps/chat/src/components/manage-proposal-card.tsx`)
...
- **Rollup + NodeNext build mode**: `@rollup/plugin-typescript` can parse raw TS/TSX or emit noisy false diagnostics when package tsconfigs use `module: "NodeNext"`; unbundled packages should use `tsc -p tsconfig.json` with `include: ["src/**/*"]`, while Rollup packages should override emit to `module: "ESNext"` / `moduleResolution: "Bundler"`. (`packages/*/package.json`, `packages/*/rollup.config.js`)
...
- **mcp-lecturer deployment boundary**: The Helm chart deploys `mcp-lecturer` as an internal ClusterIP service and injects `MCP_LECTURER_URL` into chat. Because `apps/mcp-lecturer` reads with Prisma, its deployment injects only `DATABASE_URL` from the backend GraphQL secret plus `APP_SECRET` from the chat secret instead of importing full secret sets. (`deploy/charts/klicker-uzh-v3/templates/deployment-mcp-lecturer.yaml`, `deploy/charts/klicker-uzh-v3/templates/cm-chat.yaml`)

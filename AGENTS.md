...
- **React Query disabled-query loaders**: Disabled React Query/tRPC queries can still report loading-like state when they have no data. For route-param-dependent queries, gate loader UI with the same condition used for `enabled`, otherwise overview pages can stay stuck on a spinner. (`apps/frontend-manage/src/components/catalog/CatalogBrowser.tsx`)
...
- **Turbo check dependency ordering**: Do not run the root `check` task with `--parallel`; frontend checks consume built `@klicker-uzh/graphql` artifacts, and `@klicker-uzh/prisma#check` should depend on its own `build` so generated Prisma client files exist without racing `prisma generate`. (`package.json`, `turbo.json`)

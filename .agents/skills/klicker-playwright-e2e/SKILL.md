...
- For correlated-response flows, assert the localized `live-quiz-response-collection-notice`, exercise the response-api identity cookie or the explicit post-401 memory-only signed respondent-token fallback before the first answer, and verify the Manage evaluation download control is present only for an ended correlated quiz. Keep aggregate and assessment flows covered separately because they use different response routes.

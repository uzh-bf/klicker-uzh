...
For Individual Escape Room changes, run the focused GraphQL lifecycle, completion, hint/reset, retention/progress, and QR contract tests together, then the ordered Chromium workflow in `playwright/tests/Z-escape-room.spec.ts`. Preserve the pre-attempt lecturer-roster assertion: enrolled participants must appear as `NOT_STARTED` before any response exists.

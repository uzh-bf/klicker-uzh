...
For chat dictation changes, keep the speech API fake-backed in Playwright:
install it with `page.addInitScript` before navigation, assert the exact local
availability/install options, and cover desktop, mobile, embedded, install,
interim/final, error, and draft-restore states. A fake proves the state
machine only; W0's negative real-device result must remain separate from any
positive browser-support claim.

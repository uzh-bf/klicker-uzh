...
For deployment scheduling changes (`topologySpreadConstraints`, affinity, selectors), a values-file read is not render proof: the production spread constraints stayed inert precisely because the values existed but no template rendered them. Run `node deploy/scripts/verify-topology-spread.mjs` — it invokes `helm template` on the prd values, parses the result with the repo's `yaml` library, and asserts that exactly the five intended Deployments (three Hatchet workers, assessment frontend, assessment backend) carry exactly two constraints each at `spec.template.spec` with constraint selectors matching the pod component labels, and that a chart-default render emits no spread field. Render proof is still not live-rollout proof: after Argo sync, confirm the deployed pod specs and actual pod placement.

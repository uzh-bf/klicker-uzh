...
For custom PostgreSQL DDL such as `CREATE INDEX CONCURRENTLY`, add an isolated migration proof: deploy the complete history into a disposable empty database, run `migrate diff --exit-code` against the schema, inspect the created catalog object, and remove the database. A successful incremental apply to an already-mutated development database is not sufficient.
...
For Escape Room changes, run the focused GraphQL lifecycle, completion, hint/reset, retention/progress, QR contract/placement, and affected mode tests together, then the ordered Chromium workflow in `playwright/tests/Z-escape-room.spec.ts`. Preserve pre-attempt roster assertions: enrolled participants and course groups must appear as `NOT_STARTED` before an attempt exists. Group mode must cover two members sharing start, hints, lockout, completion, monitor state, and reset; include read-only content beside an answerable question to prove the client submits only the exact answer set. Also cover a participant in multiple same-course groups so routed group identity is deterministic and membership-checked. Live Quiz mode additionally runs the Response API enforcement suite, util response-closure gate suite, response-processor deduplication suite, PWA response parsing test, and template suite. Preserve exact-origin CORS, close-vs-response draining, pending-worker-event removal, course-roster `NOT_STARTED`, and participant-claim recovery assertions. The browser flow must exercise the real response API and worker while checking authoring round-trip, explicit start, reload, lockout, cockpit progress, completion, and reset.

...
| Chat app logic                               | `pnpm --filter @klicker-uzh/chat test:run` — safe with no services                                                                                                                          |
...
For correlated Live Quiz response changes, verify the full durable boundary: aggregate participation never initializes a correlated identity; the discriminated question metadata contract and authored choice, selection, and case-study bounds fail before outbox acknowledgement; active-generation identity creation, current PIN verification, and outbox insertion are atomic under shared lifecycle locks; duplicate acknowledgement follows the permanent unique database response key; Hatchet receives only `{messageId}`; an accepted event persists through normal quiz end but not abort, mode change, or execution change; direct insert plus `P2002` re-read recovers concurrent delivery of the same event; Redis effects converge through the processed marker; excessive mutation plans settle terminally; abort cannot delete a newer Redis publication generation; free-text questions are excluded before correlated-export size preflight and CSV materialization; and terminal handling marks the receipt settled while erasing ciphertext. Run the response API, response worker, export, focused GraphQL policy, database-backed response-mode/concurrency, abort, publication, and correlated-export suites together. Publication changes also require a clean migration run that proves concurrent index creation, disabled/enabled capability-gate coverage, and the database-backed materialization/reconciliation cases.

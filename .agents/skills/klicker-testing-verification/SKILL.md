...
| Assessment audit contract/storage                                                 | `pnpm --filter @klicker-uzh/audit test` inside the DevPod — integration cases require disposable PostgreSQL and pinned Azurite; then run focused GraphQL audit specs                        |
...
For the lecturer AI gate, verify the GrowthBook `ai-beta` flag and the live `User.aiFeaturesEnabled` entitlement independently, including fail-closed missing-account behavior and no entitlement query while the flag is off. Every lecturer KB and question/graph-generation entry point retains the AI approval and returns `AI_BETA_ACCESS_REQUIRED` when the combined gate is closed; chatbot authoring uses the beta preference and rollout without requiring AI approval, while chatbot publication and model usage retain it. Participant chatbot discovery and worker-only KB settlement stay available. Toggle `KB_INGESTION_DISABLED` at call time and prove it blocks upload-ticket issue, URL creation, and ingestion while leaving deletion available. Browser proof covers Resources navigation and the element-library generation action for an enabled lecturer, plus direct-route denial for a disabled one.

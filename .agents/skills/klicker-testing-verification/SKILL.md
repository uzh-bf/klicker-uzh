...
For correlated Live Quiz response changes, verify the full durable boundary: aggregate participation never initializes a correlated identity, malformed responses fail before outbox acknowledgement, duplicate acknowledgement follows the permanent unique database response key, Hatchet receives only `{messageId}`, the worker rejects or ignores delivery without a matching unsettled outbox row, acceptance-time metadata wins over restarted Redis state, lifecycle-locked persistence cannot survive a concurrent abort, and terminal handling marks the receipt settled while erasing ciphertext. Run the response API, response worker, export, focused GraphQL policy, database-backed response-mode/concurrency, abort, publication, and correlated-export suites together. Publication changes also require a clean migration run plus the database-backed publication cases for after-commit Redis materialization, generation-scoped acknowledgement, retry recovery, and poison-row backoff.

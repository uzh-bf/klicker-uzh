...
free-text must be excluded before size checks, the combined CSV header/response
size and respondent-by-column matrix must be bounded before response rows are
loaded, and the query must remain blocked until all accepted outbox rows are
...
including the explicit post-401 memory-only respondent-token fallback when the
response cookie is blocked, and the ended-quiz evaluation download/error states
in both locales.

...
Participant CODE browser proof must exercise a published practice quiz with a real activity query: submit, persist an active receipt, reload while pending, complete and reveal only public results, ignore a stale active result after completion, restore the submitted code, switch participants without leaking receipt or completion state, fail, edit, and retry to a new receipt. Use `playwright/tests/Q-code-practice-quiz.spec.ts` for deterministic receipt transitions. In the self-contained devcontainer, preserve the real backend by forwarding unmatched GraphQL operations to its container-local port; mock only the external grading lifecycle.

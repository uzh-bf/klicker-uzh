...
For correlated Live Quiz response changes, verify the full durable boundary: aggregate participation never initializes a correlated identity; authored choice, selection, and case-study bounds fail before outbox acknowledgement; active-generation identity creation and outbox insertion are atomic under shared lifecycle locks; duplicate acknowledgement follows the permanent unique database response key; Hatchet receives only `{messageId}`; an accepted event persists through normal quiz end but not abort, mode change, or execution change; concurrent same-event processing converges through uniqueness recovery and the Redis marker; excessive mutation plans settle terminally; abort cannot delete a newer Redis publication generation; and terminal handling marks the receipt settled while erasing ciphertext. Run the response API, response worker, export, focused GraphQL policy, database-backed response-mode/concurrency, abort, publication, and correlated-export suites together. Publication changes also require a clean migration run, disabled/enabled capability-gate coverage, and the database-backed materialization/reconciliation cases.

...
- **Budget locks for existing-table indexes and constraints** — inspect production table size and write traffic before shipping a non-concurrent index or validation scan. Record whether the migration needs a maintenance window; migration-first rollout order does not make an unbounded lock acceptable.

...
Run Prisma client generation only through `pnpm --filter @klicker-uzh/prisma generate` (or a build that calls it). The package script owns the TypeScript 6 generated-namespace patch; direct `prisma generate` bypasses it. If generation fails at the patch, run `pnpm --filter @klicker-uzh/prisma test:patch-namespace` and inspect the new generated declarations instead of weakening the exact-cardinality guard.

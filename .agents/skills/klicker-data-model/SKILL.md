...
- **Chat prompt-catalog rows are immutable** (ADR 0043): ChatbotModePromptVersion and ChatbotEffectiveSystemPrompt reject identity/text updates and guarded deletes; ChatbotMode itself cannot be deleted directly while its versions exist, so lineage only disappears with the owning chatbot. Initialize exact legacy projections with `ensureChatbotPromptCatalog`; record every accepted text change with `appendChatbotModePromptVersion`; use the status and presentation helpers for non-authored changes so they never create versions. All writer calls stay inside a caller-supplied transaction, and migration 20260823180000 embeds a frozen tutor-default literal that must stay byte-stable.

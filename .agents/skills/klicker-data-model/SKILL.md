...
Use the supported `prisma-data` seed wrappers rather than invoking `seedTEST.ts`, `seedFlashcards.ts`, or internal `seed:test:raw` directly. `seed`, `seed:raw`, `seed:test`, and `seed:qa` run the GraphQL stale-fingerprint repair bootstrap only after their guarded test seed succeeds. A rejected database or failed seed must stop before assessment seeding or fingerprint writes. The separately authorized `seed:flashcards` and `seed:prod:flashcards` wrappers retain repair after a partial writer failure while preserving a non-zero overall exit. The bootstrap inherits the wrapper's already-selected `DATABASE_URL`, takes the rollout advisory lock, and is bounded to ten passes/four minutes; it must not select an environment independently or expand into an unbounded historical backfill. Any direct seed helper that updates an existing element must clear `importFingerprint` and `importFingerprintVersion` in the same update so the bounded repair finds semantic changes without a full-corpus scan.

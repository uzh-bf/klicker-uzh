...
- **Finalized CODE details carry trusted correctness** — the worker persists submitted code plus its server-computed `0..1` correctness in `QuestionResponseDetail.response`. Analytics maps that trusted value to incorrect/partial/correct and must never try to re-execute the code.

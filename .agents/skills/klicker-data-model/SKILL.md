...
- **Student-owned cards** use the `PersonalElement`, `CardGenerationLease`, and `PersonalElementDiscard` models. Their `origin`, `options`, and `sources` fields remain explicit or typed JSON, source metadata is bounded and contains no retrieved text, and the participant/course relations must be kept in sync with the schema file and migration. Personal cards are source-linked but have no lecturer verification state; course adoption is a later copy into a lecturer-owned `Element`.

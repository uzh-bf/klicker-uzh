...
- **CODE attempts are receipt-backed** — `packages/prisma/src/prisma/schema/code.prisma` owns the async `CodeSubmission` lifecycle. Its exactly-one-activity check and active-attempt uniqueness are raw SQL: `20260723170000_add_code_element_contracts` establishes the base fence and `20260828082831_add_code_live_quiz_support` splits non-LiveQuiz uniqueness from execution-scoped LiveQuiz uniqueness. A LiveQuiz receipt must carry both `liveQuizId` and `elementBlockExecution`; preserve all three constraints when changing the model. Rate-limit deferral uses `retryAt` plus the `(status, retryAt, createdAt)` recovery index; pending scans must exclude future deferrals. Timing policy lives in [Async and workers](../../../docs/async-and-workers.md).
...
- **Finalized CODE details carry trusted correctness** — the worker persists submitted code plus its server-computed `0..1` correctness in `QuestionResponseDetail.response` for PracticeQuiz/MicroLearning and `LiveQuizResponse.response` for LiveQuiz. Analytics and exports consume that trusted value and must never try to re-execute the code.

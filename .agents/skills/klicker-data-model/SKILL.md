...
- **KB graph cost state is a locked ledger** — `KBGraphQuota` is unique per owner and semester, `KBGraphBuild.costStatus` fences reservation settlement by build id, and all monetary values are integer minor units. Use the `ON CONFLICT DO NOTHING` insert followed by a row lock for concurrent first reservations; invalid or mismatched terminal results become `NEEDS_HUMAN_REVIEW` and retain the reservation rather than publishing.

...
- **Student-owned cards** use the `PersonalElement` and `ChatGenerationApproval` models. Their `options` and `sources` fields remain typed JSON, source metadata is bounded and contains no retrieved text, and the participant/course relations must be kept in sync with the schema file and migration.

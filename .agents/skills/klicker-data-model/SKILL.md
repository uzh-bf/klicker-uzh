...
- **Decimal fields**: TS-side Prisma `Decimal` values are objects, so never truthy-check them. Python Analytics maps decimals through the curated `apps/analytics/src/models.py`. After a migration, create the ignored `src/models.generated.py` reference with `pnpm --filter @klicker-uzh/analytics generate:raw` in the self-contained DevPod or `pnpm --filter @klicker-uzh/analytics generate` on the legacy host with Infisical, then reconcile only Analytics-used changes into `src/models.py`.

...
2. **Layer footprint** — which of: Prisma schema (→ migration + sync + seeds), `packages/types`, GraphQL schema/service/ops (→ codegen and tracked SDL snapshot), frontend app(s), i18n, Hatchet tasks? List the packages you will touch.
3. **Auth** — who may call it? Pick the layer-1 scope object (`asUser`/`asParticipant`/…) and, for object-scoped operations, the `PermissionLevel` (READ/EXECUTE/WRITE/ADMIN). ([docs/graphql-api-layer.md](../../../docs/graphql-api-layer.md)) For participant Chat surfaces, require the participant JWT, a published chatbot, and any course `Participation`; never use `Participation.isActive` as enrollment or authorization.

...
- **Managed profiles (0.0.39+)**: consumers declare a `managedRuntime` registry (base and profile-owned Compose services, process markers) and profiles select routed apps, services, and processes independently. Merged `--profile a,b` selections are additive and order-insensitive; the generated selective Dev Container config is ignored runtime state, never a committed artifact. Warm transitions add/drop exact services and processes without recreating the container or rerunning post-create.

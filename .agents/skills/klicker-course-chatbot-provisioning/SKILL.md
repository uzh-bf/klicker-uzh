...
```bash
ingestion-cli video-import lecture --video <file> --course <slug> [--asr-language de] [--activate] [--dispatch]
```

`--dispatch` moves inventory, package, prepare and the `--activate` corpus write off the operator laptop to the deployed data-ingestion embedding worker, which holds the only in-cluster embedding credential and Milvus writer. The laptop uploads the run tree under the `video-import/<job_id>/` prefix of a shared hand-off store, triggers the `video-candidate-import` workflow, and reads completion from a create-only receipt blob in that store — not from the video-processing edge, so a WAF-blocked or restarted laptop cannot lose a run that already finished. The worker resolves the run tree's own control file for identity; the payload is only `{job_id, activate}`. Hand-off environment: `INGESTION_VIDEO_HANDOFF_{BACKEND,CONTAINER,ACCOUNT_URL,CONNECTION_STRING,PREFIX,LOCAL_ROOT}` (`azure_blob` default, `local` for a rehearsal on one host). The store must be reachable by both sides, which the PRD ingestion artifact account is not from a laptop (private-endpoint-only); until the operator has a write role there or an in-cluster relay exists, `--dispatch` is not drivable end to end from a laptop and the local lane is the fallback.
...
- The producer names one course-generic eligibility descriptor for every course: `klicker-course-generic-hs26-eligibility.v1`, digest `sha256:d5838eeb…`. It was previously named `informatik-und-wirtschaft-hs26-eligibility.v1` (`sha256:08863ea7…`), and a source published under the old name stays valid because a published source is immutable and the consumer accepts its exact frozen predecessor identity. A binding that fails on `policy identity does not match` means the deployed producer and the binding disagree on the id and digest, which happens until the renamed-descriptor worker image is rolled out.
...
- Held-back units never reach the candidate package, and the run writes the reviewer record for them: `quarantine-decisions.json` in the run tree, named in the receipt as `quarantine_decisions`. It carries one row per held-back unit — `unit_id`, `disposition` (`quarantine` | `excluded`), `reason_code`, and `decision` / `reviewer` / `team` / `decided_at` / `note` left empty for the course team; `decision` is `accept`, `reprocess`, or `leave_out`. The file is create-only, so filling it in and re-running the import cannot lose a decision.
...
- Verify from the Klicker side: the imported-sources section lists the lecture with chunk counts, then an owner-preview retrieval question cites the video by name and timestamp. Inventory honesty: scans are bounded (`incomplete`, `unidentifiedChunks` are flags, not errors) and a zero row means no chunk metadata matched the KB scope filter. The candidate lane stamps `kb_id` alongside `chatbot_id`, so before reading a zero as an empty corpus, confirm the serving side: `tools/list` on the tenant endpoint must list `doc_query_sources` next to `doc_query` (the companion is registered only for a config that declares `token_scope`), and a direct call needs the transport bearer in `Authorization` plus an ES256 scope token in the configured scope header, whose value carries a `Bearer ` prefix: a bare JWT is rejected as `unauthorized: missing bearer token`.
...
## Import a prepared corpus (imported lane)

Scraped websites and exported document sets an acquisition tool has already pulled reach the KB through the normal data-ingestion pipeline, not a side path: the apply chunks, contextualizes and embeds server-side, and rows land in the KB's own collection as imported resources, visible read-only in Manage beside app-added ones. The acquisition tool owns the corpus — what is in, what a refresh retires — and this lane owns the transport.

One command in the data-ingestion checkout publishes a reviewed package:
...
- `--apply` uploads the reviewed Markdown under the binding's prefix and dispatches the same catalog apply the normal lane uses into the KB's `expected_collection`. Environment: `INGESTION_CONFIG_DIR` pointing at the deployment repo's runtime project config for the environment; the command fails closed when the configured collection is not the binding's `expected_collection`. Optional receipts: `--stamped-catalog-out` (the dispatched catalog, binding included) and `--results-out` (apply results read back from the store).
- Verify from the Klicker side: the KB's imported-sources section lists the corpus with chunk counts, and an owner-preview retrieval question cites its sources with working public links.

**Done when** the imported-sources section lists the corpus, retrieval cites its sources, and the receipts exist: the confirmed plan (buckets and delete counts) and the apply results.

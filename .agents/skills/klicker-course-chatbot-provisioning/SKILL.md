...
- Verify from the Klicker side: the imported-sources section lists the lecture with chunk counts, then an owner-preview retrieval question cites the video by name and timestamp. Inventory honesty: scans are bounded (`incomplete` and `unidentifiedChunks` are flags, not errors), and a zero-row inventory means no chunk metadata matched the KB scope filter — check that the imported chunks carry `kb_id` and that the deployed doc-query image exposes the scope-guarded inventory tool, which is generated only for a tenant config that declares `token_scope`.

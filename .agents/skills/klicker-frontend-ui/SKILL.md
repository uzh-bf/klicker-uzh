...
2. **Build** with the checklist:
   ...
   - Escape Room timers, progress, hints, lockouts, and stage availability render from server snapshots. Local state may animate or preserve form input, but it must not authorize advancement or reconstruct hidden stages.

...
- KB catalog/detail scale uses server-backed search/filter connections and explicit load-more controls. Poll only loaded resource rows while one is active; use indeterminate operation progress, not fabricated percentages. Keep bulk deletion behind a named confirmation, and expose source, operation-versus-serving state, contextual actions, and lazy history in the keyboard-accessible inspector.

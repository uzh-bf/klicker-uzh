...
2. **Build** with the checklist:
   ...
   - Non-modal assistant docks use a labelled complementary region with `aria-controls` / `aria-expanded`, no focus trap, no `aria-modal`, and no inert background. Move focus into the dock on open, keep Escape and a close control available across embedded origins, restore launcher focus on close, and browser-test that an uncovered page control remains usable while the dock is open. Persist desktop dimensions only from the desktop layout so mobile clamping does not overwrite them.

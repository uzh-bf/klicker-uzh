...
2. **Build** with the checklist:
   ...
   - Escape Room timers, progress, hints, lockouts, and stage availability render from server snapshots. Animate `remainingSeconds` and `expiresInSeconds` with `performance.now()`; never reconstruct the deadline from the browser wall clock. Local state may preserve form input, but it must not authorize advancement or reconstruct hidden stages. Group Activity start and hint calls include the routed `groupId`; the server remains responsible for membership authorization. Group Activities submit one exact atomic answer set: keep read-only content visible, but filter content and flashcards out of the submitted response list. Live Quiz Escape Room blocks require an explicit start before showing the current question, refetch after accepted answers and lifecycle actions, and discard attempt-keyed local response state after a lecturer reset.

...
2. **Build** with the checklist:
   ...
   - Assistant docks are a labelled complementary region on desktop with `aria-controls` / `aria-expanded`, no focus trap, no `aria-modal`, and no inert background. Below the desktop breakpoint, the same dock becomes a full-viewport dialog with contained focus, `aria-modal`, and only the Manage app-content sibling inert and `aria-hidden`; restore that isolation on close, breakpoint change, or route exit. Keep Escape and a close control available across embedded origins, restore launcher focus on close, browser-test an uncovered page control on desktop, and persist dimensions only from the desktop layout so compact clamping does not overwrite them.
   - Treat a validated cross-origin child handshake as iframe readiness. A bounded missing-handshake deadline is delayed, not failed: keep that iframe alive for late recovery, offer a retry that remounts one generation, and reserve failure copy for an actual iframe error. Close, Escape, and an explicitly fresh standalone fallback remain available in every recovery state.
   ...
   - The floating Manage assistant launcher stays icon-only while closed and reserves no global Layout padding. Keep list-ending controls clear with local horizontal desktop space and only necessary compact bottom clearance.
   ...
   - Generated-element review uses a compact semantic inbox and opens one draft in the canonical `ElementEditForm`. Show pinned original resource titles, kinds, and citation pages rather than normalized artifact filenames. Keep one draft atomically into an ordinary Element with Review as the default status, retain discarded rows with Restore, and link kept rows to the ordinary Element editor. Keep the build-wide save action only for legacy accepted-but-unsaved drafts.
...
- Knowledge-base management is a reusable package mounted by `frontend-manage`: edit `packages/kb-management`, not duplicate app-local components. The detail resource workspace uses a semantic metadata table, one `+ Add resource` chooser for Website/Document with Video disabled until supported, and an overflow menu for destructive row actions. Preserve the explicit `ADDED`-before-Ingest lifecycle and keep the chooser non-dismissible after a file upload ticket is requested until confirmation or terminal failure. Verify `/resources/knowledgeBases` plus the detail route at desktop widths, both locales, and every changed empty/active/success/failure state; mobile layout is not a Manage-app priority.
- The KB and chatbot management routes live under one top-level AI dropdown in `Header.tsx`, placed between Resources and Analytics and visible only when `useAiFeaturesEnabled()` is open (GrowthBook `ai-beta` AND `User.aiFeaturesEnabled`); both entries carry the localized Beta features badge. The Resources dropdown no longer contains them. Direct AI routes must render the localized unavailable state when the gate is closed and never redirect on the transient browser gate; GraphQL independently applies the same two conditions and returns `AI_BETA_ACCESS_REQUIRED` for denied management calls. Never rely on hidden navigation as authorization.
...
- File replacement is available only for BLOB resources. Require an explicit file selection followed by `Replace and ingest`; confirmation preserves the row identity, title, category, and previous AI-serving version while immediately queuing the new canonical source. State that the prior source file cannot be restored. On queue failure, refresh the row and use its ordinary retry action rather than asking for another upload.

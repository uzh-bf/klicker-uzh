...
3. **Auth on the field** — always declare the role with `t.withAuth(...)`. For shareable aggregates represented by `PermissionCheck`, copy the existing composition exactly (real shape from `requestCourseDeletion` in `mutation.ts`; `withPermission` WRAPS the resolver):
...
Generated-element review persists one lecturer intent through `keepGeneratedElementDraft`: validate an owned terminal draft and its revision, create the ordinary Element through the canonical manipulation service, and link the accepted draft in the same database transaction. Carry assessment choice IDs beside the ordinary Element options so generated-draft identity survives reorder, deletion, and replacement without adding IDs to persisted Element options. A retry with the consumed revision returns the existing link; another stale revision fails. Keep the generated type fixed, preserve Flashcard card-type metadata, and expose pinned source title, kind, and URL on the build so citation pages resolve to the original resource. Retain `saveGeneratedElements` only for legacy accepted-but-unsaved recovery.

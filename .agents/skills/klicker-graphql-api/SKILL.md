...
3. **Auth on the field** — copy the existing composition exactly (real shape from `deleteCourse` in `mutation.ts`; `withPermission` WRAPS the resolver):
   ...
   Escape Room participant mutations accept the activity identity, action payload, and—for routed Group Activity calls—the selected `groupId`. Treat that group ID as resource identity, not authorization: verify the activity-course match and authenticated membership before using it as the lifecycle actor. Resolve ownership, the current attempt, active stack, timer, lockout, and preview authority from server state; never accept client-provided ownership or progression flags. Group decisions must recheck membership and use one serializable transaction for the exact answer set, aggregate results, and completion or lockout.
